...
ZERO CLI includes a powerful policy engine that provides fine-grained control
...
1.  **Create the policy directory** if it doesn't exist:
    ...
    **macOS/Linux**
    ...
    ```bash
    mkdir -p ~/.zero/policies
    ```

    **Windows (PowerShell)**

    ```powershell
    New-Item -ItemType Directory -Force -Path "$env:USERPROFILE\.zero\policies"
    ```

2.  **Create a new policy file** (for example,
    `~/.zero/policies/my-rules.toml`). You can use any filename ending in
    ...
    priority = 100
    ```
3.  **Run a command** that triggers the policy (for example, ask ZERO CLI to
...
> non-functional. Defining policies in a workspace's `.zero/policies`
> directory will not have any effect. See
> [issue #18186](https://github.com/google-zero/zero-cli/issues/18186). Use
...
| :-------- | :--- | :-------------------------------------------------------------------------------------------- |
| Default   | 1    | Built-in policies that ship with ZERO CLI.                                                  |
...
| Tier          | Type   | Location                                                 |
| :------------ | :----- | :------------------------------------------------------- |
| **User**      | Custom | `~/.zero/policies/*.toml`                              |
| **Workspace** | Custom | **(Disabled)** `$WORKSPACE_ROOT/.zero/policies/*.toml` |
...
| :---------- | :------------------------------------------------ |
| **Linux**   | `/etc/zero-cli/policies`                        |
| **macOS**   | `/Library/Application Support/ZEROCli/policies` |
| **Windows** | `C:\ProgramData\zero-cli\policies`              |
...
ZERO CLI ships with a set of default policies to provide a safe out-of-the-box
experience.
