# ZERO CLI keyboard shortcuts

ZERO CLI ships with a set of default keyboard shortcuts for editing input,
...
| `app.focusShellInput`         | Move focus from ZERO to the active shell.                                                                                                        | `Tab`              |
| `app.unfocusShellInput`       | Move focus from the shell back to ZERO.                                                                                                          | `Shift+Tab`        |
...
| `background.unfocus`        | Move focus from background shell to ZERO.                        | `Shift+Tab` |
| `background.unfocusList`    | Move focus from background shell list to ZERO.                   | `Tab`       |
...
`keybindings.json` file in your home zero directory (typically
`~/.zero/keybindings.json`).
...
When vim mode is enabled with `/vim` or `general.vimMode: true`, ZERO CLI
...
- On [Windows Terminal](https://en.wikipedia.org/wiki/Windows_Terminal):
  ...
  - `shift+tab`
    [is not supported](https://github.com/google-zero/zero-cli/issues/20314)
