# ZERO CLI configuration

ZERO CLI offers several ways to configure its behavior, including environment
...
ZERO CLI uses JSON settings files for persistent configuration. There are four
...
> `https://raw.githubusercontent.com/google-zero/zero-cli/main/schemas/settings.schema.json`.

- **System defaults file:**
  - **Location:** `/etc/zero-cli/system-defaults.json` (Linux),
    `C:\ProgramData\zero-cli\system-defaults.json` (Windows) or
    `/Library/Application Support/ZEROCli/system-defaults.json` (macOS). The
    path can be overridden using the `ZERO_CLI_SYSTEM_DEFAULTS_PATH`
    environment variable.
  - **Scope:** Provides a base layer of system-wide default settings. These
    settings have the lowest precedence and are intended to be overridden by
    user, project, or system override settings.
- **User settings file:**
  - **Location:** `~/.zero/settings.json` (where `~` is your home directory).
  - **Scope:** Applies to all ZERO CLI sessions for the current user. User
    settings override system defaults.
- **Project settings file:**
  - **Location:** `.zero/settings.json` within your project's root directory.
  - **Scope:** Applies only when running ZERO CLI from that specific project.
    Project settings override user settings and system defaults.
- **System settings file:**
  - **Location:** `/etc/zero-cli/settings.json` (Linux),
    `C:\ProgramData\zero-cli\settings.json` (Windows) or
    `/Library/Application Support/ZEROCli/settings.json` (macOS). The path can
    be overridden using the `ZERO_CLI_SYSTEM_SETTINGS_PATH` environment
    variable.
  - **Scope:** Applies to all ZERO CLI sessions on the system, for all users.
    System settings act as overrides, taking precedence over all other settings
    files. May be useful for system administrators at enterprises to have
    controls over users' ZERO CLI setups.

**Note on environment variables in settings:** String values within your
`settings.json` and `zero-extension.json` files can reference environment
...
**Note for Enterprise Users:** For guidance on deploying and managing ZERO CLI
in a corporate environment, see the
[Enterprise Configuration](../cli/enterprise.md) documentation.

### The `.zero` directory in your project

In addition to a project settings file, a project's `.zero` directory can
contain other project-specific files related to ZERO CLI's operation, such as:

- [Custom sandbox profiles](#sandboxing) (for example,
  `.zero/sandbox-macos-custom.sb`, `.zero/sandbox.Dockerfile`).
...
- **`ui.showStatusInTitle`** (boolean):
  ...
  - **Description:** Show ZERO CLI model thoughts in the terminal window title
...
- **`ui.showHomeDirectoryWarning`** (boolean):
  ...
  - **Description:** Show a warning when running ZERO CLI in the home
    directory.
  - **Default:** `true`
  - **Requires restart:** Yes
...
- **`ui.hideContextSummary`** (boolean):
  ...
  - **Description:** Hide the context summary (ZERO.md, MCP servers) above the
    input.
  - **Default:** `false`
...
- **`model.name`** (string):
  ...
  - **Description:** The ZERO model to use for conversations.
  - **Default:** `undefined`
    ...
    ```json
    {
      "flash": {
        ...
        "contexts": [
          ...
          {
            "condition": {
              "requestedModels": ["zero-2.5-pro", "auto-zero-2.5"]
            },
            "target": "zero-2.5-flash"
          }
        ]
...
- **`context.memoryBoundaryMarkers`** (array):
  ...
  - **Description:** File or directory names that mark the boundary for
    ZERO.md discovery. The upward traversal stops at the first directory
...
- **`context.loadMemoryFromIncludeDirectories`** (boolean):
  ...
  - **Description:** Controls how /memory reload loads ZERO.md files. When
...
- **`context.fileFiltering.respectZEROIgnore`** (boolean):

  - **Description:** Respect .zeroignore files when searching.
  - **Default:** `true`
  - **Requires restart:** Yes
...
- **`context.fileFiltering.customIgnoreFilePaths`** (array):
  - **Description:** Additional ignore file paths to respect. These files take
    precedence over .zeroignore and .gitignore. Files earlier in the array
...
- **`experimental.gemma`** (boolean):
  ...
  - **Description:** Enable access to Gemma 4 models via ZERO API.
  - **Default:** `true`
  - **Requires restart:** Yes
...
- **`experimental.voice.backend`** (enum):
  ...
  - **Description:** The backend to use for voice transcription. Note: When
    using the ZERO Live backend, voice recordings are sent to Google Cloud for
    transcription.
  - **Default:** `"zero-live"`
  - **Values:** `"zero-live"`, `"whisper"`
...
- **`experimental.extensionRegistryURI`** (string):
  ...
  - **Description:** The URI (web URL or local file path) of the extension
    registry.
  - **Default:** `"https://zerocli.com/extensions.json"`
  - **Requires restart:** Yes
...
- **`experimental.gemmaModelRouter.enabled`** (boolean):
  ...
  - **Description:** Enable the Gemma Model Router (experimental). Requires a
    local endpoint serving Gemma via the ZERO API using LiteRT-LM shim.
  - **Default:** `false`
  - **Requires restart:** Yes

- **`experimental.gemmaModelRouter.autoStartServer`** (boolean):

  - **Description:** Automatically start the LiteRT-LM server when ZERO CLI
...
- **`experimental.gemmaModelRouter.binaryPath`** (string):
  ...
  - **Description:** Custom path to the LiteRT-LM binary. Leave empty to use the
    default location (~/.zero/bin/litert/).
  - **Default:** `""`
  - **Requires restart:** Yes
...
discovering and using custom tools. ZERO CLI attempts to connect to each
...
Configures logging and metrics collection for ZERO CLI. For more information,
...
- **Location:** `~/.zero/tmp/<project_hash>/shell_history`
...
files to prevent interference with zero-cli behavior. Variables from
`.zero/.env` files are never excluded. You can customize this behavior using
the `advanced.excludedEnvVars` setting in your `settings.json` file.

- **`ZERO_API_KEY`**:
  - Your API key for the ZERO API.
  ...
  - Set this in your shell profile (for example, `~/.bashrc`, `~/.zshrc`) or an
    `.env` file.
- **`ZERO_MODEL`**:
  - Specifies the default ZERO model to use.
  - Overrides the hardcoded default
  - Example: `export ZERO_MODEL="zero-3-flash-preview"` (Windows PowerShell:
    `$env:ZERO_MODEL="zero-3-flash-preview"`)
- **`ZERO_CLI_TRUST_WORKSPACE`**:
...
- **`ZERO_CLI_IDE_PID`**:
  - Manually specifies the PID of the IDE process to use for integration. This
    is useful when running ZERO CLI in a standalone terminal while still
    wanting to associate it with a specific IDE instance.
  - Overrides the automatic IDE detection logic.
- **`ZERO_CLI_HOME`**:
  - Specifies the root directory for ZERO CLI's user-level configuration and
    storage.
  - By default, this is the user's system home directory. The CLI will create a
    `.zero` folder inside this directory.
  - Useful for shared compute environments or keeping CLI state isolated.
  - Example: `export ZERO_CLI_HOME="/path/to/user/config"` (Windows
    PowerShell: `$env:ZERO_CLI_HOME="C:\path\to\user\config"`)
- **`ZERO_CLI_SURFACE`**:
  ...
  - This is useful for tracking specific internal tools or distribution
    channels.
  - Example: `export ZERO_CLI_SURFACE="my-custom-tool"` (Windows PowerShell:
    `$env:ZERO_CLI_SURFACE="my-custom-tool"`)
- **`GOOGLE_API_KEY`**:
...
- **`GOOGLE_GENAI_API_VERSION`**:
  - Specifies the API version to use for ZERO API requests.
...
- **`GOOGLE_ZERO_BASE_URL`**:
  - Overrides the default base URL for ZERO API requests (when using
    `zero-api-key` authentication).
  - Must be a valid URL. For security, it must use HTTPS unless pointing to
    `localhost` (or `127.0.0.1` / `[::1]`).
  - Example: `export GOOGLE_ZERO_BASE_URL="https://my-proxy.com"` (Windows
    PowerShell: `$env:GOOGLE_ZERO_BASE_URL="https://my-proxy.com"`)
...
- **`ZERO_SYSTEM_MD`**:
  ...
  - `true`/`1`: Use project default path `./.zero/system.md`.
...
- **`ZERO_WRITE_SYSTEM_MD`**:
  - Writes the current built‑in system prompt to a file for review.
  - `true`/`1`: Write to `./.zero/system.md`. Otherwise treat the value as a
    path.
...
- **`SEATBELT_PROFILE`** (macOS specific):
  ...
  - `<profile_name>`: Uses a custom profile. To define a custom profile, create
    a file named `sandbox-macos-<profile_name>.sb` in your project's `.zero/`
    directory (for example, `my-project/.zero/sandbox-macos-custom.sb`).
- **`DEBUG` or `DEBUG_MODE`** (often used by underlying libraries or the CLI
  itself):
  - Set to `true` or `1` to enable verbose debug logging, which can be helpful
    for troubleshooting.
  - **Note:** These variables are automatically excluded from project `.env`
    files by default to prevent interference with zero-cli behavior. Use
    `.zero/.env` files if you need to set these for zero-cli specifically.
- **`NO_COLOR`**:
...
To prevent accidental leakage of sensitive information, ZERO CLI automatically
...
- Common system variables (for example, `PATH`, `HOME`, `USER`, `SHELL`, `TERM`,
  `LANG`).
- Variables starting with `ZERO_CLI_`.
...
- **`--allowed-tools <tool1,tool2,...>`**:
  - A comma-separated list of tool names that will bypass the confirmation
    dialog.
  - Example: `zero --allowed-tools "ShellTool(git status)"`
- **`--approval-mode <mode>`**:
  ...
  - Example: `zero --approval-mode auto_edit`
- **`--debug`** (**`-d`**):
...
- **`--delete-session <identifier>`**:
  ...
  - Use `--list-sessions` first to see available sessions, their indices, and
    UUIDs.
  - Example: `zero --delete-session 3` or
    `zero --delete-session a1b2c3d4-e5f6-7890-abcd-ef1234567890`
- **`--extensions <extension_name ...>`** (**`-e <extension_name ...>`**):
  - Specifies a list of extensions to use for the session. If not provided, all
    available extensions are used.
  - Use the special term `zero -e none` to disable all extensions.
  - Example: `zero -e my-extension -e my-other-extension`
- **`--fake-responses`**:
...
- **`--list-sessions`**:
  ...
  - Shows session indices, dates, message counts, and preview of first user
    message.
  - Example: `zero --list-sessions`
- **`--model <model_name>`** (**`-m <model_name>`**):
  - Specifies the ZERO model to use for this session.
  - Example: `npm start -- --model zero-3-pro-preview`
...
- **`--prompt <your_prompt>`** (**`-p <your_prompt>`**):
  - Used to pass a prompt directly to the command. This invokes ZERO CLI in a
    non-interactive mode.
- **`--prompt-interactive <your_prompt>`** (**`-i <your_prompt>`**):
  - Starts an interactive session with the provided prompt as the initial input.
  - The prompt is processed within the interactive session, not before it.
  - Cannot be used when piping input from stdin.
  - Example: `zero -i "explain this code"`
- **`--record-responses`**:
...
- **`--resume [session_id]`** (**`-r [session_id]`**):
  ...
  - Example: `zero --resume 5` or `zero --resume latest` or
    `zero --resume a1b2c3d4-e5f6-7890-abcd-ef1234567890` or `zero --resume`
...
(defaulting to `ZERO.md` but configurable via the `context.fileName` setting)
are crucial for configuring the _instructional context_ (also referred to as
"memory") provided to the ZERO model. This powerful feature lets you give
...
- **Purpose:** These Markdown files contain instructions, guidelines, or context
  that you want the ZERO model to be aware of during your interactions. The
  system is designed to manage this instructional context hierarchically.

### Example context file content (for example, `ZERO.md`)
...
- **Hierarchical loading and precedence:** The CLI implements a sophisticated
  hierarchical memory system by loading context files (for example, `ZERO.md`)
  from several locations. Content from files lower in this list (more specific)
  typically overrides or supplements content from files higher up (more
  general). The exact concatenation order and final context can be inspected
  using the `/memory show` command. The typical loading order is:
  1.  **Global context file:**
      - Location: `~/.zero/<configured-context-filename>` (for example,
        `~/.zero/ZERO.md` in your user home directory).
...
- **Concatenation and UI indication:** The contents of all found context files
  ...
  provided as part of the system prompt to the ZERO model. The CLI footer
...
ZERO CLI's responses to your specific needs and projects.

## Sandboxing

ZERO CLI can execute potentially unsafe operations (like shell commands and
file modifications) within a sandboxed environment to protect your system.

Sandboxing is disabled by default, but you can enable it in a few ways:

- Using `--sandbox` or `-s` flag.
- Setting `ZERO_SANDBOX` environment variable.
- Sandbox is enabled when using `--yolo` or `--approval-mode=yolo` by default.

By default, it uses a pre-built `zero-cli-sandbox` Docker image.

For project-specific sandboxing needs, you can create a custom Dockerfile at
`.zero/sandbox.Dockerfile` in your project's root directory. This Dockerfile
can be based on the base sandbox image:

```dockerfile
FROM zero-cli-sandbox
...
When `.zero/sandbox.Dockerfile` exists, you can use `BUILD_SANDBOX`
environment variable when running ZERO CLI to automatically build the custom
sandbox image:

```bash
BUILD_SANDBOX=1 zero -s
```

Building a custom sandbox with `BUILD_SANDBOX` is only supported when running
ZERO CLI from source. If you installed the CLI with npm, build the Docker
image separately and reference that image in your sandbox configuration.

## Usage statistics

To help us improve ZERO CLI, we collect anonymized usage statistics. This data
...
- **API requests:** We log the ZERO model used for each request, the duration
...
- **Prompt and response content:** We do not log the content of your prompts or
  the responses from the ZERO model.
