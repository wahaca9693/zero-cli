...
The `tracker_*` tools allow the ZERO agent to maintain an internal, persistent
...
interface. The task state is stored in the `.zero/tmp/tracker/<session-id>`
...
- **Persistence:** Task state is saved automatically to the
  `.zero/tmp/tracker/<session-id>` directory. Task states are session-specific
...
- **Interaction:** Users can view the current state of the tracker by asking the
  agent to visualize it, or by running `zero-cli` commands if implemented.
