...
An MCP server is an application that exposes tools and resources to the ZERO
CLI through the Model Context Protocol, allowing it to interact with external
systems and data sources. MCP servers act as a bridge between the ZERO model
and your local environment or other services like APIs.

An MCP server enables ZERO CLI to:
...
With an MCP server, you can extend ZERO CLI's capabilities to perform actions
beyond its built-in features, such as interacting with databases, APIs, custom
scripts, or specialized workflows.

## Core integration architecture

ZERO CLI integrates with MCP servers through a sophisticated discovery and
...
4. **Sanitizes and validates** tool schemas for compatibility with the ZERO
   API
...
ZERO CLI supports three MCP transport types:
...
prompts. ZERO CLI discovers these automatically and gives you the possibility
...
ZERO CLI uses the `mcpServers` configuration in your `settings.json` file to
...
ZERO CLI automatically expands environment variables in the `env` block of
...
To protect your credentials, ZERO CLI performs environment sanitization when
...
- Core project keys: `ZERO_API_KEY`, `GOOGLE_API_KEY`, etc.
...
ZERO CLI supports OAuth 2.0 authentication for remote MCP servers using SSE or
...
- **Stored securely** in `~/.zero/mcp-oauth-tokens.json`
...
When ZERO CLI starts, it performs MCP server discovery through the following
detailed process:
...
4. **Name sanitization:** Tool names are cleaned to meet ZERO API
   requirements:
...
Tool parameter schemas undergo sanitization for ZERO API compatibility:
...
When the ZERO model decides to use an MCP tool, the following execution flow
occurs:
...
Once discovered, MCP tools are available to the ZERO model like built-in
...
`settings.json`. ZERO CLI merges your local configuration with the extension's
defaults:
...
- **Access tokens:** Be security-aware when configuring environment variables
  ...
  for details on how ZERO CLI protects your credentials.
...
- **Property stripping:** The system automatically removes certain schema
  properties (`$schema`, `additionalProperties`) for ZERO API compatibility
...
ZERO CLI's capabilities while maintaining security, reliability, and ease of
use.
...
ZERO CLI will correctly process this array, separating text from binary data
...
When ZERO CLI receives this response, it will:
...
context to the ZERO model.

## MCP prompts as slash commands

In addition to tools, MCP servers can expose predefined prompts that can be
executed as slash commands within ZERO CLI. This lets you create shortcuts for
...
```bash
/poem-writer --title="ZERO CLI" --mood="reverent"
```

or, using positional arguments:

```bash
/poem-writer "ZERO CLI" reverent
```

When you run this command, ZERO CLI executes the `prompts/get` method on the
...
`settings.json` file, ZERO CLI provides a convenient set of commands to manage
...
`~/.zero/settings.json` or the project config `.zero/settings.json` file.
...
```bash
# Basic syntax
zero mcp add [options] <name> <command> [args...]

# Example: Adding a local server
zero mcp add -e API_KEY=123 -e DEBUG=true my-stdio-server /path/to/server arg1 arg2 arg3

# Example: Adding a local python server
zero mcp add python-server python server.py -- --server-arg my-value
```
...
zero mcp add --transport http --header "Authorization: Bearer abc123" secure-http https://api.example.com/mcp/
```
...
zero mcp add --transport sse --header "Authorization: Bearer abc123" secure-sse https://api.example.com/sse/
```

### Listing servers (`zero mcp list`)
...
> "Disconnected". Use `zero trust` to trust the current folder.
...
You can also use `zero mcp list` from your shell to see connection errors for
...
### Enabling/disabling a server (`zero mcp enable`, `zero mcp disable`)
...
```bash
zero mcp enable <name> [--session]
zero mcp disable <name> [--session]
```
...
`~/.zero/mcp-server-enablement.json`.
