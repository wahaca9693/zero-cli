...
The `run_shell_command` tool allows the ZERO model to execute commands
...
`settings.json` file or by using the `/settings` command in ZERO CLI.
...
When an interactive command is running, you can send input to it from the ZERO
...
When `run_shell_command` executes a command, it sets the `ZERO_CLI=1`
environment variable in the subprocess's environment. This allows scripts or
tools to detect if they are being run from within ZERO CLI.
