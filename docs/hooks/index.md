...
Hooks are scripts or programs that ZERO CLI executes at specific points in the
...
ZERO CLI waits for all matching hooks to complete before continuing.
...
Hooks are triggered by specific events in ZERO CLI's lifecycle.
...
3. **Debug via Stderr**: Use `stderr` for **all** logging and debugging (for
   example, `echo "debug" >&2`). ZERO CLI captures `stderr` but never attempts
   to parse it as JSON.

#### Exit codes

ZERO CLI uses exit codes to determine the high-level outcome of a hook
execution:
...
Hooks are configured in `settings.json`. ZERO CLI merges configurations from
multiple layers in the following order of precedence (highest to lowest):

1.  **Project settings**: `.zero/settings.json` in the current directory.
2.  **User settings**: `~/.zero/settings.json`.
3.  **System settings**: `/etc/zero-cli/settings.json`.
        ...
        "hooks": [
          {
            "name": "security-check",
            "type": "command",
            "command": "$ZERO_PROJECT_DIR/.zero/hooks/security.sh",
            "timeout": 5000
          }
        ]
      }
...
- `ZERO_PROJECT_DIR`: The absolute path to the project root.
- `ZERO_PLANS_DIR`: The absolute path to the plans directory.
- `ZERO_SESSION_ID`: The unique ID for the current session.
- `ZERO_CWD`: The current working directory.
...
ZERO CLI **fingerprints** project hooks. If a hook's name or command changes
