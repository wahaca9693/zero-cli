# Writing hooks for ZERO CLI

This guide will walk you through creating hooks for ZERO CLI, from a simple
...
- ZERO CLI installed and configured
...
```bash
mkdir -p .zero/hooks
cat > .zero/hooks/log-tools.sh << 'EOF'
#!/usr/bin/env bash
...
# Log to file
echo "[$(date)] Tool executed: $tool_name" >> .zero/tool-log.txt
...
chmod +x .zero/hooks/log-tools.sh
```

**Windows (PowerShell)**

```powershell
New-Item -ItemType Directory -Force -Path ".zero\hooks"
@"
# Read hook input from stdin
...
# Log to file
"[`$(Get-Date -Format 'o')] Tool executed: `$toolName" | Out-File -FilePath ".zero\tool-log.txt" -Append -Encoding utf8

# Return success with empty JSON
"{}"
"@ | Out-File -FilePath ".zero\hooks\log-tools.ps1" -Encoding utf8
```

## Exit Code Strategies

There are two ways to control or block an action in ZERO CLI:
        ...
        "hooks": [
          {
            "name": "intent-filter",
            "type": "command",
            "command": "node .zero/hooks/filter-tools.js"
          }
        ]
      }
    ]
...
### Configuration (`.zero/settings.json`)
        ...
        "hooks": [
          {
            "name": "init",
            "type": "command",
            "command": "node .zero/hooks/init.js"
          }
        ]
      }
    ],
        ...
        "hooks": [
          {
            "name": "memory",
            "type": "command",
            "command": "node .zero/hooks/inject-memories.js"
          }
        ]
      }
    ],
        ...
        "hooks": [
          {
            "name": "filter",
            "type": "command",
            "command": "node .zero/hooks/rag-filter.js"
          }
        ]
      }
    ],
        ...
        "hooks": [
          {
            "name": "security",
            "type": "command",
            "command": "node .zero/hooks/security.js"
          }
        ]
      }
    ],
        ...
        "hooks": [
          {
            "name": "record",
            "type": "command",
            "command": "node .zero/hooks/record.js"
          }
        ]
      }
    ],
        ...
        "hooks": [
          {
            "name": "validate",
            "type": "command",
            "command": "node .zero/hooks/validate.js"
          }
        ]
      }
    ],
        ...
        "hooks": [
          {
            "name": "save",
            "type": "command",
            "command": "node .zero/hooks/consolidate.js"
          }
        ]
      }
    ]
...
[ZERO CLI extension](../extensions/index.md). This provides version control,
