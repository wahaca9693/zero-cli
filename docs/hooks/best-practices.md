...
const CACHE_FILE = '.zero/hook-cache.json';
...
# Test the hook
cat test-input.json | .zero/hooks/my-hook.sh
...
# Test the hook
Get-Content test-input.json | .\.zero\hooks\my-hook.ps1
...
ZERO CLI uses exit codes for high-level flow control:
...
input=$(cat)
echo "$input" >> .zero/hook-inputs.log
        ...
        "hooks": [
          {
            "name": "secret-scanner",
            "type": "command",
            "command": "$ZERO_PROJECT_DIR/.zero/hooks/block-secrets.sh",
...
```bash
chmod +x .zero/hooks/*.sh
chmod +x .zero/hooks/*.js
...
.zero/hook-cache.json
.zero/hook-debug.log
.zero/memory/session-*.jsonl
...
| :---------------------------- | :-------------------------------------------------------------------------------------------------------------------------------- |
| **System**                    | Configured by system administrators (for example, `/etc/zero-cli/settings.json`, `/Library/...`). Assumed to be the **safest**. |
| **User** (`~/.zero/...`)    | Configured by you. You are responsible for ensuring they are safe.                                                                |
| **Extensions**                | You explicitly approve and install these. Security depends on the extension source (integrity).                                   |
| **Project** (`./.zero/...`) | **Untrusted by default.** Safest in trusted internal repos; higher risk in third-party/public repos.                              |

#### Project Hook Security

When you open a project with hooks defined in `.zero/settings.json`:

1. **Detection**: ZERO CLI detects the hooks.
...
> changed (for example, by a `git pull`), its identity changes. ZERO CLI will
...
| **Data Exfiltration**        | A hook could read your input (prompts), output (code), or environment variables (`ZERO_API_KEY`) and send them to a remote server. |
...
Hooks inherit the environment of ZERO CLI process, which may include sensitive
API keys. ZERO CLI provides a
...
```bash
ls -la .zero/hooks/my-hook.sh
chmod +x .zero/hooks/my-hook.sh
```
...
echo "$ZERO_PROJECT_DIR/.zero/hooks/my-hook.sh"

# Verify file exists
test -f "$ZERO_PROJECT_DIR/.zero/hooks/my-hook.sh" && echo "File exists"
```
...
```bash
#!/usr/bin/env bash
if [ -z "$ZERO_PROJECT_DIR" ]; then
  echo "ZERO_PROJECT_DIR not set" >&2
  exit 1
fi
...
Prevent denial-of-service (hanging agents) by enforcing timeouts. ZERO CLI
