...
Subagents are specialized agents that operate within your main ZERO CLI
...
Subagents are "specialists" that the main ZERO agent can hire for a specific
job.
...
ZERO CLI's main agent is instructed to use specialized subagents when a task
...
ZERO CLI comes with the following built-in subagents:
    ...
    "agents": {
      "overrides": {
        "codebase_investigator": {
          "modelConfig": { "model": "zero-3-flash-preview" },
...
- **Name:** `cli_help`
- **Purpose:** Get expert knowledge about ZERO CLI itself, its commands,
...
server is bundled with ZERO CLI and launched automatically — no separate
...
| Mode         | Description                                                                                                                                                                                 |
| :----------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `persistent` | **(Default)** Launches Chrome with a persistent profile stored at `~/.zero/cli-browser-profile/`. Cookies, history, and settings are preserved between sessions.                          |
| `isolated`   | Launches Chrome with a temporary profile that is deleted after each session. Use this for clean-state automation.                                                                           |
| `existing`   | Attaches to an already-running Chrome instance. You must enable remote debugging first by navigating to `chrome://inspect/#remote-debugging` in Chrome. No new browser process is launched. |

#### First-run consent

The first time the browser agent is invoked, ZERO CLI displays a consent
...
```json
{
  "agents": {
    ...
    "browser": {
      "visualModel": "zero-2.5-computer-use-preview-10-2025"
    }
  }
}
```
...
3. Launch the CLI with port forwarding:
   ...
   ```bash
   ZERO_SANDBOX=docker SANDBOX_PORTS=9222 zero
   ```
...
1.  **Project-level:** `.zero/agents/*.md` (Shared with your team)
2.  **User-level:** `~/.zero/agents/*.md` (Personal agents)
...
**Example: `.zero/agents/security-auditor.md`**
...
| `model`        | string | No       | Specific model to use (for example, `zero-3-preview`). Defaults to `inherit` (uses the main session model).                                                                                                 |
...
Subagent tool isolation moves ZERO CLI away from a single global tool
...
ZERO CLI can also delegate tasks to remote subagents using the Agent-to-Agent
(A2A) protocol.
