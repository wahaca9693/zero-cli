...
ZERO CLI supports connecting to remote subagents using the Agent-to-Agent
(A2A) protocol. This allows ZERO CLI to interact with other agents, expanding
its capabilities by delegating tasks to remote services.

ZERO CLI can connect to any compliant A2A agent. You can find samples of A2A
...
ZERO CLI routes traffic to remote agents through an HTTP/HTTPS proxy if one is
...
Many remote agents require authentication. ZERO CLI supports several
...
ZERO CLI supports the following authentication types:
...
use, ZERO CLI opens your browser for sign-in and persists the resulting tokens
...
When ZERO CLI loads a remote agent, it validates your auth configuration
...
When connecting to a remote agent, ZERO CLI first fetches the agent card
...
Users can manage subagents using the following commands within ZERO CLI:
...
<!-- prettier-ignore -->
> [!TIP]
> You can use the `@cli_help` agent within ZERO CLI for assistance
