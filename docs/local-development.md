...
features for ZERO CLI.

## Tracing

ZERO CLI uses OpenTelemetry (OTel) to record traces that help you debug agent
...
2.  **Run ZERO CLI:**
    ...
    In a separate terminal, run your ZERO CLI command:
...
1.  **Start the telemetry collector:**
    ...
    - **Collector logs:** `~/.zero/tmp/<projectHash>/otel/collector.log`
...
1.  **Configure `.zero/settings.json`:**
...
2.  **Start the telemetry collector:**
    ...
    - **Collector logs:** `~/.zero/tmp/<projectHash>/otel/collector-gcp.log`
...
```typescript
import { runInDevTraceSpan } from '@google/zero-cli-core';
import { ZEROCliOperation } from '@google/zero-cli-core/lib/telemetry/constants.js';

await runInDevTraceSpan(
  {
    operation: ZEROCliOperation.ToolCall,
    attributes: {
      [GEN_AI_AGENT_NAME]: 'zero-cli',
    },
  },
  async ({ metadata }) => {
