...
- **Error:
  `You must be a named user on your organization's ZERO Code Assist Standard edition subscription to use this service. Please contact your administrator to request an entitlement to ZERO Code Assist Standard edition.`**

  - **Cause:** This error might occur if ZERO CLI detects the
  ...
  - **Solution:**
    ...
    - **Organizational Users:** Contact your Google Cloud administrator to be
      added to your organization's ZERO Code Assist subscription.

- **Error:
  `Failed to sign in. Message: Your current account is not eligible... because it is not currently available in your location.`**

  - **Cause:** ZERO CLI does not currently support your location. For a full
    list of supported locations, see the following pages:
    - ZERO Code Assist for individuals:
      [Available locations](https://developers.google.com/zero-code-assist/resources/available-locations#americas)
...
- **Error: Command not found (when attempting to run ZERO CLI with
  `zero`).**

  - **Cause:** ZERO CLI is not correctly installed or it is not in your
    system's `PATH`.
  - **Solution:** The update depends on how you installed ZERO CLI:
    - If you installed `zero` globally, check that your `npm` global binary
      directory is in your `PATH`. You can update ZERO CLI using the command
      `npm install -g @google/zero-cli@latest`.
    - If you are running `zero` from source, ensure you are using the correct
      command to invoke it (for example, `node packages/cli/dist/index.js ...`).
      To update ZERO CLI, pull the latest changes from the repository, and
...
- **Error: "Operation not permitted", "Permission denied", or similar.**
  ...
  - **Cause:** When sandboxing is enabled, ZERO CLI may attempt operations
...
- **ZERO CLI is not running in interactive mode in "CI" environments**

  - **Issue:** ZERO CLI does not enter interactive mode (no prompt appears) if
  ...
  - **Cause:** The `is-in-ci` package checks for the presence of `CI`,
    ...
    non-interactive, which prevents ZERO CLI from starting in its interactive
    mode.
...
- **DEBUG mode not working from project .env file**
  ...
  - **Issue:** Setting `DEBUG=true` in a project's `.env` file doesn't enable
    debug mode for zero-cli.
  - **Cause:** The `DEBUG` and `DEBUG_MODE` variables are automatically excluded
    from project `.env` files to prevent interference with zero-cli behavior.
  - **Solution:** Use a `.zero/.env` file instead, or configure the
...
- **Warning: `npm WARN deprecated node-domexception@1.0.0` or
  ...
  - **Issue:** When installing or updating ZERO CLI globally via
    `npm install -g @google/zero-cli` or `npm update -g @google/zero-cli`,
  ...
  - **Cause:** These warnings occur because some dependencies (or their
    ...
    versions. Since ZERO CLI requires Node.js 20 or higher, the platform's
...
ZERO CLI uses specific exit codes to indicate the reason for termination. This
...
guide_, consider searching ZERO CLI
[Issue tracker on GitHub](https://github.com/google-zero/zero-cli/issues).
