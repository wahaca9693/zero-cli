...
"uninstall" the CLI, you must clear this cache, which will remove zero-cli and
...
`npm install -g @google/zero-cli`), use the `npm uninstall` command with the
`-g` flag to remove it.
...
`brew install zero-cli`), use the `brew uninstall` command to remove it.
...
`sudo port install zero-cli`), use the `port uninstall` command to remove it.

```bash
sudo port uninstall zero-cli
```
