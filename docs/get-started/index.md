# Get started with ZERO CLI

Welcome to ZERO CLI! This guide will help you install, configure, and start
using ZERO CLI to enhance your workflow right from your terminal.

## Quickstart: Install, authenticate, configure, and use ZERO CLI

ZERO CLI brings the power of advanced language models directly to your command
line interface. As an AI-powered assistant, ZERO CLI can help you with a
variety of tasks, from understanding and generating code to reviewing and
editing documents.

## Install

The standard method to install and run ZERO CLI uses `npm`:

```bash
npm install -g @google/zero-cli
```

Once ZERO CLI is installed, run ZERO CLI from your command line:
...
[ZERO CLI Installation](./installation.mdx).

## Authenticate

To begin using ZERO CLI, you must authenticate with a Google service. In most
cases, you can log in with your existing Google account:

1. Run ZERO CLI after installation:
...
[ZERO CLI Authentication Setup](./authentication.mdx).
...
[ZERO CLI Configuration](../reference/configuration.md).

## Use

Once installed and authenticated, you can start using ZERO CLI by issuing
...
You can use ZERO CLI to automate file management tasks that require visual
analysis. In this example, ZERO CLI renames images based on their actual
subject matter.
...
Give ZERO the following prompt:

```cli
Rename the photos in my "photos" directory based on their contents.
```

Result: ZERO asks for permission to rename your files.
...
ZERO CLI is effective for rapid codebase exploration. The following example
shows how to ask ZERO CLI to fetch, analyze, and summarize a remote project.

Scenario: You want to understand how a popular open-source utility works by
inspecting its code, not just its README.

Give ZERO CLI the following prompt:

```cli
Clone the 'chalk' repository from https://github.com/chalk/chalk, read its key source files, and explain how it works.
```

Result: ZERO performs a sequence of actions to answer your request.
...
ZERO CLI returns an explanation based on the actual source code:
...
ZERO CLI can process and transform data across multiple files. Use this
...
Result: ZERO CLI reads each file and then asks for permission to write a new
file. Provide your permission and ZERO CLI provides the combined data:
...
ZERO CLI can generate boilerplate code and tests based on your existing
...
Result: ZERO CLI asks for permission to write a new file and creates a test
for your login page.
