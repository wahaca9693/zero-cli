...
This monorepo contains two main packages: `@google/zero-cli` and
`@google/zero-cli-core`.

## `@google/zero-cli`

This is the main package for ZERO CLI. It is responsible for the user
...
`@google/zero-cli-core`. This means that whether a user installs the package
with `npm install -g @google/zero-cli` or runs it directly with
`npx @google/zero-cli`, they are using this single, self-contained executable.

## `@google/zero-cli-core`

This package contains the core logic for interacting with the ZERO API. It is
...
- **Simplified script execution**: You can run scripts in any package from the
  ...
  `npm run build --workspace @google/zero-cli`.
