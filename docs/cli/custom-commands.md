...
prompts as personal shortcuts within ZERO CLI. You can create commands that
...
ZERO CLI discovers commands from two locations, loaded in a specific order:

1.  **User commands (global):** Located in `~/.zero/commands/`. These commands
    are available in any project you are working on.
2.  **Project commands (local):** Located in
    `<your-project-root>/.zero/commands/`. These commands are specific to the
...
- A file at `~/.zero/commands/test.toml` becomes the command `/test`.
- A file at `<project>/.zero/commands/git/commit.toml` becomes the namespaced
  command `/git:commit`.
...
- `prompt` (String): The prompt that will be sent to the ZERO model when the
...
```toml
# In: <project>/.zero/commands/changelog.toml
...
When a custom command attempts to execute a shell command, ZERO CLI will now
...
````toml
# In: <project>/.zero/commands/git/commit.toml
...
- **Directory listing**: `@{path/to/dir}` is traversed and each file present
  ...
  respects `.gitignore` and `.zeroignore` if enabled.
...
```toml
# In: <project>/.zero/commands/review.toml
...
```bash
mkdir -p ~/.zero/commands/refactor
touch ~/.zero/commands/refactor/pure.toml
```

**Windows (PowerShell)**

```powershell
New-Item -ItemType Directory -Force -Path "$env:USERPROFILE\.zero\commands\refactor"
New-Item -ItemType File -Force -Path "$env:USERPROFILE\.zero\commands\refactor\pure.toml"
```

**2. Add the content to the file:**

Open `~/.zero/commands/refactor/pure.toml` in your editor and add the
following content. We are including the optional `description` for best
practice.

```toml
# In: ~/.zero/commands/refactor/pure.toml
...
ZERO CLI will then execute the multi-line prompt defined in your TOML file.
