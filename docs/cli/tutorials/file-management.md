# File management with ZERO CLI

Explore, analyze, and modify your codebase using ZERO CLI. In this guide,
you'll learn how to provide ZERO CLI with files and directories, modify and
create files, and control what ZERO CLI can see.
...
ZERO CLI will generally try to read relevant files, sometimes prompting you
for access (depending on your settings). To ensure that ZERO CLI uses a file,
...
If you _don't_ know the exact file path, you can ask ZERO CLI to find it for
...
ZERO uses the `glob` or `list_directory` tools to search your project
...
Once ZERO CLI has context, you can direct it to make specific edits. The agent
...
ZERO CLI uses the `replace` tool to propose a targeted code change.
...
ZERO CLI uses the `write_file` tool to generate the new file from scratch.

## Review and confirm changes

ZERO CLI prioritizes safety. Before any file is modified, it presents a
...
ZERO CLI uses the `run_shell_command` tool to execute your test runner (for
example, `npm test` or `jest`). This ensures the changes didn't break existing
functionality.

## Advanced: Controlling what ZERO sees

By default, ZERO CLI respects your `.gitignore` file. It won't read or search
...
`.zeroignore` file in your project root.
