...
Connect ZERO CLI to your external databases and services. In this guide,
you'll learn how to extend ZERO CLI's capabilities by installing the GitHub
...
## How to configure ZERO CLI

You tell ZERO about new servers by editing your `settings.json`.

1.  Open `~/.zero/settings.json` (or the project-specific
    `.zero/settings.json`).
2.  Add the `mcpServers` block. This tells ZERO: "Run this docker container
    and talk to it."
...
Restart ZERO CLI. It will automatically try to start the defined servers.
...
**Prompt:** `List the open PRs in the google/zero-cli repository.`
