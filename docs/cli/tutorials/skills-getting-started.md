...
Agent Skills extend ZERO CLI with specialized expertise. In this tutorial,
...
```bash
mkdir -p .zero/skills/api-auditor/scripts
```

**Windows (PowerShell)**

```powershell
New-Item -ItemType Directory -Force -Path ".zero\skills\api-auditor\scripts"
```

### 2. Create the definition (`SKILL.md`)

The `SKILL.md` file defines the skill's purpose and instructions for the agent.
Create a file at `.zero/skills/api-auditor/SKILL.md`. This tells the agent
...
a file at `.zero/skills/api-auditor/scripts/audit.js`. This is the code the
agent will run.

```javascript
// .zero/skills/api-auditor/scripts/audit.js
...
ZERO CLI automatically discovers skills in the `.zero/skills` directory (as
well as the `.agents/skills` alias).

To check if ZERO CLI found your new skill, use the `/skills list` command
...
1.  **The folder must be trusted (workspace skills only).** Skills under
    `<workspace>/.zero/skills/` are only loaded when the workspace folder is
    marked as trusted. Run `/trust` and restart the session if needed. Skills
    under `~/.zero/skills/` (user scope) are not affected by trust.
2.  **Check the path layout.** `SKILL.md` is discovered either at the root of
    the skills directory (`.zero/skills/SKILL.md`) or one directory deep
    (`.zero/skills/<skill-name>/SKILL.md`). The recommended layout uses a
...
You can also manage skills using the `zero skills` command from your terminal:

- **Install**: `zero skills install <url-or-path>`
- **Link**: `zero skills link <path>` (useful for local development)
- **Uninstall**: `zero skills uninstall <name>`
