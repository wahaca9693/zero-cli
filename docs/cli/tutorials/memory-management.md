...
Control what ZERO CLI knows about you and your projects. In this guide, you'll
learn how to define project-wide rules with `ZERO.md`, teach the agent
...
ZERO CLI is powerful but general. It doesn't know your preferred testing
...
## How to define project-wide rules (ZERO.md)

The most powerful way to control the agent's behavior is through `ZERO.md`
...
1.  In the root of your project, create a file named `ZERO.md`.
...
3.  Start a new session. ZERO CLI will now know these rules automatically.
...
1.  **Global:** `~/.zero/ZERO.md` (Rules for _every_ project you work on).
2.  **Project Root:** `./ZERO.md` (Rules for the current repository).
3.  **Subdirectory:** `./src/ZERO.md` (Rules specific to the `src` folder).
...
`ZERO.md` files and saved memories), use the `/memory show` command.
...
If you edit a `ZERO.md` file while a session is running, the agent won't know
...
- **Keep it focused:** Avoid adding excessive content to `ZERO.md`. Keep
...
- **Review often:** Periodically check your `ZERO.md` files to remove outdated
  rules.
...
- Explore the [Command reference](../../reference/commands.md) for more
  `/memory` options.
- Read the technical spec for [Project context](../../cli/zero-md.md).
