...
Automate tasks with ZERO CLI. Learn how to use headless mode, pipe data into
ZERO CLI, automate workflows with shell scripts, and generate structured JSON
output for other applications.

## Prerequisites

- ZERO CLI installed and authenticated.
- Familiarity with shell scripting (Bash/Zsh).

## Why headless mode?

Headless mode runs ZERO CLI once and exits. It's perfect for:
...
Run ZERO CLI in headless mode by providing a prompt with the `-p` (or
...
```bash
zero -p "Write a poem about TypeScript"
```

## How to pipe input to ZERO CLI

Feed data into ZERO using the standard Unix pipe `|`. ZERO reads the
...
```bash
cat error.log | zero -p "Explain why this failed"
```

**Windows (PowerShell)**

```powershell
Get-Content error.log | zero -p "Explain why this failed"
```

Pipe a command:

```bash
git diff | zero -p "Write a commit message for these changes"
```

## Use ZERO CLI output in scripts

Because ZERO prints to stdout, you can chain it with other tools or save the
results to a file.
...
1.  Save the following code as `generate_docs.sh` (or `generate_docs.ps1` for
    ...
    **macOS/Linux (`generate_docs.sh`)**
    ...
    for file in *.py; do
      ...
      # Ask ZERO CLI to generate the documentation and print it to stdout
      zero -p "Generate a Markdown documentation summary for @$file. Print the
    ...
    **Windows PowerShell (`generate_docs.ps1`)**
    ...
    Get-ChildItem -Filter *.py | ForEach-Object {
      ...
      # Ask ZERO CLI to generate the documentation and print it to stdout
      zero -p "Generate a Markdown documentation summary for @$($_.Name). Print the result to standard output." | Out-File -FilePath $newName -Encoding utf8
    }
    ```
...
1.  Save the following script as `generate_json.sh` (or `generate_json.ps1` for
    ...
    **macOS/Linux (`generate_json.sh`)**
    ...
    # Extract data
    zero --output-format json "Return a raw JSON object with keys 'version' and 'deps' from @package.json" | jq -r '.response' > data.json
    ```
    ...
    **Windows PowerShell (`generate_json.ps1`)**
    ...
    $output = zero --output-format json "Return a raw JSON object with keys 'version' and 'deps' from @package.json" | ConvertFrom-Json
...
2.  Scroll to the very bottom of the file and paste this code:
    ...
    function gcommit() {
      ...
      # Ask ZERO to write the message
      echo "Generating commit message..."
      msg=$(echo "$diff" | zero -p "Write a concise Conventional Commit message for this diff. Output ONLY the message.")
...
2.  Scroll to the very bottom of the file and paste this code:
    ...
    function gcommit {
      ...
      $msg = $diff | zero -p "Write a concise Conventional Commit message for this diff. Output ONLY the message."
...
4.  Use your new command:
    ...
    ZERO CLI will analyze your staged changes and commit them with a generated
    message.
