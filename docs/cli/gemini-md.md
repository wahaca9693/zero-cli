# Provide context with ZERO.md files

Context files, which use the default name `ZERO.md`, are a powerful feature
for providing instructional context to the ZERO model. You can use these files
...
1.  **Global context file:**
    ...
    - **Location:** `~/.zero/ZERO.md` (in your user home directory).
    - **Scope:** Provides default instructions for all your projects.

2.  **Environment and workspace context files:**

    - **Location:** The CLI searches for `ZERO.md` files in your configured
...
3.  **Just-in-time (JIT) context files:**
    - **Location:** When a tool accesses a file or directory, the CLI
      automatically scans for `ZERO.md` files in that directory and its
...
### Example `ZERO.md` file

Here is an example of what you can include in a `ZERO.md` file at the root of
...
- **`/memory reload`**: Forces a re-scan and reload of all `ZERO.md` files
  from all configured locations.

## Modularize context with imports

You can break down large `ZERO.md` files into smaller, more manageable
...
While `ZERO.md` is the default filename, you can configure this in your
...
{
  "context": {
    "fileName": ["AGENTS.md", "CONTEXT.md", "ZERO.md"]
  }
}
```
...
- Learn about [Ignoring files](./zero-ignore.md) to exclude content from the
  context system.
