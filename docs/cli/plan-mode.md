...
You can either configure ZERO CLI to start in Plan Mode by default or enter
Plan Mode manually during a session.

### Launch in Plan Mode

To start ZERO CLI directly in Plan Mode by default:

1.  Use the `/settings` command.
2.  Set **Default Approval Mode** to `Plan`.

To launch ZERO CLI in Plan Mode once:

1. Use `zero --approval-mode=plan` when launching ZERO CLI.

### Enter Plan Mode manually

To start Plan Mode while using ZERO CLI:

- **Keyboard shortcut:** Press `Shift+Tab` to cycle through approval modes
  (`Default` -> `Auto-Edit` -> `Plan`). Plan Mode is automatically removed from
  the rotation when ZERO CLI is actively processing or showing confirmation
  dialogs.
...
- **Natural Language:** Ask ZERO CLI to "start a plan for...". ZERO CLI
  calls the
  [`enter_plan_mode`](../tools/planning.md#1-enter_plan_mode-enterplanmode) tool
  to switch modes. This tool is not available when ZERO CLI is in
  [YOLO mode](../reference/configuration.md#command-line-arguments).

## How to use Plan Mode

Plan Mode lets you collaborate with ZERO CLI to design a solution before
ZERO CLI takes action.

1.  **Provide a goal:** Start by describing what you want to achieve. ZERO CLI
    will then enter Plan Mode (if it's not already) to research the task.
2.  **Discuss and agree on strategy:** As ZERO CLI analyzes your codebase, it
    ...
    options using [`ask_user`](../tools/ask-user.md). **ZERO CLI will stop and
...
3.  **Review the plan:** Once you've agreed on the strategy, ZERO CLI creates
...
4.  **Approve or iterate:** ZERO CLI will present the finalized plan for your
    formal approval.
    ...
    - **Iterate:** If the plan needs adjustments, provide feedback in the input
      box or [edit the plan file directly](#collaborative-plan-editing). ZERO
...
You can collaborate with ZERO CLI by making direct changes or leaving comments
in the implementation plan. This is often faster and more precise than
describing complex changes in natural language.

1.  **Open the plan:** Press `Ctrl+X` when ZERO CLI presents a plan for
    review.
...
4.  **Review and refine:** ZERO CLI automatically detects the changes, reviews
...
- **Approve a plan:** When ZERO CLI presents a finalized plan, approving it
...
- **Natural language:** Ask ZERO CLI to "exit plan mode" or "stop planning."
...
- **Planning (Write):**
  ...
  files in the `~/.zero/tmp/<project>/<session-id>/plans/` directory or your
...
workflows. You can customize how ZERO CLI plans by using skills, adjusting
safety policies, changing where plans are stored, or adding hooks.

### Custom planning with skills

You can use [Agent Skills](../cli/skills.md) to customize how ZERO CLI
...
- A **"Security Audit"** skill could prompt ZERO CLI to look for specific
  vulnerabilities during codebase exploration.
- A **"Frontend Design"** skill could guide ZERO CLI to use specific UI
  components and accessibility standards in its proposal.

To use a skill in Plan Mode, you can explicitly ask ZERO CLI to "use the
`<skill-name>` skill to plan..." or ZERO CLI may autonomously activate it
...
`~/.zero/policies/` directory (Tier 2).
...
`~/.zero/policies/mcp-read-only.toml`
...
`~/.zero/policies/git-research.toml`
...
`~/.zero/policies/research-subagents.toml`
...
Tell ZERO CLI it can use these tools in your prompt, for example: _"You can
...
outside your project: `~/.zero/tmp/<project>/<session-id>/plans/`.

You can configure a custom directory for plans in your `settings.json`. For
example, to store plans in a `.zero/plans` directory within your project:

```json
{
  "general": {
    "plan": {
      "directory": ".zero/plans"
    }
  }
}
```
...
writing to the `.zero/plans` directory within your project, create a policy
file at `~/.zero/policies/plan-custom-directory.toml`:
...
# This example matches any .md file in a .zero/plans directory within the project.
argsPattern = "\"file_path\":\"[^\"]+[\\\\/]+\\.zero[\\\\/]+plans[\\\\/]+[\\w-]+\\.md\""
```

### Using hooks with Plan Mode

You can use the [hook system](../hooks/writing-hooks.md) to automate parts of
the planning workflow or enforce additional checks when ZERO CLI transitions
...
Storage whenever ZERO CLI exits Plan Mode to start the implementation.

**`.zero/hooks/archive-plan.sh`:**
...
# Construct the absolute path using the ZERO_PLANS_DIR environment variable
plan_path="$ZERO_PLANS_DIR/$plan_filename"
...
if [ -f "$plan_path" ]; then
  ...
  gsutil cp "$plan_path" "gs://my-audit-bucket/zero-plans/$filename" > /dev/null 2>&1 &
fi
        ...
        "hooks": [
          {
            "name": "archive-plan",
            "type": "command",
            "command": "~/.zero/hooks/archive-plan.sh"
          }
        ]
      }
    ]
...
how ZERO CLI researches and stores plans for your specific domain.
...
When using an [auto model](../reference/configuration.md#model), ZERO CLI
...
ZERO CLI automatically and silently falls back to a faster model to ensure
...
By default, ZERO CLI automatically cleans up old session data, including all
...
- **Command Line:** Use `zero --delete-session <index|id>`.
...
When running ZERO CLI in non-interactive environments (such as headless
...
- **Automated implementation:** When exiting Plan Mode to execute the plan,
  ZERO CLI automatically switches to
...
```bash
zero --approval-mode plan -p "Analyze telemetry and suggest improvements"
```

[`plan.toml`]:
  https://github.com/google-zero/zero-cli/blob/main/packages/core/src/policy/policies/plan.toml
[Conductor]: https://github.com/zero-cli-extensions/conductor
[open an issue]: https://github.com/google-zero/zero-cli/issues
