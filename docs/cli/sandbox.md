# Sandboxing in ZERO CLI

This document provides a guide to sandboxing in ZERO CLI, including
prerequisites, quickstart, and configuration.

## Prerequisites

Before using sandboxing, you need to install and set up ZERO CLI:
...
2. **Environment variable**:
   `ZERO_SANDBOX=true|docker|podman|sandbox-exec|runsc|lxc`
...
technology. By default, it uses the `ghcr.io/google/zero-cli:latest` image.
...
To enable Docker sandboxing, run ZERO CLI with the sandbox flag and specify
Docker as the provider:

```bash
# Using the environment variable (Recommended)
export ZERO_SANDBOX=docker
zero -p "build the project"
...
name or have ZERO CLI build one for you automatically. You can use any Docker
...
set the `ZERO_SANDBOX_IMAGE` environment variable.
...
```bash
export ZERO_SANDBOX_IMAGE="us-central1-docker.pkg.dev/my-project/my-repo/my-custom-sandbox:latest"
```
...
and ZERO CLI will build the image automatically.

1.  Create a `.zero/sandbox.Dockerfile` in your project root.
2.  Ensure you have the `gh` CLI installed and authenticated (if you are using
    the default `ghcr.io/google/zero-cli` image as a base).
3.  Run your command with the `BUILD_SANDBOX` environment variable set:

```bash
BUILD_SANDBOX=1 ZERO_SANDBOX=docker zero -p "run my custom build"
```
...
When you set `sandbox: "runsc"`, ZERO CLI runs
...
`ZERO_SANDBOX=runsc` or `sandbox: "runsc"`).
...
- A container must be created and running before starting ZERO CLI. ZERO
...
lxc launch ubuntu:24.04 zero-sandbox

# Enable LXC sandboxing
export ZERO_SANDBOX=lxc
zero -p "build the project"
```
...
```bash
export ZERO_SANDBOX=lxc
export ZERO_SANDBOX_IMAGE=my-snapcraft-container
zero -p "build the snap"
```
...
(like `shell_exec` and `write_file`) instead of sandboxing the entire ZERO CLI
process.
...
<!-- prettier-ignore -->
> [!NOTE]
> Changing the `security.toolSandboxing` setting requires a restart of ZERO
> CLI to take effect.

## Sandbox expansion

Sandbox expansion is a dynamic permission system that lets ZERO CLI request
...
requiring extra permissions (like `npm install`), ZERO CLI will present you
with a "Sandbox Expansion Request."

### How sandbox expansion works

1.  **Detection**: ZERO CLI detects a sandbox denial or proactively identifies
...
If you are running ZERO CLI itself from within an official or custom Docker
...
docker run -it \
  ...
  -e ZERO_SANDBOX=docker \
  ghcr.io/google/zero-cli:latest
```
...
- Add to a custom Dockerfile. Automatic `BUILD_SANDBOX` builds are only
  available when running ZERO CLI from source; npm installs need a prebuilt
  image instead.
...
> zero-cli due to automatic exclusion. Use `.zero/.env` files for
> zero-cli specific debug settings.
