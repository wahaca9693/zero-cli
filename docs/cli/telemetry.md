...
ZERO CLI provides built-in support for OpenTelemetry, transforming every agent
...
enterprise-wide usage in the cloud, ZERO CLI's observability system provides
the actionable intelligence needed to move from "black box" AI to predictable,
high-performance systems.

## OpenTelemetry integration

ZERO CLI integrates with **[OpenTelemetry]**, a vendor-neutral,
...
You control telemetry behavior through the `.zero/settings.json` file.
Environment variables can override these settings.

| Setting        | Environment Variable              | Description                                         | Values            | Default                 |
| -------------- | --------------------------------- | --------------------------------------------------- | ----------------- | ----------------------- |
| `enabled`      | `ZERO_TELEMETRY_ENABLED`        | Enable or disable telemetry                         | `true`/`false`    | `false`                 |
| `traces`       | `ZERO_TELEMETRY_TRACES_ENABLED` | Enable detailed attribute tracing                   | `true`/`false`    | `false`                 |
| `target`       | `ZERO_TELEMETRY_TARGET`         | Where to send telemetry data                        | `"gcp"`/`"local"` | `"local"`               |
| `otlpEndpoint` | `ZERO_TELEMETRY_OTLP_ENDPOINT`  | OTLP collector endpoint                             | URL string        | `http://localhost:4317` |
| `otlpProtocol` | `ZERO_TELEMETRY_OTLP_PROTOCOL`  | OTLP transport protocol                             | `"grpc"`/`"http"` | `"grpc"`                |
| `outfile`      | `ZERO_TELEMETRY_OUTFILE`        | Save telemetry to file (overrides `otlpEndpoint`)   | file path         | -                       |
| `logPrompts`   | `ZERO_TELEMETRY_LOG_PROMPTS`    | Include prompts in telemetry logs                   | `true`/`false`    | `true`                  |
| `useCollector` | `ZERO_TELEMETRY_USE_COLLECTOR`  | Use external OTLP collector (advanced)              | `true`/`false`    | `false`                 |
| `useCliAuth`   | `ZERO_TELEMETRY_USE_CLI_AUTH`   | Use CLI credentials for telemetry (GCP target only) | `true`/`false`    | `false`                 |
| -              | `ZERO_CLI_SURFACE`              | Optional custom label for traffic reporting         | string            | -                       |
...
2.  Authenticate with Google Cloud using one of these methods:
    ...
    * **Method B: CLI Auth** (Direct export only): Simplest method for local
      users. ZERO CLI uses the same OAuth credentials you used for login. To
      enable this, set `useCliAuth: true` in your `.zero/settings.json`:
...
1.  Enable telemetry in `.zero/settings.json`:
    ```json
    {
      "telemetry": {
        "enabled": true,
    ...
    ```
2.  Run ZERO CLI and send prompts.
...
After you enable telemetry and run ZERO CLI, you can view your data in the
...
ZERO CLI provides a pre-configured
...
![ZERO CLI Monitoring Dashboard Overview](/docs/assets/monitoring-dashboard-overview.png)

![ZERO CLI Monitoring Dashboard Metrics](/docs/assets/monitoring-dashboard-metrics.png)

![ZERO CLI Monitoring Dashboard Logs](/docs/assets/monitoring-dashboard-logs.png)

To learn more, see
[Instant insights: ZERO CLI’s pre-configured monitoring dashboards](https://cloud.google.com/blog/topics/developers-practitioners/instant-insights-zero-clis-new-pre-configured-monitoring-dashboards/).
...
1.  Enable telemetry in `.zero/settings.json`:
    ```json
    {
      "telemetry": {
        "enabled": true,
        "target": "local",
        "outfile": ".zero/telemetry.log"
      }
    }
    ```
2.  Run ZERO CLI and send prompts.
3.  View logs and metrics in `.zero/telemetry.log`.
...
ZERO CLI includes identifiers in its `User-Agent` header to help you
differentiate and report on API traffic from different environments (for
example, identifying calls from ZERO Code Assist versus a standard terminal).
...
| :---------------------------------- | :--------------------------- | :---------- |
| **ZERO Code Assist (Agent Mode)** | `ZEROCLI-a2a-server`       | `vscode`    |
| **Zed (via ACP)**                   | `ZEROCLI-acp-zed`          | `zed`       |
| **XCode (via ACP)**                 | `ZEROCLI-acp-xcode`        | `xcode`     |
| **IntelliJ IDEA (via ACP)**         | `ZEROCLI-acp-intellijidea` | `jetbrains` |
| **Standard Terminal**               | `ZEROCLI`                  | `terminal`  |

**Example User-Agent:**
`ZEROCLI-a2a-server/0.34.0/zero-pro (linux; x64; vscode)`

### Custom identification

You can provide a custom identifier for your own scripts or automation by
setting the `ZERO_CLI_SURFACE` environment variable. This is useful for
tracking specific internal tools or distribution channels in your GCP logs.

**macOS/Linux**

```bash
export ZERO_CLI_SURFACE="my-custom-tool"
```
...
`ZEROCLI/0.34.0/zero-pro (linux; x64; my-custom-tool)`
...
ZERO CLI includes `session.id`, `installation.id`, `active_approval_mode`, and
`user.email` (when authenticated) as common attributes on all data.

### Logs

Logs provide timestamped records of specific events. ZERO CLI logs events
...
##### `zero_cli.tool_output_truncated`
...
API logs capture requests, responses, and errors from ZERO API.

##### `zero_cli.api_request`

Request sent to ZERO API.
...
##### `zero_cli.api_response`

Response received from ZERO API.
...
##### `zero_cli.malformed_json_response`
...
These logs track how ZERO CLI selects and routes requests to models.

##### `zero_cli.slash_command`
...
##### `zero_cli.chat.content_retry_failure`
...
##### `zero_cli.conversation_finished`
...
##### `zero_cli.web_fetch_fallback_attempt`
...
##### `zero_cli.agent.recovery_attempt`
...
##### `zero_cli.keychain.availability`
...
ZERO CLI exports several custom metrics.

##### Sessions

##### `zero_cli.session.count`
...
- `zero_cli.onboarding.start` (Counter, Int): Incremented when the
  authentication flow begins.

- `zero_cli.onboarding.success` (Counter, Int): Incremented when the user
...
##### `zero_cli.chat.invalid_chunk.count`

Counts invalid stream chunks.

##### `zero_cli.chat.content_retry.count`

Counts content error retries.

##### `zero_cli.chat.content_retry_failure.count`

Counts requests where all retries failed.

##### Model routing

##### `zero_cli.slash_command.model.call_count`
...
##### `zero_cli.model_routing.latency`
...
##### `zero_cli.model_routing.failure.count`
...
##### `zero_cli.ui.flicker.count`

Counts terminal flicker events.

##### Performance

ZERO CLI provides detailed performance metrics for advanced monitoring.

##### `zero_cli.startup.duration`
...
##### `zero_cli.tool.queue.depth`

Measures tool execution queue depth.

##### `zero_cli.tool.execution.breakdown`
...
> `ZERO_TELEMETRY_TRACES_ENABLED=true`) to capture them.
...
- `gen_ai.operation.name`: High-level operation (for example, `tool_call`,
  ...
  `schedule_tool_calls`).
- `gen_ai.agent.name`: Set to `zero-cli`.
