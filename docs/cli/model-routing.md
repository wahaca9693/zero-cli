...
ZERO CLI includes a model routing feature that automatically switches to a
...
2.  **User consent:** Depending on the failure and the model's policy, the CLI
    ...
    use a silent fallback chain for `zero-2.5-flash-lite` and will fall back
    to `zero-2.5-flash` and `zero-2.5-pro` without prompting or changing the
    configured model.
...
ZERO CLI supports using a local model for routing decisions. When configured,
ZERO CLI will use a locally-running **Gemma** model to make routing decisions
...
The easiest way to set this up is using the automated `zero gemma setup`
command.

For more details on how to configure local model routing, see
[`zero gemma` — Local Model Routing Setup](../core/gemma-setup.md).

### Model selection precedence

The model used by ZERO CLI is determined by the following order of precedence:

1.  **`--model` command-line flag:** A model specified with the `--model` flag
    when launching the CLI will always be used.
2.  **`ZERO_MODEL` environment variable:** If the `--model` flag is not used,
    the CLI will use the model specified in the `ZERO_MODEL` environment
    variable.
...
4.  **Local model (experimental):** If the Gemma local model router is enabled
    ...
    (instead of ZERO models) to route the request to an appropriate model.
