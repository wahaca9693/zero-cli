...
ZERO session its own copy of the codebase. Git worktrees create separate
...
>
> - [Open an issue](https://github.com/google-zero/zero-cli/issues/new?template=bug_report.yml) on GitHub.
> - Use the **/bug** command within ZERO CLI to file an issue.
...
Use the `--worktree` (`-w`) flag to create an isolated worktree and start ZERO
CLI in it.

- **Start with a specific name:** The value you pass becomes both the directory
  name (within `.zero/worktrees/`) and the branch name.

  ```bash
  zero --worktree feature-search
  ```

- **Start with a random name:** If you omit the name, ZERO generates a random
...
When you exit a worktree session (using `/quit` or `Ctrl+C`), ZERO leaves the
...
ZERO prioritizes a fast and safe exit: it **does not automatically delete**
your worktree or branch. You are responsible for cleaning up your worktrees
manually once you are finished with them.

When you exit, ZERO displays instructions on how to resume your work or how to
...
ZERO CLI with the `--resume` flag and the session ID:

```bash
cd .zero/worktrees/feature-search
zero --resume <session_id>
```
...
- **Clean up a preserved Git worktree:**
  ```bash
  git worktree remove .zero/worktrees/feature-search --force
...
- **Create a Git worktree manually:**
  ...
  cd ../project-feature-search && zero
  ```
