# ZERO CLI for the enterprise

This document outlines configuration patterns and best practices for deploying
and managing ZERO CLI in an enterprise environment. By leveraging system-level
...
> ZERO CLI. However, they should not be considered a foolproof security
...
2. User Settings (`~/.zero/settings.json`)
3. Workspace Settings (`<project>/.zero/settings.json`)
...
- **System defaults `system-defaults.json`:**
  ...
  ```json
  {
    ...
    "context": {
      "includeDirectories": ["/etc/zero-cli/common-context"]
    }
  }
  ```

- **User `settings.json` (`~/.zero/settings.json`):**
  ...
  ```json
  {
    ...
    "context": {
      "includeDirectories": ["~/zero-context"]
    }
  }
  ```

- **Workspace `settings.json` (`<project>/.zero/settings.json`):**
...
- **System overrides `settings.json`:**
  ```json
  {
    ...
    "context": {
      "includeDirectories": ["/etc/zero-cli/global-context"]
    }
  }
  ```
...
- **Final merged configuration:**
  ```json
  {
    ...
    "context": {
      "includeDirectories": [
        "/etc/zero-cli/common-context",
        "~/zero-context",
        "./project-context",
        "/etc/zero-cli/global-context"
      ]
    }
  }
  ```
...
- **Location**:
  - **Linux**: `/etc/zero-cli/settings.json`
  - **Windows**: `C:\ProgramData\zero-cli\settings.json`
  - **macOS**: `/Library/Application Support/ZEROCli/settings.json`
  - The path can be overridden using the `ZERO_CLI_SYSTEM_SETTINGS_PATH`
...
While the `ZERO_CLI_SYSTEM_SETTINGS_PATH` environment variable provides
...
This approach ensures that no matter how the user calls the `zero` command,
the enterprise settings are always loaded with the highest precedence.

**Example wrapper script:**

Administrators can create a script named `zero` and place it in a directory
that appears earlier in the user's `PATH` than the actual ZERO CLI binary (for
example, `/usr/local/bin/zero`).
...
export ZERO_CLI_SYSTEM_SETTINGS_PATH="/etc/zero-cli/settings.json"

# Find the original zero executable.
# This is a simple example; a more robust solution might be needed
# depending on the installation method.
REAL_ZERO_PATH=$(type -aP zero | grep -v "^$(type -P zero)$" | head -n 1)

if [ -z "$REAL_ZERO_PATH" ]; then
  echo "Error: The original 'zero' executable was not found." >&2
  exit 1
fi

# Pass all arguments to the real ZERO CLI executable.
exec "$REAL_ZERO_PATH" "$@"
```

By deploying this script, the `ZERO_CLI_SYSTEM_SETTINGS_PATH` is set within
the script's environment, and the `exec` command replaces the script process
with the actual ZERO CLI process, which inherits the environment variable.
...
```powershell
Add-Content -Path $PROFILE -Value '$env:ZERO_CLI_SYSTEM_SETTINGS_PATH="C:\ProgramData\zero-cli\settings.json"'
```

## User isolation in shared environments

In shared compute environments (like ML experiment runners or shared build
servers), you can isolate ZERO CLI state by overriding the user's home
directory.

By default, ZERO CLI stores configuration and history in `~/.zero`. You can
use the `ZERO_CLI_HOME` environment variable to point to a unique directory
for a specific user or job. The CLI will create a `.zero` folder inside the
specified path.
...
export ZERO_CLI_HOME="/tmp/zero-job-123"
zero
```
...
```powershell
# Isolate state for a specific job
$env:ZERO_CLI_HOME="C:\temp\zero-job-123"
zero
```

## Restricting tool access

You can significantly enhance security by controlling which tools the ZERO
...
ZERO CLI loads `settings.json` files from three levels: System, Workspace, and
...
In corporate environments with strict network policies, you can configure ZERO
...
For auditing and monitoring purposes, you can configure ZERO CLI to send
...
control that is configured on a proxy server, not within ZERO CLI itself. It
...
patterns discussed above to create a secure, controlled environment for ZERO
CLI.
...
```json
{
  ...
  "mcpServers": {
    "corp-tools": {
      "command": "/opt/zero-tools/start.sh",
      "timeout": 5000
    }
  },
  "telemetry": {
