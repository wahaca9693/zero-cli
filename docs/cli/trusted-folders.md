...
which projects can use the full capabilities of ZERO CLI. It prevents
...
Once the feature is enabled, the first time you run ZERO CLI from a folder, a
...
Your choice is saved in a central file (`~/.zero/trustedFolders.json`), so you
will only be asked once per folder.

## Understanding folder contents: The discovery phase

Before you make a choice, ZERO CLI performs a **discovery phase** to scan the
...
When a folder is **untrusted**, ZERO CLI runs in a restricted "safe mode" to
protect you. In this mode, the following features are disabled:

1.  **Workspace settings are ignored**: The CLI will **not** load the
    `.zero/settings.json` file from the project. This prevents the loading of
...
Granting trust to a folder unlocks the full functionality of ZERO CLI for that
workspace.

## Headless and automated environments

When running ZERO CLI in a headless environment (for example, a CI/CD
...
- **Environment variable:** Set the `ZERO_CLI_TRUST_WORKSPACE=true`
...
review the
[ZERO CLI trust guidance for GitHub Actions](https://github.com/google-github-actions/run-zero-cli/blob/main/docs/trust-guidance.md).

## Overriding the trust file location

By default, trust settings are saved to `~/.zero/trustedFolders.json`. If you
need to store this file in a different location, you can set the
`ZERO_CLI_TRUSTED_FOLDERS_PATH` environment variable to the desired absolute
file path.
...
- **View all trust rules**: To see a complete list of all your trusted and
  ...
  `~/.zero/trustedFolders.json` file in your home directory.
...
2.  **Local trust file**: If the IDE is not connected, the CLI checks the
    central `~/.zero/trustedFolders.json` file.
