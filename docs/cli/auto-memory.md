...
Auto Memory is an experimental feature that mines your past ZERO CLI sessions
...
Every session you run with ZERO CLI is recorded locally as a transcript. Auto
...
1.  Open your global settings file at `~/.zero/settings.json`. If you only
    want Auto Memory in one project, edit `.zero/settings.json` in that
    project instead.
...
3.  Restart ZERO CLI. The flag requires a restart because the extraction
...
1.  **Eligibility scan.** The service indexes recent sessions from
    `~/.zero/tmp/<project>/chats/`. Sessions are eligible only if they have
...
6.  **Notification.** When a run produces new candidates, ZERO CLI surfaces an
...
- **Promote** a skill to your user (`~/.zero/skills/`) or workspace
  (`.zero/skills/`) directory.
...
- The extraction agent runs on a preview ZERO Flash model. Extraction quality
...
- Project or workspace shared instructions in project `ZERO.md` files are not
...
- Explore the [memory management tutorial](./tutorials/memory-management.md) for
  the complementary explicit-memory and `ZERO.md` workflows.
