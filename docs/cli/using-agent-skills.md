...
Agent Skills provide ZERO CLI with specialized expertise on demand. This guide
...
1.  **Built-in Skills**: Included with ZERO CLI and always available.
...
The `zero skills` command provides management utilities directly from your
system shell.
...
```bash
zero skills install https://github.com/user/my-awesome-skill
```
...
```bash
zero skills link ./path/to/my-skill
```
