...
ZERO CLI. It serves as a checklist and quality gate for release manager to
...
- **Command:** `npx -y @google/zero-cli@<tag> --version` must return the
...
- **Setup:**
  ...
  - [ ] Uninstall any existing global version:
        `npm uninstall -g @google/zero-cli`
  - [ ] Clear npx cache (optional but recommended): `npm cache clean --force`
  - [ ] Install the preview version: `npm install -g @google/zero-cli@preview`
  - [ ] Verify version: `zero --version`
...
- **Basic prompting:**
  ...
  - [ ] Run `zero "Tell me a joke"` and verify a sensible response.
  - [ ] Run in interactive mode: `zero`. Ask a follow-up question to test
        context.

- **Piped input:**

  - [ ] Run `echo "Summarize this" | zero` and verify it processes stdin.
...
- **Function calling:**
  - [ ] In interactive mode, ask zero to "create a file named hello.md with
...
- [ ] Navigate to `go/zero-cli-offline-evals-dash`.
