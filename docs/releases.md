...
package names beginning with `@google-zero/**` instead of `@google/**`.
...
| ---------- | ----------------------------- | ----------------------------------------- |
| CLI        | @google/zero-cli            | @google-zero/zero-cli                 |
| Core       | @google/zero-cli-core       | @google-zero/zero-cli-core A2A Server |
| A2A Server | @google/zero-cli-a2a-server | @google-zero/zero-cli-a2a-server      |
...
The workflow will then run `npm dist-tag add` for the appropriate `zero-cli`,
`zero-cli-core` and `zero-cli-a2a-server` packages, pointing the specified
...
- `npx -y @google/zero-cli@latest --version` to validate the push worked as
  expected if you were not doing a rc or dev tag
- `npx -y @google/zero-cli@<release tag> --version` to validate the tag pushed
  appropriately
- _This is destructive locally_
  `npm uninstall @google/zero-cli && npm uninstall -g @google/zero-cli && npm cache clean --force &&  npm install @google/zero-cli@<version>`
...
1.  Go to the
    [Actions tab](https://github.com/google-zero/zero-cli/actions/workflows/release-manual.yml)
    of the repository.
...
`packages/cli/google-zero-cli-0.1.6.tgz`).
...
- **What happens:** The `npm publish` command is run for the
  `@google/zero-cli-core` and `@google/zero-cli` packages.
- **Why:** This publishes them as standard Node.js packages. Users installing
  via `npm install -g @google/zero-cli` will download these packages, and
  `npm` will handle installing the `@google/zero-cli-core` dependency
...
1.  **The JavaScript bundle is created:**
    ...
    - **What happens:** The built JavaScript from both `packages/core/dist` and
      ...
      example, `zero.js`). The `node-pty` library is excluded from this bundle
...
2.  **The `bundle` directory is assembled:**
    ...
    - **What happens:** A temporary `bundle` folder is created at the project
      root. The single `zero.js` executable is placed inside it, along with
      other essential files.
    - **File movement:**
      - `zero.js` (from esbuild) -> `bundle/zero.js`
...
3.  **The GitHub release is created:**
    - **What happens:** The contents of the `bundle` directory, including the
      `zero.js` executable, are attached as assets to a new GitHub Release.
    - **Why:** This makes the single-file version of the CLI available for
      direct download and enables the
      `npx https://github.com/google-zero/zero-cli` command, which downloads
...
- **GitHub release:** Publishes a single, bundled `zero.js` file that contains
