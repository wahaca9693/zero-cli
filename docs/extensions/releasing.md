...
Release ZERO CLI extensions to your users through a Git repository or GitHub
...
The [ZERO CLI extension gallery](https://zerocli.com/extensions/browse/)
...
1.  **Use a public repository:** Ensure your extension is hosted in a public
    GitHub repository.
2.  **Add the GitHub topic:** Add the `zero-cli-extension` topic to your
    repository's **About** section. Our crawler uses this topic to find new
    extensions.
3.  **Place the manifest at the root:** Ensure your `zero-extension.json` file
...
extension using `zero extensions install <your-repo-uri>`.

Users can optionally depend on a specific branch, tag, or commit using the
`--ref` argument. For example:

```bash
zero extensions install <your-repo-uri> --ref=stable
```
...
ZERO CLI checks for updates by looking for the **Latest** release on GitHub.
...
To let ZERO CLI find the correct asset for a user's platform, use the
...
Archives must be fully contained extensions. The `zero-extension.json` file
...
`migratedTo` property in `zero-extension.json` to seamlessly transition your
users.
...
2.  **Update the old repository:** In your original repository, update the
    `zero-extension.json` file to include the `migratedTo` property pointing
...
When users check for updates, ZERO CLI detects the `migratedTo` field,
...
ZERO CLI automatically checks for extension updates based on the installation
method. Understanding these mechanisms helps you ensure your users always have
the latest version.

### Sync manifest and tags

For GitHub releases, always ensure the `version` in `zero-extension.json`
...
metadata file at `~/.zero/extensions/<name>/.zero-extension-install.json`.
