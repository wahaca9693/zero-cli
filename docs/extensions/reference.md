...
This guide covers the `zero extensions` commands and the structure of the
`zero-extension.json` configuration file.

## Manage extensions

Use the `zero extensions` command group to manage your extensions from the
terminal.

Note that commands like `zero extensions install` are not supported within the
...
ZERO CLI creates a copy of the extension during installation. You must run
`zero extensions update` to pull changes from the source. To install from
GitHub, you must have `git` installed on your machine.

```bash
zero extensions install <source> [--ref <ref>] [--auto-update] [--pre-release] [--consent] [--skip-settings]
```
...
```bash
zero extensions uninstall <name...>
```
...
```bash
zero extensions disable <name> [--scope <scope>]
```
...
```bash
zero extensions enable <name> [--scope <scope>]
```
...
Update an extension to the version specified in its `zero-extension.json`
file.

```bash
zero extensions update <name>
```
...
```bash
zero extensions new <path> [template]
```
...
Create a symbolic link between your development directory and ZERO CLI
extensions directory. This lets you test changes immediately without
reinstalling.

```bash
zero extensions link <path>
```

## Extension format

ZERO CLI loads extensions from `<home>/.zero/extensions`. Each extension
must have a `zero-extension.json` file in its root directory.
...
- `description`: A short description of the extension. This will be displayed on
  [zerocli.com/extensions](https://zerocli.com/extensions).
...
- `contextFileName`: The name of the file that contains the context for the
  ...
  If this property is not used but a `ZERO.md` file is present in your
...
- `plan`: Planning features configuration.
  - `directory`: The directory where planning artifacts are stored. This serves
    ...
    is `~/.zero/tmp/<project>/<session-id>/plans/`.

When ZERO CLI starts, it loads all the extensions and merges their
...
```bash
zero extensions config <name> [setting] [--scope <scope>]
```
...
2. Variables explicitly declared and requested in the `zero-extension.json`
...
`commands/` subdirectory. ZERO CLI uses the directory structure to determine
the command name.
...
hooks are not defined in the `zero-extension.json` manifest.
...
Extensions can contribute policy rules and safety checkers to ZERO CLI
...
place your `.toml` policy files inside it. ZERO CLI automatically loads all
...
<!-- prettier-ignore -->
> [!WARNING]
> For security, ZERO CLI ignores any `allow` decisions or `yolo`
...
defined in the `themes` array in `zero-extension.json`.
...
ZERO CLI supports variable substitution in `zero-extension.json` and
`hooks/hooks.json`.
