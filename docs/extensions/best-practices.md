# ZERO CLI extension best practices
...
Developing extensions for ZERO CLI is a lightweight, iterative process. Use
...
Use the `zero extensions link` command to develop locally without reinstalling
...
### Use `ZERO.md` effectively

Your `ZERO.md` file provides essential context to the model.
...
zero extensions install github.com/user/repo

# Install the development version
zero extensions install github.com/user/repo --ref dev
```

### Clean artifacts

When using GitHub Releases, ensure your archives only contain necessary files
(such as `dist/`, `zero-extension.json`, and `package.json`). Exclude
...
- **Manual verification:** Use `zero extensions link` to test your extension
...
- **Check the manifest:** Ensure `zero-extension.json` is in the root
...
- **Restart the CLI:** Extensions are loaded at the start of a session. Restart
  ZERO CLI after making changes to the manifest or linking a new extension.
...
- **Test the command:** Run the server's `command` and `args` directly in your
  terminal to ensure it starts correctly outside of ZERO CLI.
