# Build ZERO CLI extensions

ZERO CLI extensions let you expand the capabilities of ZERO CLI by adding
...
Before you start, ensure you have ZERO CLI installed and a basic understanding
of Node.js.

## Extension features

Extensions offer several ways to customize ZERO CLI. Use this table to decide
...
The manifest file tells ZERO CLI how to load and use your extension.
...
When a user installs this extension, ZERO CLI will prompt them to enter the
...
Link your extension to your ZERO CLI installation for local development.
...
2.  **Link the extension:**
    ...
    The `link` command creates a symbolic link from ZERO CLI extensions
...
Restart your ZERO CLI session to use the new `fetch_posts` tool. Test it by
asking: "fetch posts".
...
After saving the file, restart ZERO CLI. Run `/fs:grep-code "some pattern"` to
use your new command.

## Step 6: Add a custom `ZERO.md`

Provide persistent context to the model by adding a `ZERO.md` file to your
extension. This is useful for setting behavior or providing essential tool
information.

1.  Create a file named `ZERO.md` in the root of your extension directory:
...
2.  Update your `zero-extension.json` to load this file:
...
Restart ZERO CLI. The model now has the context from your `ZERO.md` file in
...
ZERO CLI automatically discovers skills bundled with your extension. The model
