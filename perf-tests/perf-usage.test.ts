...
import {
  ...
  PerfTestHarness,
  type PerfSnapshot,
} from '@google/zero-cli-test-utils';
...
describe('CPU Performance Tests', () => {
      ...
      try {
        ...
        return await harness.measure('cold-startup', async () => {
          await rig.run({
            args: ['hello'],
            timeout: 120000,
            env: { ZERO_API_KEY: 'fake-perf-test-key' },
          });
        });
      } finally {
        await rig.cleanup();
    ...
    const result = await harness.runScenario('skill-loading-time', async () => {
      ...
      try {
        ...
        for (let i = 0; i < SKILL_COUNT; i++) {
          const skillDir = `.zero/skills/perf-skill-${i}`;
          rig.mkdir(skillDir);
          rig.createFile(
            `${skillDir}/SKILL.md`,
            [
          ...
          const snapshot = await harness.measureWithEventLoop(
            ...
            async () => {
              await rig.run({
                ...
                timeout: 120000,
                env: {
                  ZERO_API_KEY: 'fake-perf-test-key',
                  ZERO_TELEMETRY_ENABLED: 'true',
                  ZERO_MEMORY_MONITOR_INTERVAL: '500',
                  ZERO_EVENT_LOOP_MONITOR_ENABLED: 'true',
                  DEBUG: 'true',
                },
              });
            },
  ...
  describe('long-conversation', () => {
    ...
    beforeAll(async () => {
      ...
      const zeroDir = join(rig.homeDir!, '.zero');
      const projectTempDir = join(zeroDir, 'tmp', identifier);
      const targetChatsDir = join(projectTempDir, 'chats');

      mkdirSync(targetChatsDir, { recursive: true });
      writeFileSync(
        join(zeroDir, 'projects.json'),
        JSON.stringify({
    ...
    it('terminal-scrolling: latency when scrolling a large terminal buffer', async () => {
      const result = await harness.runScenario(
        ...
        async () => {
          ...
          const settingsPath = join(rig.homeDir!, '.zero', 'settings.json');
          writeFileSync(
            settingsPath,
            JSON.stringify({
