...
```bash
export ZERO_API_KEY=$(grep '^ZERO_API_KEY=' .env | cut -d '=' -f2) && RUN_EVALS=1 npx vitest run --config evals/vitest.config.ts <file_name>
```
...
- **Verbose Reasoning**: Capture raw buffer traces by setting
  `ZERO_DEBUG_LOG_FILE`:
  ```bash
  export ZERO_DEBUG_LOG_FILE="debug.log"
  ```
...
```bash
zero /fix-behavioral-eval [optional-run-uri]
```
