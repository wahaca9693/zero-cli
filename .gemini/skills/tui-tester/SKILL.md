---
name: tui-tester
description: Expert guidance for testing ZERO CLI behavior and visual output using terminal automation.
---

# TUI Tester Skill

This skill provides the operational manual for verifying ZERO CLI behavioral changes and visual output using terminal automation.
...
### 4. ZERO CLI Specifics
- **Build First**: Always run `npm run build` or `npm run build:all` before testing local changes.
- **Bypass Trust**: Set `ZERO_CLI_TRUST_WORKSPACE=true` to avoid focus-stealing modals.
- **Isolate Config**: Use `ZERO_CLI_HOME` to prevent interference with your personal settings.

## Workflow Example

```bash
# Start the CLI
env ZERO_CLI_TRUST_WORKSPACE=true agent-tui run node packages/cli/dist/index.js
