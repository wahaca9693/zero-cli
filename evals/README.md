...
> [!TIP] **Agent Automation**: If you are pair-programming with ZERO CLI, you
...
behavioral evals focus on specific, granular behaviors relevant to the ZERO
CLI's features.
...
First, build the bundled ZERO CLI. You must do this after every code change.
...
The
[Evals: Nightly](https://github.com/google-zero/zero-cli/actions/workflows/evals-nightly.yml)
...
Tests should score at least 66% with key models including ZERO 3.1 pro, ZERO
3.0 pro, and ZERO 3 flash prior to check in and they must pass 100% of the
...
- **CI Evals**: Included in the
  [E2E (Chained)](https://github.com/google-zero/zero-cli/actions/workflows/chained_e2e.yml)
...
MODEL_LIST=zero-3-flash-preview node scripts/run_eval_regression.js
```
...
OUTPUT=$(node scripts/get_trustworthy_evals.js "zero-3-flash-preview")

# 2. Run the regression logic for those tests
node scripts/run_regression_check.js "zero-3-flash-preview" "$OUTPUT"
```
...
setting the `ZERO_DEBUG_LOG_FILE` environment variable.
...
unable to accomplish the goal with positive traits. ZERO is quite good at
