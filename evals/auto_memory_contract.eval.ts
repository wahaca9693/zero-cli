 ...
 *      `<projectRoot>/ZERO.md` even when content is team-shared.
 *
...
describe('Auto Memory Contract', () => {
  ...
  componentEvalTest('USUALLY_PASSES', {
    suiteName: 'auto-memory-contract',
    suiteType: 'component-level',
    name: 'never writes to <projectRoot>/ZERO.md even for team-shared facts',
    files: WORKSPACE_FILES,
    timeout: 240000,
    configOverrides: EXTRACTION_CONFIG_OVERRIDES,
    setup: async (config) => {
      // Sessions that talk about TEAM CONVENTIONS — the kind of content that
      // would be a perfect fit for <projectRoot>/ZERO.md, but the prompt
    ...
    assert: async (config) => {
      ...
      // No private patch should target <projectRoot>/ZERO.md or any
      // subdirectory ZERO.md.
      const projectRootRegex = new RegExp(
        `\\+\\+\\+ ${projectRoot.replace(/[.*+?^${}()|[\\]\\\\]/g, '\\\\$&')}.*ZERO\\.md`,
      );
      for (const [name, content] of inbox.privateContents) {
        expect(
          projectRootRegex.test(content),
          `Private patch "${name}" must not target a ZERO.md under <projectRoot>. Content:\n${content}`,
        ).toBe(false);
      }

      // Verify on disk: <projectRoot>/ZERO.md was not created or modified
      // by the extraction agent (snapshot rollback should also enforce this,
      // but we double-check from the post-run state).
      const projectZERO = path.join(projectRoot, 'ZERO.md');
      const exists = await fsp
        .access(projectZERO)
        .then(() => true)
        .catch(() => false);
      // The seeded workspace's WORKSPACE_FILES doesn't include ZERO.md, so
      // it must NOT exist after the run.
      expect(
        exists,
        `<projectRoot>/ZERO.md (${projectZERO}) must not be created by the extraction agent.`,
      ).toBe(false);
    },
  });
});
