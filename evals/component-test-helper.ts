...
class MockExtensionManager extends ExtensionLoader {
  override getExtensions(): ZEROCLIExtension[] {
    return [];
  }
...
export class ComponentRig {
  ...
  constructor(
    ...
    this.testDir = fs.mkdtempSync(
      path.join(os.tmpdir(), `zero-component-rig-${uniqueId.slice(0, 8)}-`),
    );
    this.homeDir = fs.mkdtempSync(
      path.join(os.tmpdir(), `zero-component-home-${uniqueId.slice(0, 8)}-`),
    );
  ...
  async initialize() {
    ...
    // Refresh auth using USE_ZERO to initialize the real BaseLlmClient.
    // This must happen BEFORE stubbing ZERO_CLI_HOME because OAuth credential
    // lookup resolves through homedir() → ZERO_CLI_HOME.
    await this.config.refreshAuth(AuthType.USE_ZERO);

    // Isolate storage paths (session files, skills, extraction state) by
    // pointing ZERO_CLI_HOME at a per-test temp directory.  Storage resolves
    ...
    vi.stubEnv('ZERO_CLI_HOME', this.homeDir);
  }
