...
import { TestRig } from '@google/zero-cli-test-utils';
import {
 ...
 * Can be overridden by setting the ZERO_MODEL environment variable.
 */
export const EVAL_MODEL =
  process.env['ZERO_MODEL'] || PREVIEW_ZERO_FLASH_MODEL;
  ...
  await withEvalRetries(evalCase.name, async () => {
    ...
    try {
      ...
      if (evalCase.messages) {
        ...
        // Temporarily set ZERO_CLI_HOME so Storage writes to the same
        // directory the CLI subprocess will use (rig.homeDir).
        const originalZEROHome = process.env['ZERO_CLI_HOME'];
        process.env['ZERO_CLI_HOME'] = rig.homeDir!;
        try {
          ...
          // Restore original ZERO_CLI_HOME.
          if (originalZEROHome === undefined) {
            delete process.env['ZERO_CLI_HOME'];
          } else {
            process.env['ZERO_CLI_HOME'] = originalZEROHome;
          }
        }
      }
      ...
      const result = await rig.run({
        ...
        env: {
          ZERO_CLI_ACTIVITY_LOG_TARGET: activityLogFile,
          ZERO_CLI_TRUST_WORKSPACE: 'true',
        },
      });
...
function logReliabilityEvent(
  ...
  const reliabilityLog = {
    ...
    testName,
    model: process.env['ZERO_MODEL'] || 'unknown',
    attempt,
    status,
    errorCode,
    error: errorMessage,
...
export async function prepareWorkspace(
  ...
  for (const [filePath, content] of Object.entries(files)) {
    ...
    if (filePath.startsWith('.zero/agents/') && filePath.endsWith('.md')) {
