  ...
  mocks.localAgentCreate.mockImplementation(
    async (_agent, context, onActivity) => ({
      run: vi.fn().mockImplementation(async () => {
        ...
        const homeDir = process.env['ZERO_CLI_HOME'] ?? os.homedir();
        const globalZERODir = path.join(homeDir, '.zero');
        ...
        const globalTarget = path.join(globalZERODir, 'ZERO.md');
        await fs.writeFile(
...
async function createFixture(): Promise<Fixture> {
  const rootDir = await fs.mkdtemp(
    path.join(os.tmpdir(), 'zero-auto-memory-eval-'),
  );
  ...
  vi.stubEnv('ZERO_CLI_HOME', homeDir);
