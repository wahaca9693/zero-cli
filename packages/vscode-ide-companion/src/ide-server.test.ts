...
const getPortFromMock = (
  ...
  const port = vi
    .mocked(replaceMock)
    .mock.calls.find((call) => call[0] === 'ZERO_CLI_IDE_SERVER_PORT')?.[1];
...
describe('IDEServer', () => {
  ...
  it('should set environment variables and workspace path on start with multiple folders', async () => {
    ...
    const expectedPortFile = path.join(
      ...
      'zero',
      'ide',
      `zero-ide-server-${process.ppid}-${port}.json`,
    );
    ...
    expect(fs.mkdir).toHaveBeenCalledWith(path.join('/tmp', 'zero', 'ide'), {
      recursive: true,
    });
  ...
  it('should set an empty string if no folders are open', async () => {
    ...
    expect(replaceMock).toHaveBeenCalledWith('ZERO_CLI_IDE_WORKSPACE_PATH', '');
