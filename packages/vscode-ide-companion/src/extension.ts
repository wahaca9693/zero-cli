...
export const DIFF_SCHEME = 'zero-diff';
  ...
  try {
    ...
    if (
      ...
      const selection = await vscode.window.showInformationMessage(
        `A new version (${latestVersion}) of the ZERO CLI Companion extension is available.`,
...
export async function activate(context: vscode.ExtensionContext) {
  logger = vscode.window.createOutputChannel('ZERO CLI IDE Companion');
  ...
  if (
    ...
    void vscode.window.showInformationMessage(
      'ZERO CLI Companion extension successfully installed.',
    );
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand('zero-cli.runZEROCLI', async () => {
      const workspaceFolders = vscode.workspace.workspaceFolders;
      if (!workspaceFolders || workspaceFolders.length === 0) {
        vscode.window.showInformationMessage(
          'No folder open. Please open a folder to run ZERO CLI.',
        );
        return;
      }
      ...
      if (workspaceFolders.length === 1) {
        ...
        selectedFolder = await vscode.window.showWorkspaceFolderPick({
          placeHolder: 'Select a folder to run ZERO CLI in',
        });
      }
      ...
      if (selectedFolder) {
        ...
        const terminal = vscode.window.createTerminal({
          name: `ZERO CLI (${selectedFolder.name})`,
          cwd: selectedFolder.uri.fsPath,
        });
        terminal.show();
        terminal.sendText(zeroCmd);
      }
    }),
    vscode.commands.registerCommand('zero-cli.showNotices', async () => {
