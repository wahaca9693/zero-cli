...
const BUNDLE_PATH = join(__dirname, '..', '..', '..', 'bundle/zero.js');

// Get timeout based on environment
export function getDefaultTimeout() {
  if (env['CI']) return 60000; // 1 minute in CI
  if (env['ZERO_SANDBOX']) return 30000; // 30s in containers
...
export class TestRig {
  ...
  setup(
    ...
    const testFileDir =
      env['INTEGRATION_TEST_FILE_DIR'] || join(os.tmpdir(), 'zero-cli-tests');
  ...
  private _createSettingsFile(overrideSettings?: Record<string, unknown>) {
    const projectZERODir = join(this.testDir!, ZERO_DIR);
    mkdirSync(projectZERODir, { recursive: true });

    const userZERODir = join(this.homeDir!, ZERO_DIR);
    mkdirSync(userZERODir, { recursive: true });
    ...
    const settings = deepMerge(
      {
        ...
        ...(env['ZERO_TEST_TYPE'] === 'integration'
          ? {
              model: {
                name: PREVIEW_ZERO_FLASH_MODEL,
              },
            }
          : {}),
    ...
    writeFileSync(
      join(projectZERODir, 'settings.json'),
      JSON.stringify(settings, null, 2),
    );
    writeFileSync(
      join(userZERODir, 'settings.json'),
      JSON.stringify(settings, null, 2),
    );
  }
  ...
  /**
   * The command and args to use to invoke ZERO CLI. Allows us to switch
   * between using the bundled zero.js (the default) and using the installed
   * 'zero' (used to verify npm bundles).
   */
  private _getCommandAndArgs(extraInitialArgs: string[] = []): {
    command: string;
    initialArgs: string[];
  } {
    const binaryPath = env['INTEGRATION_TEST_ZERO_BINARY_PATH'];
    const isNpmReleaseTest =
      env['INTEGRATION_TEST_USE_INSTALLED_ZERO'] === 'true';
    const zeroCommand = os.platform() === 'win32' ? 'zero.cmd' : 'zero';
    let command = 'node';
  ...
  addTestMcpServer(name: string, config: TestMcpConfig | string) {
    ...
    const updateSettings = (dir: string) => {
      const settingsPath = join(dir, ZERO_DIR, 'settings.json');
      let settings: any = {};
      if (fs.existsSync(settingsPath)) {
        settings = JSON.parse(fs.readFileSync(settingsPath, 'utf-8'));
      } else {
        fs.mkdirSync(join(dir, ZERO_DIR), { recursive: true });
      }
  ...
  private _getCleanEnv(
    ...
    // Clear all ZERO_ environment variables that might interfere with tests
    // except for those we explicitly want to keep or set.
    for (const key of Object.keys(cleanEnv)) {
      if (
        (key.startsWith('ZERO_') || key.startsWith('GOOGLE_ZERO_')) &&
        key !== 'ZERO_API_KEY' &&
        key !== 'GOOGLE_API_KEY' &&
        key !== 'ZERO_MODEL' &&
        key !== 'ZERO_DEBUG' &&
        key !== 'ZERO_CLI_TEST_VAR' &&
        key !== 'ZERO_CLI_INTEGRATION_TEST' &&
        key !== 'GOOGLE_ZERO_BASE_URL' &&
        !key.startsWith('ZERO_CLI_ACTIVITY_LOG')
      ) {
        delete cleanEnv[key];
      }
    }
    ...
    return {
      ...cleanEnv,
      ZERO_CLI_HOME: this.homeDir!,
      ZERO_PTY_INFO: 'child_process',
      ...extraEnv,
    };
  }
    ...
    return poll(
      () => {
        ...
        return logs.some(
          (logData) =>
            logData.attributes &&
            logData.attributes['event.name'] === `zero_cli.${eventName}`,
        );
      },
      timeout,
      100,
          ...
          if (braceDepth === 0) {
            ...
            try {
              ...
              if (
                ...
                obj.attributes &&
                obj.attributes['event.name'] === 'zero_cli.tool_call'
              ) {
                logs.push({
  ...
  readToolLogs() {
    ...
    for (const logData of parsedLogs) {
      ...
      if (
        logData.attributes &&
        logData.attributes['event.name'] === 'zero_cli.tool_call'
      ) {
  ...
  readAllApiRequest(): ParsedLog[] {
    ...
    const apiRequests = logs.filter(
      (logData) =>
        logData.attributes &&
        logData.attributes['event.name'] === `zero_cli.api_request`,
    );
    return apiRequests;
  }
  ...
  async waitForMetric(metricName: string, timeout?: number) {
    ...
    const fullName = metricName.startsWith('zero_cli.')
      ? metricName
      : `zero_cli.${metricName}`;
  ...
  readMetric(metricName: string): TelemetryMetric | null {
    ...
    for (const logData of logs) {
      if (logData && logData.scopeMetrics) {
        for (const scopeMetric of logData.scopeMetrics) {
          for (const metric of scopeMetric.metrics) {
            if (metric.descriptor.name === `zero_cli.${metricName}`) {
              return metric;
            }
          }
        }
  ...
  private _getMemorySnapshots(): {
    ...
    for (const logData of logs) {
      if (logData && logData.scopeMetrics) {
        for (const scopeMetric of logData.scopeMetrics) {
          for (const metric of scopeMetric.metrics) {
            if (metric.descriptor.name === 'zero_cli.memory.usage') {
  ...
  readHookLogs() {
    ...
    for (const logData of parsedLogs) {
      ...
      if (
        logData.attributes &&
        logData.attributes['event.name'] === 'zero_cli.hook_call'
      ) {
        logs.push({
          hookCall: {
