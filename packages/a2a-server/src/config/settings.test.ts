...
vi.mock('node:os', async (importOriginal) => {
  ...
  return {
    ...actual,
    homedir: () => path.join(actual.tmpdir(), `zero-home-${mocks.suffix}`),
  };
});
...
vi.mock('@allhands/zero-cli-core', async (importOriginal) => {
  ...
  return {
    ...
    homedir: () => path.join(os.tmpdir(), `zero-home-${mocks.suffix}`),
...
describe('loadSettings', () => {
  const mockHomeDir = path.join(os.tmpdir(), `zero-home-${mocks.suffix}`);
  const mockWorkspaceDir = path.join(
    os.tmpdir(),
    `zero-workspace-${mocks.suffix}`,
  );
  const mockZEROHomeDir = path.join(mockHomeDir, '.zero');
  const mockZEROWorkspaceDir = path.join(mockWorkspaceDir, '.zero');

  beforeEach(() => {
    vi.clearAllMocks();
    // Create the directories using the real fs
    if (!fs.existsSync(mockZEROHomeDir)) {
      fs.mkdirSync(mockZEROHomeDir, { recursive: true });
    }
    if (!fs.existsSync(mockZEROWorkspaceDir)) {
      fs.mkdirSync(mockZEROWorkspaceDir, { recursive: true });
    }
