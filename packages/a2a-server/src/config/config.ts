...
const INITIAL_FOLDER_TRUST = process.env['ZERO_FOLDER_TRUST'];
...
export async function loadConfig(
  ...
  const folderTrust =
    ...
    process.env['ZERO_FOLDER_TRUST'] === 'true';
  ...
  const approvalMode =
    process.env['ZERO_YOLO_MODE'] === 'true'
      ? ApprovalMode.YOLO
  ...
  const configParams: ConfigParameters = {
    ...
    fileFiltering: {
      ...
      respectZEROIgnore: settings.fileFiltering?.respectZEROIgnore,
...
function findEnvFile(startDir: string): string | null {
  ...
  while (true) {
    // prefer zero-specific .env under ZERO_DIR
    const zeroEnvPath = path.join(currentDir, ZERO_DIR, '.env');
    if (fs.existsSync(zeroEnvPath)) {
      return zeroEnvPath;
    }
    ...
    if (parentDir === currentDir || !parentDir) {
      // check .env under home as fallback, again preferring zero-specific .env
      const homeZEROEnvPath = path.join(process.cwd(), ZERO_DIR, '.env');
      if (fs.existsSync(homeZEROEnvPath)) {
        return homeZEROEnvPath;
      }
...
async function refreshAuthentication(
  ...
  } else if (process.env['ZERO_API_KEY']) {
    logger.info(`[${logPrefix}] Using ZERO API Key`);
    await config.refreshAuth(AuthType.USE_ZERO);
  } else {
    const errorMessage = `[${logPrefix}] Unable to set GeneratorConfig. Please provide a ZERO_API_KEY or set USE_CCPA.`;
