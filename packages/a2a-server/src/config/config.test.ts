...
describe('loadConfig', () => {
  ...
  describe('tool configuration', () => {
    ...
    describe('YOLO mode', () => {
      it('should enable YOLO mode and add policy rule when ZERO_YOLO_MODE is true', async () => {
        vi.stubEnv('ZERO_YOLO_MODE', 'true');
        await loadConfig(mockSettings, mockExtensionLoader, taskId);
        expect(Config).toHaveBeenCalledWith(
          expect.objectContaining({
            approvalMode: 'yolo',
      ...
      it('should use default approval mode and load default rules when ZERO_YOLO_MODE is not true', async () => {
        vi.stubEnv('ZERO_YOLO_MODE', 'false');
        await loadConfig(mockSettings, mockExtensionLoader, taskId);
        expect(Config).toHaveBeenCalledWith(
          expect.objectContaining({
            approvalMode: 'default',
...
describe('setIsTrusted', () => {
  ...
  it('should return true when ZERO_FOLDER_TRUST env var is true', async () => {
    vi.stubEnv('ZERO_FOLDER_TRUST', 'true');
  ...
  it('should return false when ZERO_FOLDER_TRUST env var is false', async () => {
    vi.stubEnv('ZERO_FOLDER_TRUST', 'false');
    const { setIsTrusted } = await import('./config.js');
