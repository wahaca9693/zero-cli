...
export class Task {
  ...
  private constructor(
    ...
    this.zeroClient = loopContext.zeroClient;
  ...
  // state managed within the @zero-cli/core module.
  ...
  async acceptAgentMessage(event: ServerZEROStreamEvent): Promise<void> {
    const stateChange: StateChange = {
      kind: CoderAgentEvent.StateChangeEvent,
    };
    const traceId =
    ...
    switch (event.type) {
      ...
      case ZEROEventType.ToolCallResponse:
        // This event type from ServerZEROStreamEvent might be for when LLM *generates* a tool response part.
      ...
      default: {
        ...
        let errorEvent: ServerZEROErrorEvent | undefined;
        if (
          event.type === ZEROEventType.Error &&
          event.value &&
          typeof event.value === 'object' &&
  ...
  async *sendCompletedToolsToLlm(
    ...
    yield* this.zeroClient.sendMessageStream(
      llmParts,
      aborted,
