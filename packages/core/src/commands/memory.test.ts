...
describe('memory commands', () => {
  ...
  beforeEach(() => {
    ...
    mockConfig = {
      getUserMemory: vi.fn(),
      getZEROMdFileCount: vi.fn(),
      getZEROMdFilePaths: vi.fn(),
  ...
  describe('showMemory', () => {
    it('should show memory content if it exists', () => {
      ...
      vi.mocked(mockConfig.getZEROMdFileCount).mockReturnValue(1);
    ...
    it('should show a message if memory is empty', () => {
      vi.mocked(mockConfig.getUserMemory).mockReturnValue('');
      vi.mocked(mockConfig.getZEROMdFileCount).mockReturnValue(0);
  ...
  describe('refreshMemory', () => {
    it('should refresh memory and show success message', async () => {
      ...
      vi.mocked(mockConfig.getZEROMdFileCount).mockReturnValue(2);
  ...
  describe('listMemoryFiles', () => {
    it('should list the memory files in use', () => {
      const filePaths = ['/path/to/ZERO.md', '/other/path/ZERO.md'];
      vi.mocked(mockConfig.getZEROMdFilePaths).mockReturnValue(filePaths);
      ...
      if (result.type === 'message') {
        expect(result.messageType).toBe('info');
        expect(result.content).toContain(
          'There are 2 ZERO.md file(s) in use:',
        );
    ...
    it('should show a message if no memory files are in use', () => {
      vi.mocked(mockConfig.getZEROMdFilePaths).mockReturnValue([]);
      ...
      if (result.type === 'message') {
        expect(result.messageType).toBe('info');
        expect(result.content).toBe('No ZERO.md files in use.');
      }
    });
  ...
  describe('memory patch inbox', () => {
    ...
    beforeEach(async () => {
      ...
      vi.mocked(Storage.getGlobalZERODir).mockReturnValue(globalMemoryDir);
    });
    ...
    it('omits patches whose headers leave the allowed root from the listing', async () => {
      ...
      await fs.writeFile(
        ...
        buildCreationPatch(path.join(projectRoot, 'ZERO.md'), 'Hi.\n'),
      );
    ...
    it('omits global patches with disallowed targets from the listing', async () => {
      // Same defense for the global tier: only ~/.zero/ZERO.md is allowed.
      ...
      await fs.writeFile(
        ...
        buildCreationPatch(
          path.join(globalMemoryDir, 'ZERO.md', 'nested.md'),
          'rejected\n',
        ),
      );
    ...
    it('applies a global creation patch to ~/.zero/ZERO.md', async () => {
      const target = path.join(globalMemoryDir, 'ZERO.md');
      ...
      await expect(
        fs.access(path.join(patchDir, 'ZERO.patch')),
      ).rejects.toThrow();
    });

    it('applies a global update patch to ~/.zero/ZERO.md', async () => {
      const target = path.join(globalMemoryDir, 'ZERO.md');
    ...
    it('dismisses a single memory patch from the inbox (legacy single-file mode)', async () => {
      ...
      await fs.writeFile(
        path.join(patchDir, 'ZERO.patch'),
        buildCreationPatch(
          path.join(globalMemoryDir, 'ZERO.md'),
          'Prefer concise.\n',
        ),
      );
    ...
    it('rejects global patches that target anything other than ~/.zero/ZERO.md', async () => {
      const patchDir = path.join(memoryTempDir, '.inbox', 'global');
      await fs.mkdir(patchDir, { recursive: true });
      ...
      // Sibling .md files in ~/.zero/ are also not allowed.
      await fs.writeFile(
        path.join(patchDir, 'sibling.patch'),
        buildCreationPatch(
          path.join(globalMemoryDir, 'notes.md'),
      ...
      await expect(
        fs.access(path.join(globalMemoryDir, 'ZERO.md', 'nested.md')),
      ).rejects.toThrow();
    });
  ...
  describe('moveInboxSkill', () => {
    ...
    it('should move a skill to global skills directory', async () => {
      ...
      expect(result.success).toBe(true);
      expect(result.message).toBe('Moved "my-skill" to ~/.zero/skills.');
