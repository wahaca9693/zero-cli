...
/**
 * Connects to the ZERO Live API using raw WebSockets to support API Key authentication.
 */
export class ZEROLiveTranscriptionProvider
  ...
  implements TranscriptionProvider
{
  ...
  async connect(): Promise<void> {
    const modelName = 'zero-3.1-flash-live-preview';
    const baseUrl =
    ...
    debugLogger.debug(
      `[ZEROLiveTranscription] Connecting to model ${modelName} via raw WebSocket with API Key...`,
    );
    ...
    try {
      ...
      this.ws.on('message', (data) => {
        try {
          ...
          if (result.success) {
            ...
            if (response.serverContent) {
              ...
              if (content.inputTranscription?.text) {
                ...
                debugLogger.debug(
                  `[ZEROLiveTranscription] Transcription received (Cloud): "${text}"`,
                );
          ...
          debugLogger.error(
            '[ZEROLiveTranscription] Error parsing message:',
            e,
          );
        }
      });
      ...
      this.ws.on('error', (error) => {
        debugLogger.error('[ZEROLiveTranscription] WebSocket Error:', error);
        this.emit('error', error);
      });

      this.ws.on('close', (code, reason) => {
        debugLogger.debug(
          `[ZEROLiveTranscription] Connection Closed. Code: ${code}, Reason: ${reason}`,
        );
        this.emit('close');
        this.ws = null;
      });
      ...
      debugLogger.error(
        '[ZEROLiveTranscription] Failed to establish connection:',
        err,
      );
      throw err;
    }
