...
/**
 * @fileoverview LegacyAgentSession backed by the existing ZERO client +
...
export class LegacyAgentProtocol implements AgentProtocol {
  ...
  private readonly _client: ZEROClient;
  ...
  constructor(deps: LegacyAgentSessionDeps) {
    ...
    this._client = deps.client ?? deps.config.getZEROClient();
  ...
  async send(payload: AgentSend): Promise<{ streamId: string }> {
    ...
    const parts = contentPartsToZEROParts(message.content);
    ...
    while (true) {
      ...
      for (const tc of completedToolCalls) {
        ...
        const content: ContentPart[] = response.error
          ...
          : zeroPartsToContentParts(response.responseParts);
