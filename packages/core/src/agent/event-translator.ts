 ...
 * ServerZEROStreamEvent objects into AgentEvent objects.
 *
...
export function translateEvent(
  ...
  switch (event.type) {
    ...
    case ZEROEventType.Finished:
      handleFinished(event.value, state, out);
      break;

    case ZEROEventType.Error:
      handleError(event.value.error, state, out);
      break;

    case ZEROEventType.UserCancelled:
      ensureStreamStart(state, out);
      out.push(
    ...
    case ZEROEventType.MaxSessionTurns:
      ensureStreamStart(state, out);
      out.push(
        makeEvent('agent_end', state, {
          reason: 'max_turns',
    ...
    case ZEROEventType.LoopDetected:
      ensureStreamStart(state, out);
      out.push(
    ...
    case ZEROEventType.ContextWindowWillOverflow:
      ensureStreamStart(state, out);
      out.push(
        makeEvent('error', state, {
    ...
    case ZEROEventType.AgentExecutionStopped:
      ensureStreamStart(state, out);
      out.push(
        makeEvent('agent_end', state, {
          reason: 'completed',
    ...
    case ZEROEventType.AgentExecutionBlocked:
      ensureStreamStart(state, out);
      out.push(
        makeEvent('error', state, {
    ...
    case ZEROEventType.InvalidStream:
      ensureStreamStart(state, out);
      out.push(
        makeEvent('error', state, {
          status: 'INTERNAL',
    ...
    case ZEROEventType.ToolCallRequest:
      ensureStreamStart(state, out);
    ...
    case ZEROEventType.ToolCallResponse: {
      ensureStreamStart(state, out);
      ...
      out.push(
        makeEvent('tool_response', state, {
          ...
          content: event.value.error
            ...
            : zeroPartsToContentParts(event.value.responseParts),
    ...
    case ZEROEventType.ToolCallConfirmation:
      // Elicitations are handled separately by the session layer
      break;

    // Internal concerns — no AgentEvent emitted
    case ZEROEventType.ChatCompressed:
    case ZEROEventType.Retry:
      break;
...
/**
 * Maps a ZERO FinishReason to an AgentEnd reason.
 */
...
/**
 * Maps ZERO usageMetadata to Usage.
 */
