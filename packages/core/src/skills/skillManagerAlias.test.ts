...
describe('SkillManager Alias', () => {
  ...
  it('should discover skills from .agents/skills directory', async () => {
    const userZERODir = path.join(testRootDir, 'user', '.zero', 'skills');
    const userAgentDir = path.join(testRootDir, 'user', '.agents', 'skills');
    const projectZERODir = path.join(
      testRootDir,
      'workspace',
      '.zero',
      'skills',
    ...
    await fs.mkdir(userZERODir, { recursive: true });
    await fs.mkdir(userAgentDir, { recursive: true });
    await fs.mkdir(projectZERODir, { recursive: true });
    ...
    vi.spyOn(Storage, 'getUserSkillsDir').mockReturnValue(userZERODir);
    ...
    vi.spyOn(storage, 'getProjectSkillsDir').mockReturnValue(projectZERODir);
    ...
    expect(names).toContain('user-zero');
    expect(names).toContain('user-agent');
    expect(names).toContain('project-zero');
    expect(names).toContain('project-agent');
  });

  it('should give .agents precedence over .zero when in the same tier', async () => {
    const userZERODir = path.join(testRootDir, 'user', '.zero', 'skills');
    const userAgentDir = path.join(testRootDir, 'user', '.agents', 'skills');
