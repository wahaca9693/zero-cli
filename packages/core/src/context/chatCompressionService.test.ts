...
import type { ZEROChat } from '../core/zeroChat.js';
...
describe('modelStringToModelConfigAlias', () => {
  it('should return the default model for unexpected aliases', () => {
    expect(modelStringToModelConfigAlias('zero-flash-flash')).toBe(
      'chat-compression-default',
    );
  });

  it('should handle valid names', () => {
    expect(modelStringToModelConfigAlias('zero-3-pro-preview')).toBe(
      'chat-compression-3-pro',
    );
    expect(modelStringToModelConfigAlias('zero-2.5-pro')).toBe(
      'chat-compression-2.5-pro',
    );
    expect(modelStringToModelConfigAlias('zero-2.5-flash')).toBe(
      'chat-compression-2.5-flash',
    );
    expect(modelStringToModelConfigAlias('zero-2.5-flash-lite')).toBe(
...
describe('ChatCompressionService', () => {
  ...
  let mockChat: ZEROChat;
  let mockConfig: Config;
  let testTempDir: string;
  const mockModel = 'zero-2.5-pro';
