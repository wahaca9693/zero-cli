 ...
 * by the ZERO API. We use this as a baseline heuristic for inlineData/fileData.
 */
...
export class StaticTokenCalculator implements AdvancedTokenCalculator {
  ...
  /**
   * Calculates the token cost for a single ZERO Content object.
   */
  calculateContentTokens(content: Content): number {
    return (
  ...
  /**
   * Slower, precise estimation for a ZERO Content/Part graph.
