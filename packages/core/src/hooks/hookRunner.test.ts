...
describe('HookRunner', () => {
    ...
    describe('command hooks', () => {
      ...
      it('should expand environment variables in commands', async () => {
        const configWithEnvVar: HookConfig = {
          type: HookType.Command,
          command: '$ZERO_PROJECT_DIR/hooks/test.sh',
        };
        ...
        expect(spawn).toHaveBeenCalledWith(
          ...
          expect.objectContaining({
            ...
            env: expect.objectContaining({
              ZERO_PROJECT_DIR: '/test/project',
              ZERO_PLANS_DIR: '/test/project/plans',
              ZERO_CWD: '/test/project',
              ZERO_SESSION_ID: 'test-session',
      ...
      it('should expand and escape ZERO_PLANS_DIR in commands', async () => {
        const configWithEnvVar: HookConfig = {
          type: HookType.Command,
          command: 'ls $ZERO_PLANS_DIR',
        };
      ...
      it('should not allow command injection via ZERO_PROJECT_DIR', async () => {
  ...
  describe('executeHooksSequential', () => {
    ...
    it('should pass modified LLM request from one hook to the next for BeforeModel', async () => {
      ...
      expect(secondHookInput.llm_request.model).toBe('zero-1.5-pro');
