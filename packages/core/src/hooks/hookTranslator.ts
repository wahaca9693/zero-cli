...
/**
 * Decoupled LLM request format - stable across ZERO CLI versions
 */
...
/**
 * Decoupled LLM response format - stable across ZERO CLI versions
 */
...
/**
 * Decoupled tool configuration - stable across ZERO CLI versions
 */
...
export class HookTranslatorGenAIv1 extends HookTranslator {
  ...
  toHookLLMRequest(sdkRequest: GenerateContentParameters): LLMRequest {
    ...
    return {
      model: sdkRequest.model || DEFAULT_ZERO_FLASH_MODEL,
      messages,
      config: {
        temperature: config?.temperature,
        maxOutputTokens: config?.maxOutputTokens,
  ...
  fromHookLLMRequest(
    ...
    // (see https://github.com/google-zero/zero-cli/issues/25558).
    //
