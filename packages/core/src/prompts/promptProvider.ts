...
import { ZERO_DIR, makeRelative } from '../utils/paths.js';
...
export class PromptProvider {
  ...
  getCoreSystemPrompt(
    ...
    const desiredModel = resolveModel(
      ...
      context.config.getZERO31LaunchedSync?.() ?? false,
      false,
      context.config.getHasAccessToPreviewModel?.() ?? true,
      context.config,
      context.config.hasZERO35FlashGAAccess?.() ?? false,
    );
    const isModernModel = supportsModernFeatures(desiredModel);
    const activeSnippets = isModernModel ? snippets : legacySnippets;
    const contextFilenames = getAllZEROMdFilenames();
    ...
    if (systemMdResolution.value && !systemMdResolution.isDisabled) {
      let systemMdPath = path.resolve(path.join(ZERO_DIR, 'system.md'));
    ...
    this.maybeWriteSystemMd(
      sanitizedPrompt,
      systemMdResolution,
      path.resolve(path.join(ZERO_DIR, 'system.md')),
    );
  ...
  private maybeWriteSystemMd(
    ...
    const writeSystemMdResolution = resolvePathFromEnv(
      process.env['ZERO_WRITE_SYSTEM_MD'],
    );
