...
describe('PromptProvider', () => {
  ...
  beforeEach(() => {
    vi.resetAllMocks();
    vi.stubEnv('ZERO_SYSTEM_MD', '');
    vi.stubEnv('ZERO_WRITE_SYSTEM_MD', '');
    ...
    mockConfig = {
      ...
      getZERO31LaunchedSync: vi.fn().mockReturnValue(true),
  ...
  it('should handle multiple context filenames in the system prompt', () => {
    vi.mocked(getAllZEROMdFilenames).mockReturnValue([
      DEFAULT_CONTEXT_FILENAME,
      'CUSTOM.md',
      'ANOTHER.md',
    ]);
  ...
  describe('Topic & Update Narration', () => {
    beforeEach(() => {
      ...
      vi.mocked(mockConfig.getZERO31LaunchedSync).mockReturnValue(true);
    });
