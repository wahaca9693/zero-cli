...
const USER_AGENT =
  'Mozilla/5.0 (compatible; Google-ZERO-CLI/1.0; +https://github.com/google-zero/zero-cli)';
...
> {
  ...
  private async executeFallback(
    ...
    try {
      const zeroClient = this.context.zeroClient;
...
`;
      const result = await zeroClient.generateContent(
...
`;
      const response = await zeroClient.generateContent(
        { model: 'web-fetch' },
