...
describe('ReadFileTool', () => {
  ...
  describe('execute', () => {
    ...
    describe('with .zeroignore', () => {
      ...
      it('should throw error if path is ignored by a .zeroignore pattern', async () => {
      ...
      it('should allow reading ignored files if respectZEROIgnore is false', async () => {
        const ignoredFilePath = path.join(tempRootDir, 'foo.bar');
        await fsp.writeFile(ignoredFilePath, 'content', 'utf-8');
  ...
  describe('getSchema', () => {
    ...
    it('should return the schema from the resolver when modelId is provided', () => {
      const modelId = 'zero-2.0-flash';
      const schema = tool.getSchema(modelId);
      expect(schema.name).toBe(ReadFileTool.Name);
      expect(schema.description).toMatchSnapshot();
    });

    it('should return the ZERO 3 schema when a ZERO 3 modelId is provided', () => {
      const modelId = 'zero-3-pro-preview';
      const schema = tool.getSchema(modelId);
      expect(schema.name).toBe(ReadFileTool.Name);
      expect(schema.description).toMatchSnapshot();
