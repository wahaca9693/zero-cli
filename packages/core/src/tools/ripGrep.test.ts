...
import { ZERO_IGNORE_FILE_NAME } from '../config/constants.js';
...
function createMockConfig(
  ...
  const config = {
    ...
    getFileFilteringRespectZEROIgnore(this: Config) {
      return this.getFileFilteringOptions().respectZEROIgnore;
    },
    storage: {
...
describe('RipGrepTool', () => {
  ...
  describe('execute', () => {
    ...
    it('should filter out files based on FileDiscoveryService even if ripgrep returns them', async () => {
      // Create .zeroignore to ignore 'ignored.txt'
      await fs.writeFile(
        path.join(tempRootDir, ZERO_IGNORE_FILE_NAME),
        'ignored.txt',
      );

      // Re-initialize tool so FileDiscoveryService loads the new .zeroignore
  ...
  describe('advanced search options', () => {
    ...
    it('should add .zeroignore when enabled and patterns exist', async () => {
      const zeroIgnorePath = resolveToRealPath(
        path.join(tempRootDir, ZERO_IGNORE_FILE_NAME),
      );
      await fs.writeFile(zeroIgnorePath, 'ignored.log');

      const configWithZEROIgnore = createMockConfig(tempRootDir);
      vi.spyOn(
        configWithZEROIgnore,
        'getFileFilteringOptions',
      ).mockReturnValue({
        respectGitIgnore: true,
        respectZEROIgnore: true,
      ...
      const zeroIgnoreTool = new RipGrepTool(
        configWithZEROIgnore,
        createMockMessageBus(),
      );
      ...
      const params: RipGrepToolParams = { pattern: 'secret' };
      const invocation = zeroIgnoreTool.build(params);
      await invocation.execute({ abortSignal });

      expect(mockSpawn).toHaveBeenLastCalledWith(
        expect.anything(),
        expect.arrayContaining(['--ignore-file', zeroIgnorePath]),
        expect.anything(),
      );
    });

    it('should skip .zeroignore when disabled', async () => {
      const zeroIgnorePath = resolveToRealPath(
        path.join(tempRootDir, ZERO_IGNORE_FILE_NAME),
      );
      await fs.writeFile(zeroIgnorePath, 'ignored.log');
      const configWithoutZEROIgnore = createMockConfig(tempRootDir);
      vi.spyOn(
        configWithoutZEROIgnore,
        'getFileFilteringOptions',
      ).mockReturnValue({
        respectGitIgnore: true,
      ...
      expect(mockSpawn).toHaveBeenLastCalledWith(
        expect.anything(),
        expect.not.arrayContaining(['--ignore-file', zeroIgnorePath]),
        expect.anything(),
      );
    });
