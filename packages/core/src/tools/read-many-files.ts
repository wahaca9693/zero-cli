...
export interface ReadManyFilesParams {
  ...
  /**
   * Whether to respect .gitignore and .zeroignore patterns (optional, defaults to true)
   */
  file_filtering_options?: {
    respect_git_ignore?: boolean;
    respect_zero_ignore?: boolean;
  };
}
 ...
 * This combines the shared patterns with dynamic patterns like ZERO.md.
...
${finalExclusionPatternsForDescription
  ...
  async execute({ abortSignal: signal }: ExecuteOptions): Promise<ToolResult> {
    ...
    try {
      ...
      const { filteredPaths, ignoredCount } =
        fileDiscovery.filterFilesWithReport(relativeEntries, {
          ...
          respectZEROIgnore:
            this.params.file_filtering_options?.respect_zero_ignore ??
            this.config.getFileFilteringOptions().respectZEROIgnore ??
    ...
    // one, preventing shared parent ZERO.md files from being injected twice.
