...
async function getDocsRoot(): Promise<string> {
  ...
  throw new Error('Could not find ZERO CLI documentation directory.');
}
  ...
  async execute({ abortSignal: _signal }: ExecuteOptions): Promise<ToolResult> {
    try {
      ...
      if (!this.params.path) {
        ...
        const resultContent = `Available ZERO CLI documentation files:\n\n${fileList}`;
...
/**
 * A tool that provides access to ZERO CLI's internal documentation.
