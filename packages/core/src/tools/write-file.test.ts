...
let mockZEROClientInstance: Mocked<ZEROClient>;
...
const mockConfigInternal = {
  ...
  getZEROClient: vi.fn(), // Initialize as a plain mock function
  ...
  getUserMemory: () => '',
  setUserMemory: vi.fn(),
  getZEROMdFileCount: () => 0,
  setZEROMdFileCount: vi.fn(),
  getToolRegistry: () =>
    ({
      registerTool: vi.fn(),
      discoverTools: vi.fn(),
...
describe('WriteFileTool', () => {
  ...
  beforeEach(() => {
    ...
    const rawRootDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'zero-cli-test-root-'),
    );
    rootDir = fs.realpathSync(rawRootDir);

    const rawPlansDir = fs.mkdtempSync(
      path.join(os.tmpdir(), 'zero-cli-test-plans-'),
    );
    ...
    // Setup ZEROClient mock
    mockZEROClientInstance = new (vi.mocked(ZEROClient))(
      mockConfig,
    ) as Mocked<ZEROClient>;
    vi.mocked(ZEROClient).mockImplementation(() => mockZEROClientInstance);
    ...
    mockConfigInternal.getZEROClient.mockReturnValue(
      mockZEROClientInstance,
    );
  ...
  describe('getCorrectedFileContent', () => {
    ...
    it('should set aggressiveUnescape to false for zero-3 models', async () => {
      const filePath = path.join(rootDir, 'zero3_file.txt');
      const proposedContent = 'Proposed new content.';
      const abortSignal = new AbortController().signal;

      const mockZERO3Config = {
        // eslint-disable-next-line @typescript-eslint/no-misused-spread
        ...mockConfig,
        getActiveModel: () => 'zero-3.0-pro',
      } as unknown as Config;
