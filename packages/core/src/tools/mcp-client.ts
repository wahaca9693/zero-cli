...
import {
  ZERO_CLI_IDENTIFICATION_ENV_VAR,
  ZERO_CLI_IDENTIFICATION_ENV_VAR_VALUE,
...
export async function createTransport(
  ...
  if (mcpServerConfig.command) {
    if (!cliConfig.isTrustedFolder()) {
      throw new Error(
        `MCP server '${mcpServerName}' uses stdio transport but current folder is not trusted. Use 'zero trust' to enable it.`,
      );
    }
    const extensionEnv = getExtensionEnvironment(mcpServerConfig.extension);
    const expansionEnv = { ...process.env, ...extensionEnv };
