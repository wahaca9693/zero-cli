...
> {
  ...
  private async performRipgrepSearch(options: {
    ...
    if (!no_ignore) {
      ...
      // Add .zeroignore and custom ignore files support (if provided/mandated)
      // (ripgrep natively handles .gitignore)
      const zeroIgnorePaths = this.fileDiscoveryService.getIgnoreFilePaths();
      for (const ignorePath of zeroIgnorePaths) {
