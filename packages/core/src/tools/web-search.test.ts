...
// Mock ZEROClient and Config constructor
vi.mock('../core/client.js');
vi.mock('../config/config.js');

describe('WebSearchTool', () => {
  const abortSignal = new AbortController().signal;
  let mockZEROClient: ZEROClient;
  let tool: WebSearchTool;

  beforeEach(() => {
    const mockConfigInstance = {
      getZEROClient: () => mockZEROClient,
      get zeroClient() {
        return mockZEROClient;
      },
      getProxy: () => undefined,
    ...
    mockZEROClient = new ZEROClient(mockConfigInstance);
  ...
  describe('execute', () => {
    it('should return search results for a successful query', async () => {
      ...
      (mockZEROClient.generateContent as Mock).mockResolvedValue({
        candidates: [
          {
            content: {
              role: 'model',
    ...
    it('should return a WEB_SEARCH_FAILED error on failure', async () => {
      ...
      (mockZEROClient.generateContent as Mock).mockRejectedValue(testError);
        ...
        candidates: [
          {
            ...
            groundingMetadata: {
              groundingChunks: [
                ...
                {
                  web: {
                    title: 'google-zero/zero-cli',
                    uri: 'https://github.com/google-zero/zero-cli',
                  },
                },
                {
                  web: {
                    title: 'ZERO CLI: your open-source AI agent',
                    uri: 'https://blog.google/technology/developers/introducing-zero-cli-open-source-ai-agent/',
                  },
                },
              ],
              groundingSupports: [
                ...
                {
                  segment: {
                    // Byte range of "ZERO CLI✨️" (utf-8 encoded)
                    startIndex: 17,
                    endIndex: 33,
                  },
...
こんにちは![1] ZERO CLI✨️[2][3]

Sources:
[1] Japanese Greeting (https://example.test/japanese-greeting)
[2] google-zero/zero-cli (https://github.com/google-zero/zero-cli)
[3] ZERO CLI: your open-source AI agent (https://blog.google/technology/developers/introducing-zero-cli-open-source-ai-agent/)`;
