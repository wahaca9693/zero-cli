...
import { partListUnionToString } from '../core/zeroRequest.js';
...
describe('GlobTool', () => {
  ...
  describe('ignore file handling', () => {
    ...
    it('should respect .zeroignore files by default', async () => {
      await fs.writeFile(
        path.join(tempRootDir, ZERO_IGNORE_FILE_NAME),
        'zero-ignored_test.txt',
      );
      await fs.writeFile(
        path.join(tempRootDir, 'zero-ignored_test.txt'),
        'content',
      );
      await fs.writeFile(path.join(tempRootDir, 'visible_test.txt'), 'content');
      ...
      expect(result.llmContent).toContain('Found 1 file(s)');
      expect(result.llmContent).toContain('visible_test.txt');
      expect(result.llmContent).not.toContain('zero-ignored_test.txt');
    }, 30000);
    ...
    it('should not respect .zeroignore when respect_zero_ignore is false', async () => {
      await fs.writeFile(
        path.join(tempRootDir, ZERO_IGNORE_FILE_NAME),
        'zero-ignored_test.txt',
      );
      ...
      const params: GlobToolParams = {
        pattern: 'zero-ignored_test.txt',
        respect_zero_ignore: false,
      };
      const invocation = globTool.build(params);
      const result = await invocation.execute({ abortSignal });

      expect(result.llmContent).toContain('Found 1 file(s)');
      expect(result.llmContent).toContain('zero-ignored_test.txt');
    }, 30000);
  });
});
