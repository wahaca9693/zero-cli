...
import { ZEROChat, StreamEventType } from '../core/zeroChat.js';
import {
  type Content,
  type Part,
  type FunctionCall,
...
import { DEFAULT_ZERO_MODEL, isAutoModel } from '../config/models.js';
...
export class LocalAgentExecutor<TOutput extends z.ZodTypeAny> {
  ...
  private get executionContext(): AgentLoopContext {
    return {
      ...
      zeroClient: this.context.zeroClient,
  ...
  private async tryCompressChat(
    ...
    const model = this.definition.modelConfig.model ?? DEFAULT_ZERO_MODEL;
  ...
  /** Initializes a `ZEROChat` instance for the agent run. */
  private async createChatObject(
    ...
    try {
      ...
      await reportError(
        e,
        `Error initializing ZERO chat for agent ${this.definition.name}.`,
        startHistory,
        'startChat',
      );
  ...
  private async processFunctionCalls(
    ...
    // Ensure exactly one response per function call to satisfy the ZERO API protocol.
