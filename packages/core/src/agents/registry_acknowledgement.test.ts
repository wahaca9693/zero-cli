...
describe('AgentRegistry Acknowledgement', () => {
  ...
  let config: Config;
  let tempDir: string;
  let originalZEROCliHome: string | undefined;
  let ackService: AcknowledgedAgentsService;

  beforeEach(async () => {
    // Create a unique temp directory for each test
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), 'zero-cli-test-'));

    // Override ZERO_CLI_HOME to point to the temp directory
    originalZEROCliHome = process.env['ZERO_CLI_HOME'];
    process.env['ZERO_CLI_HOME'] = tempDir;
  ...
  afterEach(async () => {
    ...
    if (originalZEROCliHome) {
      process.env['ZERO_CLI_HOME'] = originalZEROCliHome;
    } else {
