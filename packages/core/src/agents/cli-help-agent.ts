...
import { ZERO_MODEL_ALIAS_FLASH } from '../config/models.js';
import { z } from 'zod';
...
const CliHelpReportSchema = z.object({
  answer: z
    .string()
    .describe('The detailed answer to the user question about ZERO CLI.'),
  sources: z
    .array(z.string())
...
/**
 * An agent specialized in answering questions about ZERO CLI itself,
...
export const CliHelpAgent = (
  ...
  description:
    'Specialized agent for answering questions about the ZERO CLI application. Invoke this agent for questions regarding CLI features, configuration schemas (e.g., policies), or instructions on how to create custom subagents. It queries internal documentation to provide accurate usage guidance.',
  inputConfig: {
    inputSchema: {
      type: 'object',
      properties: {
        question: {
          type: 'string',
          description: 'The specific question about ZERO CLI.',
        },
      },
      required: ['question'],
    },
  ...
  promptConfig: {
    query:
      'Your task is to answer the following question about ZERO CLI:\n' +
      '<question>\n' +
      '${question}\n' +
      '</question>',
    systemPrompt:
      "You are **CLI Help Agent**, an expert on ZERO CLI. Your purpose is to provide accurate information about ZERO CLI's features, configuration, and current state.\n\n" +
      '### Runtime Context\n' +
