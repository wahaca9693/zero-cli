...
import { SecurityScannerAgent } from './security-scanner-agent.js';
import { PerformanceAnalyzerAgent } from './performance-analyzer-agent.js';
import { TelegramAgent } from './telegram-agent.js';
// New Advanced Agents
import { MultimodalAgent } from './multimodal-agent.js';
import { VoiceAgent } from './voice-agent.js';
import { DocumentAnalyzerAgent } from './document-analyzer-agent.js';
import { DataAnalystAgent } from './data-analyst-agent.js';
import { TestGeneratorAgent } from './test-generator-agent.js';
import { APIDesignerAgent } from './api-designer-agent.js';
import { DevOpsAgent } from './devops-agent.js';
// Additional Expert Agents
import { ArchitectAgent } from './architect-agent.js';
import { DatabaseExpertAgent } from './database-expert-agent.js';
...
export class AgentRegistry {
  ...
  private async loadAgents(errors?: string[]): Promise<void> {
    ...
    // Load project-level agents: .zero/agents/ (relative to Project Root)
    ...
    // Load user-level agents: ~/.zero/agents/
  ...
  private loadBuiltInAgents(): void {
    ...
    // Register Enhanced Custom Agents (Original)
    this.registerLocalAgent(CodeReviewerAgent(this.config));
    this.registerLocalAgent(SecurityScannerAgent(this.config));
    this.registerLocalAgent(PerformanceAnalyzerAgent(this.config));
    this.registerLocalAgent(TelegramAgent(this.config));

    // Register New Advanced Agents
    this.registerLocalAgent(MultimodalAgent(this.config));
    this.registerLocalAgent(VoiceAgent(this.config));
    this.registerLocalAgent(DocumentAnalyzerAgent(this.config));
    this.registerLocalAgent(DataAnalystAgent(this.config));
    this.registerLocalAgent(TestGeneratorAgent(this.config));
    this.registerLocalAgent(APIDesignerAgent(this.config));
    this.registerLocalAgent(DevOpsAgent(this.config));
    // Register Additional Expert Agents
    this.registerLocalAgent(ArchitectAgent(this.config));
    this.registerLocalAgent(DatabaseExpertAgent(this.config));
    this.registerLocalAgent(RefactorerAgent(this.config));
