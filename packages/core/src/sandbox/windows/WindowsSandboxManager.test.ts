...
describe('WindowsSandboxManager', () => {
  ...
  function createTempDir(name: string, parent = os.tmpdir()): string {
    const rawPath = fs.mkdtempSync(path.join(parent, `zero-test-${name}-`));
  ...
  it('should prepare a ZEROSandbox.exe command', async () => {
    ...
    expect(result.program).toContain('ZEROSandbox.exe');
  ...
  it('should ensure governance files exist', async () => {
    ...
    expect(fs.existsSync(path.join(testCwd, '.zeroignore'))).toBe(true);
