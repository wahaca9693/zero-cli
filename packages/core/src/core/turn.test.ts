...
import {
  Turn,
  ZEROEventType,
  type ServerZEROToolCallRequestEvent,
  type ServerZEROErrorEvent,
} from './turn.js';
...
describe('Turn', () => {
  ...
  beforeEach(() => {
    ...
    turn = new Turn(mockChatInstance as unknown as ZEROChat, 'prompt-id-1');
  ...
  describe('run', () => {
    ...
    it('should yield tool_call_request events for function calls', async () => {
      ...
      const event1 = events[0] as ServerZEROToolCallRequestEvent;
      expect(event1.type).toBe(ZEROEventType.ToolCallRequest);
      ...
      const event2 = events[1] as ServerZEROToolCallRequestEvent;
      expect(event2.type).toBe(ZEROEventType.ToolCallRequest);
    ...
    it('should yield UserCancelled event if signal is aborted', async () => {
      ...
      expect(events).toEqual([
        { type: ZEROEventType.Content, value: 'First part' },
    ...
    it('should yield InvalidStream event if sendMessageStream throws InvalidStreamError', async () => {
      ...
      expect(events).toEqual([{ type: ZEROEventType.InvalidStream }]);
    ...
    it('should yield Error event and report if sendMessageStream throws', async () => {
      ...
      const errorEvent = events[0] as ServerZEROErrorEvent;
      expect(errorEvent.type).toBe(ZEROEventType.Error);
    ...
    it('should handle function calls with undefined name or args', async () => {
      ...
      const event3 = events[2] as ServerZEROToolCallRequestEvent;
    ...
    it.each([
      ...
      expect(events).toEqual([
        { type: ZEROEventType.Content, value: contentText },
        {
    ...
    it('should handle multiple responses with different finish reasons', async () => {
      ...
      expect(events).toEqual([
        { type: ZEROEventType.Content, value: 'First part' },
        { type: ZEROEventType.Content, value: 'Second part' },
        {
          type: ZEROEventType.Finished,
    ...
    it('should yield citation and finished events when response has citationMetadata', async () => {
      ...
      expect(events).toEqual([
        { type: ZEROEventType.Content, value: 'Some text.' },
        {
    ...
    it('should not yield citation event if there is no finish reason', async () => {
      ...
      expect(events.some((e) => e.type === ZEROEventType.Citation)).toBe(
        false,
      );
    });
    ...
    it('should yield a Retry event when it receives one from the chat stream', async () => {
      ...
      expect(events).toEqual([
        ...
        { type: ZEROEventType.Content, value: 'Success' },
      ]);
    });
    ...
    it('should process all parts when thought is first part in chunk', async () => {
      ...
      const thoughtEvent = events.find(
        (e) => e.type === ZEROEventType.Thought,
      );
      ...
      const contentEvent = events.find(
        (e) => e.type === ZEROEventType.Content,
      );
      ...
      const toolCallEvent = events.find(
        (e) => e.type === ZEROEventType.ToolCallRequest,
      );
      ...
      const citationEvent = events.find(
        (e) => e.type === ZEROEventType.Citation,
      );
      ...
      const finishedEvent = events.find(
        (e) => e.type === ZEROEventType.Finished,
      );
