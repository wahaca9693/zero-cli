...
describe('ZEROChat', () => {
  ...
  beforeEach(() => {
    ...
    let currentModel = 'zero-pro';
    let currentActiveModel = 'zero-pro';
    ...
    mockConfig = {
      ...
      hasZERO35FlashGAAccess: vi.fn().mockReturnValue(false),
      getDebugMode: () => false,
      ...
      modelConfigService: {
        getResolvedConfig: vi.fn().mockImplementation((modelConfigKey) => {
          ...
          const thinkingConfig = model.startsWith('zero-3')
            ? {
  ...
  describe('constructor', () => {
    it('should initialize lastPromptTokenCount based on history size', () => {
      ...
      const chatWithHistory = new ZEROChat(mockConfig, '', [], history);
    ...
    it('should initialize lastPromptTokenCount for empty history', () => {
      const chatEmpty = new ZEROChat(mockConfig);
  ...
  describe('setHistory', () => {
    it('should recalculate lastPromptTokenCount when history is updated', () => {
      ...
      const chatWithHistory = new ZEROChat(
        mockConfig,
        '',
        [],
        initialHistory,
  ...
  describe('sendMessageStream', () => {
    ...
    it('should use thinkingLevel and remove thinkingBudget for zero-3 models', async () => {
      const response = (async function* () {
        yield {
          candidates: [
            {
      ...
      const stream = await chat.sendMessageStream(
        { model: 'zero-3-test-only-model-string-for-testing' },
        'hello',
    ...
    it('should use thinkingBudget and remove thinkingLevel for non-zero-3 models', async () => {
      const response = (async function* () {
        yield {
          candidates: [
            {
    ...
    it('should flush transcript before tool dispatch for pure tool call with no text or thoughts', async () => {
      ...
      const zeroWrite = newWrites.find((w) => {
        try {
          const data = JSON.parse(w[1] as string);
          return data.type === 'zero';
        } catch {
          return false;
        }
      });

      expect(zeroWrite).toBeDefined();
    });
  });
  ...
  describe('ensureActiveLoopHasThoughtSignatures', () => {
    it('should add thoughtSignature to the first functionCall in each model turn of the active loop', () => {
      const chat = new ZEROChat(mockConfig, '', [], []);
  ...
  describe('Availability Service Integration', () => {
    ...
    it('should pass attempted model to onPersistent429 callback which calls handleFallback', async () => {
      ...
      // handleFallback is called with the ATTEMPTED model (model-a), not the requested one (zero-pro)
      expect(mockHandleFallback).toHaveBeenCalledWith(
        expect.anything(),
        'model-a',
        expect.anything(),
