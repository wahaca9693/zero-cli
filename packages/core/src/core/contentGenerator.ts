...
import type { UserTierId, ZEROUserTier } from '../code_assist/types.js';
...
export interface ContentGenerator {
  ...
  paidTier?: ZEROUserTier;
}

export enum AuthType {
  LOGIN_WITH_GOOGLE = 'oauth-personal',
  USE_ZERO = 'zero-api-key',
...
export async function createContentGeneratorConfig(
  ...
  if (authType === AuthType.USE_ZERO && zeroApiKey) {
    contentGeneratorConfig.apiKey = zeroApiKey;
...
export async function createContentGenerator(
  ...
  const generator = await (async () => {
    ...
    const model = resolveModel(
      gcConfig.getModel(),
      config.authType === AuthType.USE_ZERO ||
        config.authType === AuthType.USE_VERTEX_AI ||
        ((await gcConfig.getZERO31Launched?.()) ?? false),
      false,
      gcConfig.getHasAccessToPreviewModel?.() ?? true,
      gcConfig,
      gcConfig.hasZERO35FlashGAAccess?.() ?? false,
    );
    const customHeadersEnv =
      process.env['ZERO_CLI_CUSTOM_HEADERS'] || undefined;
    ...
    if (clientName === 'acp-vscode' || surface === 'vscode') {
      ...
      userAgent = `CloudCodeVSCode/${version} (aidev_client; os_type=${osType}; os_version=${osVersion}; arch=${arch}; host_path=${hostPath}; proxy_client=zerocli)`;
    } else {
    ...
    const apiKeyAuthMechanism =
      process.env['ZERO_API_KEY_AUTH_MECHANISM'] || 'x-goog-api-key';
    ...
    if (
      ...
      if (gcConfig?.getUsageStatisticsEnabled()) {
        ...
        headers = {
          ...headers,
          'x-zero-api-privileged-user-id': `${installationId}`,
        };
      }
      ...
      if (!baseUrl) {
        const envBaseUrl =
          config.authType === AuthType.USE_VERTEX_AI
            ...
            : process.env['GOOGLE_ZERO_BASE_URL'];
        if (envBaseUrl) {
