...
export class ZEROChat {
  ...
  constructor(
    ...
    if (history.length > 0) {
      ...
      initialHistory = resumedSessionData.conversation.messages
        .filter((m) => m.type === 'user' || m.type === 'zero')
        .map((m) => ({
          id: m.id,
          content: {
   ...
   * @example
   * ```ts
   * const chat = ai.chats.create({model: 'zero-2.0-flash'});
  ...
  private async makeApiCallAndProcessStream(
    ...
    // before sending to the ZERO API. This whitelists only standard ZERO fields.
    ...
    const apiCall = async () => {
      const useZERO3_1 =
        (await this.context.config.getZERO31Launched?.()) ?? false;
      ...
      let modelToUse = resolveModel(
        ...
        hasAccessToPreview,
        this.context.config,
        this.context.config.hasZERO35FlashGAAccess?.() ?? false,
      );
   ...
   * This is called by external components when tool calls complete, before sending responses to ZERO.
   */
