...
import { partListUnionToString } from './zeroRequest.js';
import {
...
export class ZEROClient {
  ...
  async startChat(
    ...
    try {
      ...
      await reportError(
        error,
        'Error initializing ZERO chat session.',
        [...history],
        'startChat',
      );
  ...
  private _getActiveModelForCurrentTurn(): string {
    ...
    return resolveModel(
      ...
      this.config.getZERO31LaunchedSync?.() ?? false,
      false,
      this.config.getHasAccessToPreviewModel?.() ?? true,
      this.config,
      this.config.hasZERO35FlashGAAccess?.() ?? false,
    );
  }
  ...
  private async *processTurn(
  ...
  ): AsyncGenerator<ServerZEROStreamEvent, Turn> {
    ...
    if (
      ...
      yield { type: ZEROEventType.MaxSessionTurns };
      return turn;
    }
    ...
    if (this.config.getContextManagementConfig().enabled) {
      ...
      if (compressed.compressionStatus === CompressionStatus.COMPRESSED) {
        yield { type: ZEROEventType.ChatCompressed, value: compressed };
      }
    }
    ...
    // waiting for a response. The ZERO API requires that a functionResponse
    ...
    if (loopResult.count > 1) {
      yield { type: ZEROEventType.LoopDetected };
      return turn;
    ...
    if (!signal.aborted && !this.currentSequenceModel) {
      yield { type: ZEROEventType.ModelInfo, value: modelToUse };
    }
    ...
    for await (const event of resultStream) {
      ...
      if (event.type === ZEROEventType.Finished && this.contextManager) {
      ...
      this.updateTelemetryTokenCount();
      if (event.type === ZEROEventType.Error) {
        isError = true;
      }
    }
  ...
  async *sendMessageStream(
    ...
    if (hooksEnabled && messageBus) {
      ...
      if (hookResult) {
        if (
          'type' in hookResult &&
          hookResult.type === ZEROEventType.AgentExecutionStopped
        ) {
          ...
          'type' in hookResult &&
          hookResult.type === ZEROEventType.AgentExecutionBlocked
        ) {
          yield hookResult;
          return new Turn(this.getChat(), prompt_id);
