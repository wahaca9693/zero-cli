...
export class Logger {
  private zeroDir: string | undefined;
  ...
  async initialize(): Promise<void> {
    ...
    this.zeroDir = this.storage.getProjectTempDir();
    this.logFilePath = path.join(this.zeroDir, LOG_FILE_NAME);

    try {
      await fs.mkdir(this.zeroDir, { recursive: true });
  ...
  private _checkpointPath(tag: string): string {
    ...
    return path.join(this.zeroDir, `checkpoint-${encodedTag}.json`);
  }
  ...
  private async _getCheckpointPath(tag: string): Promise<string> {
    ...
    const oldPath = path.join(this.zeroDir!, `checkpoint-${tag}.json`);
    try {
  ...
  async deleteCheckpoint(tag: string): Promise<boolean> {
    if (!this.initialized || !this.zeroDir) {
      debugLogger.error(
