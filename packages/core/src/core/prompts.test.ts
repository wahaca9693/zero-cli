...
import { ZERO_DIR } from '../utils/paths.js';
...
describe('Core System Prompt (prompts.ts)', () => {
  ...
  beforeEach(() => {
    ...
    vi.stubEnv('ZERO_SYSTEM_MD', undefined);
    vi.stubEnv('ZERO_WRITE_SYSTEM_MD', undefined);
    const mockRegistry = {
      getAllToolNames: vi
        .fn()
    ...
    mockConfig = {
      ...
      storage: {
        ...
        getProjectTempTrackerDir: vi
          .fn()
          .mockReturnValue('/mock/.zero/tmp/session/tracker'),
      },
      ...
      getActiveModel: vi.fn().mockReturnValue(DEFAULT_ZERO_MODEL),
      getMessageBus: vi.fn(),
  ...
  it('should include available_skills with updated verbiage for preview models', () => {
    vi.mocked(mockConfig.getActiveModel).mockReturnValue(PREVIEW_ZERO_MODEL);
    const skills = [
      {
        name: 'test-skill',
        description: 'A test skill description',
  ...
  it('should use legacy system prompt for non-preview model', () => {
    vi.mocked(mockConfig.getActiveModel).mockReturnValue(DEFAULT_ZERO_MODEL);
    const prompt = getCoreSystemPrompt(mockConfig);
    expect(prompt).toContain(
  ...
  it('should use chatty system prompt for preview model', () => {
    vi.mocked(mockConfig.getActiveModel).mockReturnValue(PREVIEW_ZERO_MODEL);
    const prompt = getCoreSystemPrompt(mockConfig);
    expect(prompt).toContain('You are ZERO CLI, an interactive CLI agent'); // Check for core content
    expect(prompt).toContain('- **User Hints:**');
  ...
  it('should append userMemory with separator when provided', () => {
    ...
    expect(prompt).toContain('# Contextual Instructions (ZERO.md)');
    expect(prompt).toContain('<loaded_context>');
    expect(prompt).toContain(memory);
    expect(prompt).toContain('You are ZERO CLI, an interactive CLI agent'); // Ensure base prompt follows
  ...
  it.each([
    ...
    [
      undefined,
      'You are ZERO CLI, an interactive CLI agent',
  ...
  it.each([
    ...
    (enableCodebaseInvestigator, expectCodebaseInvestigator) => {
      ...
      const testConfig = {
        ...
        getActiveModel: vi.fn().mockReturnValue(PREVIEW_ZERO_MODEL),
        getPreviewFeatures: vi.fn().mockReturnValue(true),
        getAgentRegistry: vi.fn().mockReturnValue({
          getDirectoryContext: vi.fn().mockReturnValue('Mock Agent Directory'),
          getAllDefinitions: vi.fn().mockReturnValue([]),
  ...
  describe('ZERO_SYSTEM_MD environment variable', () => {
    it.each(['false', '0'])(
      'should use default prompt when ZERO_SYSTEM_MD is "%s"',
      (value) => {
        vi.stubEnv('ZERO_SYSTEM_MD', value);
        const prompt = getCoreSystemPrompt(mockConfig);
    ...
    it('should throw error if ZERO_SYSTEM_MD points to a non-existent file', () => {
      const customPath = '/non/existent/path/system.md';
      vi.stubEnv('ZERO_SYSTEM_MD', customPath);
    ...
    it.each(['true', '1'])(
      'should read from default path when ZERO_SYSTEM_MD is "%s"',
      (value) => {
        const defaultPath = path.resolve(path.join(ZERO_DIR, 'system.md'));
        vi.stubEnv('ZERO_SYSTEM_MD', value);
    ...
    it('should read from custom path when ZERO_SYSTEM_MD provides one, preserving case', () => {
    ...
    it('should expand tilde in custom path when ZERO_SYSTEM_MD is set', () => {
  ...
  describe('ZERO_WRITE_SYSTEM_MD environment variable', () => {
    it.each(['false', '0'])(
      'should not write to file when ZERO_WRITE_SYSTEM_MD is "%s"',
      (value) => {
        vi.stubEnv('ZERO_WRITE_SYSTEM_MD', value);
    ...
    it.each(['true', '1'])(
      'should write to default path when ZERO_WRITE_SYSTEM_MD is "%s"',
      (value) => {
        const defaultPath = path.resolve(path.join(ZERO_DIR, 'system.md'));
        vi.stubEnv('ZERO_WRITE_SYSTEM_MD', value);
        getCoreSystemPrompt(mockConfig);
    ...
    it('should write to custom path when ZERO_WRITE_SYSTEM_MD provides one', () => {
      const customPath = path.resolve('/custom/path/system.md');
      vi.stubEnv('ZERO_WRITE_SYSTEM_MD', customPath);
      getCoreSystemPrompt(mockConfig);
      expect(fs.writeFileSync).toHaveBeenCalledWith(
        customPath,
        expect.any(String),
    ...
    it.each([
      ...
      'should expand tilde in custom path when ZERO_WRITE_SYSTEM_MD is "%s"',
