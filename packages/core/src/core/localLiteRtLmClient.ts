...
/**
 * A client for making single, non-streaming calls to a local ZERO-compatible API
...
export class LocalLiteRtLmClient {
  ...
  /**
   * Sends a prompt to the local ZERO model and expects a JSON object in response.
  ...
  async generateJson(
    ...
    const zeroContents = contents.map((c) => ({
      role: c.role,
      parts: c.parts ? c.parts.map((p) => ({ text: p.text })) : [],
    }));

    if (reminder) {
      const lastContent = zeroContents.at(-1);
    ...
    try {
      ...
      if (!text) {
        throw new Error(
          'Invalid response from Local ZERO API: No text found',
        );
      }
