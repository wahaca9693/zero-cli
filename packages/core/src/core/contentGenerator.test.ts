...
const mockConfig = {
  getModel: vi.fn().mockReturnValue('zero-pro'),
...
describe('getAuthTypeFromEnv', () => {
  ...
  it('should detect GATEWAY when GOOGLE_ZERO_BASE_URL is present', () => {
    vi.stubEnv('GOOGLE_ZERO_BASE_URL', 'https://gateway.example.com');
    expect(getAuthTypeFromEnv()).toBe(AuthType.GATEWAY);
  });

  it('should detect USE_ZERO when ZERO_API_KEY is present', () => {
    vi.stubEnv('ZERO_API_KEY', 'fake-key');
    expect(getAuthTypeFromEnv()).toBe(AuthType.USE_ZERO);
  });
...
describe('createContentGenerator', () => {
      ...
      httpOptions: expect.objectContaining({
        headers: expect.objectContaining({
          'User-Agent': expect.stringMatching(
            /ZEROCLI\/1\.2\.3\/zero-pro \(.*; .*; terminal\)/,
          ),
        }),
      }),
    });
  ...
  it('should use standard User-Agent for a2a-server running outside VS Code', async () => {
    ...
    await createContentGenerator(
      { apiKey: 'test-api-key', authType: AuthType.USE_ZERO },
      mockConfig,
      undefined,
    );
    ...
    expect(GoogleGenAI).toHaveBeenCalledWith(
      expect.objectContaining({
        httpOptions: expect.objectContaining({
          headers: expect.objectContaining({
            'User-Agent': expect.stringMatching(
              /ZEROCLI-a2a-server\/1\.2\.3\/zero-pro \(.*; .*; terminal\)/,
            ),
          }),
        }),
      }),
    ...
    expect(GoogleGenAI).toHaveBeenCalledWith(
      expect.objectContaining({
        httpOptions: expect.objectContaining({
          headers: expect.objectContaining({
            'User-Agent': expect.stringMatching(
              /CloudCodeVSCode\/1\.2\.3 \(aidev_client; os_type=.*; os_version=.*; arch=.*; host_path=VSCode\/1\.85\.0; proxy_client=zerocli\)/,
            ),
          }),
        }),
      }),
    ...
    expect(GoogleGenAI).toHaveBeenCalledWith(
      expect.objectContaining({
        httpOptions: expect.objectContaining({
          headers: expect.objectContaining({
            'User-Agent': expect.stringMatching(
              /ZEROCLI-my-client\/1\.2\.3\/zero-pro \(.*; .*; terminal\)/,
            ),
          }),
        }),
      }),
  ...
  it('should allow custom headers to override User-Agent', async () => {
    ...
    vi.stubEnv('ZERO_CLI_CUSTOM_HEADERS', 'User-Agent:MyCustomUA');
  ...
  it('should include custom headers from ZERO_CLI_CUSTOM_HEADERS for Code Assist requests', async () => {
    const mockGenerator = {} as unknown as ContentGenerator;
    vi.mocked(createCodeAssistContentGenerator).mockResolvedValue(
      mockGenerator as never,
    );
    vi.stubEnv(
      'ZERO_CLI_CUSTOM_HEADERS',
  ...
  it('should include custom headers from ZERO_CLI_CUSTOM_HEADERS for GoogleGenAI requests without inferring auth mechanism', async () => {
    const mockConfig = {
      getModel: vi.fn().mockReturnValue('zero-pro'),
      getProxy: vi.fn().mockReturnValue(undefined),
  ...
  it('should pass api key as Authorization Header when ZERO_API_KEY_AUTH_MECHANISM is set to bearer', async () => {
    const mockConfig = {
      getModel: vi.fn().mockReturnValue('zero-pro'),
      getProxy: vi.fn().mockReturnValue(undefined),
      getUsageStatisticsEnabled: () => false,
    ...
    vi.mocked(GoogleGenAI).mockImplementation(() => mockGenerator as never);
    vi.stubEnv('ZERO_API_KEY_AUTH_MECHANISM', 'bearer');
  ...
  it('should not pass api key as Authorization Header when ZERO_API_KEY_AUTH_MECHANISM is not set (default behavior)', async () => {
    const mockConfig = {
      getModel: vi.fn().mockReturnValue('zero-pro'),
      getProxy: vi.fn().mockReturnValue(undefined),
      getUsageStatisticsEnabled: () => false,
    ...
    vi.mocked(GoogleGenAI).mockImplementation(() => mockGenerator as never);
    // ZERO_API_KEY_AUTH_MECHANISM is not stubbed, so it will be undefined, triggering default 'x-goog-api-key'
  ...
  it('should pass baseUrl to GoogleGenAI when GOOGLE_ZERO_BASE_URL is set', async () => {
    const mockConfig = {
      getModel: vi.fn().mockReturnValue('zero-pro'),
      getProxy: vi.fn().mockReturnValue(undefined),
      getUsageStatisticsEnabled: () => false,
    ...
    vi.mocked(GoogleGenAI).mockImplementation(() => mockGenerator as never);
    vi.stubEnv('GOOGLE_ZERO_BASE_URL', 'https://zero.test.local');
  ...
  it('should prefer an explicit baseUrl over GOOGLE_ZERO_BASE_URL', async () => {
    const mockConfig = {
      getModel: vi.fn().mockReturnValue('zero-pro'),
      getProxy: vi.fn().mockReturnValue(undefined),
      getUsageStatisticsEnabled: () => false,
    ...
    vi.mocked(GoogleGenAI).mockImplementation(() => mockGenerator as never);
    vi.stubEnv('GOOGLE_ZERO_BASE_URL', 'https://env.test.local');
    vi.stubEnv('ZERO_API_KEY', 'test-api-key');
  ...
  it('should not apply model mapping for ZERO API', async () => {
    const mockModels = {
      generateContent: vi.fn().mockResolvedValue({}),
    };
    const mockGenerator = {
...
describe('createContentGeneratorConfig', () => {
  ...
  it('should configure for ZERO using ZERO_API_KEY when set', async () => {
    vi.stubEnv('ZERO_API_KEY', 'env-zero-key');
    const config = await createContentGeneratorConfig(
      mockConfig,
      AuthType.USE_ZERO,
    );
    expect(config.apiKey).toBe('env-zero-key');
    expect(config.vertexai).toBe(false);
  });

  it('should not configure for ZERO if ZERO_API_KEY is empty', async () => {
    vi.stubEnv('ZERO_API_KEY', '');
    const config = await createContentGeneratorConfig(
      mockConfig,
      AuthType.USE_ZERO,
  ...
  it('should not configure for ZERO if ZERO_API_KEY is not set and storage is empty', async () => {
    vi.stubEnv('ZERO_API_KEY', '');
  ...
  it('should configure for GATEWAY using ZERO_API_KEY from environment if set', async () => {
    vi.stubEnv('ZERO_API_KEY', 'env-gateway-key');
    const config = await createContentGeneratorConfig(
      mockConfig,
      AuthType.GATEWAY,
    );
