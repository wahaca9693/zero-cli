...
import { ZEROChat } from './zeroChat.js';
...
import { DEFAULT_ZERO_MODEL_AUTO } from '../config/models.js';
...
describe('ZERO Client (client.ts)', () => {
  ...
  beforeEach(async () => {
    ...
    // Because the ZEROClient constructor kicks off an async process (startChat)
    ...
    client = new ZEROClient(mockConfig as unknown as AgentLoopContext);
    await client.initialize();
    vi.mocked(mockConfig.getZEROClient).mockReturnValue(client);
    (mockConfig as unknown as { zeroClient: ZEROClient }).zeroClient =
      client;
  ...
  describe('startChat', () => {
    it('should include environment context when resuming a session', async () => {
      ...
      expect(history[0].parts?.[0]?.text).toContain('This is the ZERO CLI');
  ...
  describe('tryCompressChat', () => {
    ...
    function setup({
      ...
      const mockOriginalChat: Partial<ZEROChat> = {
      ...
      client['chat'] = mockOriginalChat as ZEROChat;
      ...
      const mockNewChat: Partial<ZEROChat> = {
      ...
      client['startChat'] = vi
        .fn()
        .mockResolvedValue(mockNewChat as ZEROChat);
  ...
  describe('sendMessageStream', () => {
    ...
    it('yields UserCancelled when processTurn throws AbortError', async () => {
      ...
      expect(events).toEqual([{ type: ZEROEventType.UserCancelled }]);
    });
...
\`\`\`
    ...
    it('should not add context if ideMode is enabled but no open files', async () => {
      ...
      const mockChat: Partial<ZEROChat> = {
        addHistory: vi.fn(),
        setTools: vi.fn(),
        getHistory: vi.fn().mockReturnValue([]),
        getLastPromptTokenCount: vi.fn(),
      };
      client['chat'] = mockChat as ZEROChat;
...
\`\`\`
    ...
    it('should cleanly abort and return Turn on LoopDetected without unhandled promise rejections', async () => {
      ...
      // Assert
      expect(events).toContainEqual({ type: ZEROEventType.LoopDetected });
    ...
    it('should yield MaxSessionTurns and stop when session turn limit is reached', async () => {
      ...
      expect(events).toEqual([{ type: ZEROEventType.MaxSessionTurns }]);
    ...
    it("should use the sticky model's token limit for the overflow check", async () => {
      // Arrange
      const STICKY_MODEL = 'zero-1.5-flash';
    ...
    it('should attempt compression before overflow check and proceed if compression frees space', async () => {
      ...
      // Use the real ZEROChat to manage state and token counts more realistically
    ...
    it('should handle massive function responses by truncating them and then yielding overflow warning', async () => {
      ...
      const realChat = new ZEROChat(mockConfig, '', [], history);
    ...
    describe('Model Routing', () => {
      ...
      it('should re-route within the same prompt when the configured model changes', async () => {
        ...
        vi.mocked(mockConfig.getModel).mockReturnValue('zero-2.5-flash');
        coreEvents.emitModelChanged('zero-2.5-flash');
    ...
    it('should propagate InvalidStream events without injecting "Please continue." or recursing', async () => {
      ...
      expect(events).toEqual([
        { type: ZEROEventType.ModelInfo, value: 'default-routed-model' },
    ...
    it('should not call checkNextSpeaker when turn.run() yields a value then an error', async () => {
      ...
      const mockStream = (async function* () {
        yield { type: ZEROEventType.Content, value: 'some content' };
        yield {
          type: ZEROEventType.Error,
          value: { error: { message: 'test error' } },
        };
    ...
    describe('Loop Recovery (Two-Strike)', () => {
      ...
      it('should trigger recovery (Strike 1) and continue', async () => {
        ...
        mockTurnRunFn.mockImplementation(() =>
          (async function* () {
            yield { type: ZEROEventType.Content, value: 'First event' };
            yield { type: ZEROEventType.Content, value: 'Second event' };
          })(),
        );
      ...
      it('should terminate (Strike 2) after recovery fails', async () => {
        ...
        mockTurnRunFn.mockImplementation(() =>
          (async function* () {
            yield { type: ZEROEventType.Content, value: 'Event' };
            yield { type: ZEROEventType.Content, value: 'Event' };
          })(),
        );
      ...
      it('should suppress LoopDetected event on Strike 1', async () => {
        ...
        mockTurnRunFn.mockImplementation(() =>
          (async function* () {
            yield { type: ZEROEventType.Content, value: 'Event' };
            yield { type: ZEROEventType.Content, value: 'Event 2' };
          })(),
        );
    ...
    describe('Hook System', () => {
      ...
      it('should fire BeforeAgent and AfterAgent exactly once for a simple turn', async () => {
        ...
        mockTurnRunFn.mockImplementation(async function* (
          ...
          yield { type: ZEROEventType.Content, value: 'Hook Response' };
        });
      ...
      it('should fire BeforeAgent once and AfterAgent once even with recursion', async () => {
        ...
        mockTurnRunFn.mockImplementation(async function* (
          ...
          yield { type: ZEROEventType.Content, value: response };
        });
      ...
      it('should use original request in AfterAgent hook even when continuation happened', async () => {
        ...
        mockTurnRunFn.mockImplementation(async function* (
          ...
          yield { type: ZEROEventType.Content, value: 'Ok' };
        });
      ...
      it('should stop execution in AfterAgent when hook returns continue: false', async () => {
        ...
        mockTurnRunFn.mockImplementation(async function* () {
          yield { type: ZEROEventType.Content, value: 'Hello' };
        });
