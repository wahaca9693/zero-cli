...
describe('ProjectRegistry', () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'zero-registry-test-'));
  ...
  it('handles collisions with unique suffixes', async () => {
    ...
    const id1 = await registry.getShortId(path.join(tempDir, 'one', 'zero'));
    const id2 = await registry.getShortId(path.join(tempDir, 'two', 'zero'));
    const id3 = await registry.getShortId(
      path.join(tempDir, 'three', 'zero'),
    );

    expect(id1).toBe('zero');
    expect(id2).toBe('zero-1');
    expect(id3).toBe('zero-2');
  });
  ...
  it('handles collisions if a slug is taken on disk by another project', async () => {
    // 1. project-y takes 'zero' on disk
    ...
    // 2. project-z tries to get shortId for 'zero'
    const registry = new ProjectRegistry(registryPath, [baseDir1]);
    await registry.initialize();
    const projectZ = normalizePath(path.join(tempDir, 'zero'));
    const shortId = await registry.getShortId(projectZ);

    // 3. It should avoid 'zero' and pick 'zero-1' (or similar)
    expect(shortId).not.toBe('zero');
    expect(shortId).toBe('zero-1');
  });
