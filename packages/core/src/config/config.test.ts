...
import { setZEROMdFilename as mockSetZEROMdFilename } from '../tools/memoryTool.js';
import {
...
import { ZEROClient } from '../core/client.js';
...
vi.mock('../tools/memoryTool', async (importOriginal) => {
  ...
  return {
    ...actual,
    setZEROMdFilename: vi.fn(),
    getCurrentZEROMdFilename: vi.fn(() => 'ZERO.md'),
  };
});
...
vi.mock('../core/client.js', () => ({
  ZEROClient: vi.fn().mockImplementation(() => ({
...
describe('Server Config (config.ts)', () => {
  const MODEL = DEFAULT_ZERO_MODEL;
  const SANDBOX: SandboxConfig = createMockSandboxConfig({
    command: 'docker',
    image: 'zero-cli-sandbox',
  });
  ...
  const EMBEDDING_MODEL = 'zero-embedding';
  ...
  describe('initialize', () => {
    ...
    describe('getZERO31LaunchedSync', () => {
      it.each([AuthType.USE_ZERO, AuthType.USE_VERTEX_AI, AuthType.GATEWAY])(
        ...
        async (authType) => {
          ...
          expect(config.getZERO31LaunchedSync()).toBe(true);
        },
      );
      ...
      it('should fallback to experiments for other auth types', async () => {
        vi.mocked(getExperiments).mockResolvedValue({
          experimentIds: [],
          flags: {
            [ExperimentFlags.ZERO_3_1_PRO_LAUNCHED]: {
              flagId: ExperimentFlags.ZERO_3_1_PRO_LAUNCHED,
              boolValue: true,
            },
          },
        });
    ...
    describe('getProModelNoAccessSync', () => {
      ...
      it('should return false for other auth types even if experiment is true', async () => {
        ...
        await config.refreshAuth(AuthType.USE_ZERO);
  ...
  describe('refreshAuth', () => {
    it('should refresh auth and update config', async () => {
      const config = new Config(baseParams);
      const authType = AuthType.USE_ZERO;
      ...
      expect(ZEROClient).toHaveBeenCalledWith(config);
    });
    ...
    it('should strip thoughts when switching from GenAI to Vertex', async () => {
      ...
      expect(
        loopContext.zeroClient.stripThoughtsFromHistory,
    ...
    it('should switch to flash model if user has no Pro access and model is auto', async () => {
      ...
      await vi.waitFor(() => {
        expect(config.getModel()).toBe(PREVIEW_ZERO_FLASH_MODEL);
      });
    });
    ...
    it('should NOT switch to flash model if user has Pro access and model is auto', async () => {
      ...
      expect(config.getModel()).toBe(PREVIEW_ZERO_MODEL_AUTO);
    });
  });
  ...
  it('Config constructor should call setZEROMdFilename with contextFileName if provided', () => {
    ...
    expect(mockSetZEROMdFilename).toHaveBeenCalledWith(contextFileName);
  });

  it('Config constructor should not call setZEROMdFilename if contextFileName is not provided', () => {
    new Config(baseParams); // baseParams does not have contextFileName
    expect(mockSetZEROMdFilename).not.toHaveBeenCalled();
  });
...
describe('BaseLlmClient Lifecycle', () => {
  ...
  it('should successfully initialize BaseLlmClient after refreshAuth is called', async () => {
    const config = new Config(baseParams);
    const authType = AuthType.USE_ZERO;
    const mockContentConfig = { model: 'zero-flash', apiKey: 'test-key' };
...
describe('Config getHooks', () => {
  ...
  describe('setModel', () => {
    ...
    it('should call onModelChange when a new model is set and should persist', () => {
      ...
      config.setModel(DEFAULT_ZERO_MODEL, false);

      expect(onModelChange).toHaveBeenCalledWith(DEFAULT_ZERO_MODEL);
    });
    ...
    it('should NOT call onModelChange when a new model is temporary', () => {
      ...
      config.setModel(DEFAULT_ZERO_MODEL, true);
...
describe('Config Quota & Preview Model Access', () => {
  ...
  describe('refreshUserQuota', () => {
    ...
    it('should update hasAccessToPreviewModel to true if quota includes ZERO 3.1 preview model', async () => {
      mockCodeAssistServer.retrieveUserQuota.mockResolvedValue({
        buckets: [
          {
            modelId: 'zero-3.1-pro-preview',
            remainingAmount: '100',
            remainingFraction: 1.0,
          },
        ],
    ...
    it('should calculate pooled quota correctly for auto models', async () => {
      mockCodeAssistServer.retrieveUserQuota.mockResolvedValue({
        buckets: [
          {
            modelId: 'zero-2.5-pro',
            remainingAmount: '10',
            remainingFraction: 0.2,
          },
          {
            modelId: 'zero-2.5-flash',
            remainingAmount: '80',
    ...
    it('should derive quota from remainingFraction when remainingAmount is missing', async () => {
      mockCodeAssistServer.retrieveUserQuota.mockResolvedValue({
        buckets: [
          {
            modelId: 'zero-3-flash-preview',
            remainingFraction: 0.96,
          },
        ],
      });

      config.setModel('zero-3-flash-preview');
    ...
    it('should store quota from remainingFraction when remainingFraction is 0', async () => {
      ...
      config.setModel('zero-3-pro-preview');
      mockCoreEvents.emitQuotaChanged.mockClear();
      await config.refreshUserQuota();
...
describe('Config JIT Initialization', () => {
  ...
  beforeEach(() => {
    ...
    mockMemoryContextManager = {
      ...
      getUserProjectMemory: vi.fn().mockReturnValue(''),
      getLoadedPaths: vi.fn().mockReturnValue(new Set(['/path/to/ZERO.md'])),
  ...
  it('should initialize MemoryContextManager, load memory, and delegate to it', async () => {
    ...
    expect(config.getZEROMdFileCount()).toBe(1);
    expect(config.getZEROMdFilePaths()).toEqual(['/path/to/ZERO.md']);
  });

  describe('memory path access', () => {
    it('should NOT add the global ~/.zero directory to the workspace', async () => {
      // Memory does not broaden the workspace to include the global ~/.zero/
      // directory. Cross-project personal preferences are routed to
      // ~/.zero/ZERO.md via the surgical isPathAllowed allowlist instead.
      const params: ConfigParameters = {
        sessionId: 'test-session',
        targetDir: '/tmp/test',
        debugMode: false,
      ...
      expect(directories).not.toContain(Storage.getGlobalZERODir());
    });

    it('should allow isPathAllowed to write the global ~/.zero/ZERO.md file', async () => {
      // Surgical allowlist: the prompt routes cross-project personal
      // preferences to ~/.zero/ZERO.md, so the agent must be able to edit
    ...
    it('should NOT allow isPathAllowed to write other files under ~/.zero/ (least privilege)', async () => {
      // The allowlist is surgical: only ~/.zero/ZERO.md is reachable.
      ...
      const globalDir = Storage.getGlobalZERODir();
    ...
    it('should restrict scoped auto-memory extraction writes to generated artifacts', () => {
      ...
      const workspaceMemoryPath = path.join('/tmp/test', 'ZERO.md');
...
describe('Model Persistence Bug Fix (#19864)', () => {
  const baseParams: ConfigParameters = {
    ...
    targetDir: '/path/to/target',
    debugMode: false,
    model: PREVIEW_ZERO_3_1_MODEL, // User saved preview model
  };
  ...
  it('should NOT reset preview model for CodeAssist auth when refreshUserQuota is not called (no projectId)', async () => {
    ...
    expect(config.getModel()).toBe(PREVIEW_ZERO_3_1_MODEL);
    ...
    expect(config.getModel()).toBe(PREVIEW_ZERO_3_1_MODEL);
    expect(config.getModel()).not.toBe(DEFAULT_ZERO_MODEL_AUTO);
  });

  it('should NOT reset preview model for USE_ZERO (hasAccessToPreviewModel is set to true)', async () => {
    const mockContentConfig = {
      authType: AuthType.USE_ZERO,
    } as Partial<ContentGeneratorConfig> as ContentGeneratorConfig;
    ...
    // For USE_ZERO, hasAccessToPreviewModel should be set to true
  ...
  it('should persist model when user selects it with persistMode=true', () => {
    ...
    const config = new Config({
      ...baseParams,
      model: DEFAULT_ZERO_MODEL_AUTO, // Initial model
      onModelChange,
    });

    // User selects preview model with persist mode enabled
    config.setModel(PREVIEW_ZERO_3_1_MODEL, false); // isTemporary = false

    // Verify onModelChange was called to persist the model
    expect(onModelChange).toHaveBeenCalledWith(PREVIEW_ZERO_3_1_MODEL);
    expect(config.getModel()).toBe(PREVIEW_ZERO_3_1_MODEL);
  });
});
...
describe('hasZERO35FlashGAAccess model setting', () => {
  const baseParams: ConfigParameters = {
    sessionId: 'test',
    targetDir: '.',
    debugMode: false,
  ...
  it('should set DEFAULT_ZERO_FLASH_MODEL to zero-3.5-flash and PREVIEW_ZERO_FLASH_MODEL to zero-3-flash-preview if hasZERO35FlashGAAccess returns true and authType is USE_ZERO', () => {
    const config = new Config(baseParams);
    config['contentGeneratorConfig'] = { authType: AuthType.USE_ZERO };

    // Set experiment to return true for ZERO_3_5_FLASH_GA_LAUNCHED
    config.setExperiments({
      experimentIds: [],
      flags: {
        [ExperimentFlags.ZERO_3_5_FLASH_GA_LAUNCHED]: {
          boolValue: true,
        },
      },
    });
    ...
    // Call the method
    const result = config.hasZERO35FlashGAAccess();
    expect(result).toBe(true);

    expect(DEFAULT_ZERO_FLASH_MODEL).toBe('zero-3.5-flash');
    expect(PREVIEW_ZERO_FLASH_MODEL).toBe('zero-3-flash-preview');
  });

  it('should set DEFAULT_ZERO_FLASH_MODEL and PREVIEW_ZERO_FLASH_MODEL to zero-3.5-flash if hasZERO35FlashGAAccess returns true and authType is not USE_ZERO', () => {
    const config = new Config(baseParams);
    ...
    expect(DEFAULT_ZERO_FLASH_MODEL).toBe('zero-3.5-flash');
    expect(PREVIEW_ZERO_FLASH_MODEL).toBe('zero-3.5-flash');
  });
});
