...
import { DEFAULT_ZERO_MODEL, DEFAULT_ZERO_FLASH_MODEL } from './models.js';
...
describe('Flash Model Fallback Configuration', () => {
  ...
  describe('getModel', () => {
    it('should return contentGeneratorConfig model if available', () => {
      ...
      config.setModel(DEFAULT_ZERO_FLASH_MODEL);
      expect(config.getModel()).toBe(DEFAULT_ZERO_FLASH_MODEL);
    });
  ...
  describe('activateFallbackMode', () => {
    it('should set model to fallback and log event', () => {
      config.activateFallbackMode(DEFAULT_ZERO_FLASH_MODEL);
      expect(config.getModel()).toBe(DEFAULT_ZERO_FLASH_MODEL);
    ...
    it('should set fallback override when failedModel is provided and register runtime override', () => {
      ...
      expect(config.getModel()).toBe(DEFAULT_ZERO_FLASH_MODEL);
      expect(config.getFallbackOverride(DEFAULT_ZERO_MODEL)).toBe(
        DEFAULT_ZERO_FLASH_MODEL,
      );
      ...
      expect(
        config
          ...
          .getResolvedConfig({ model: DEFAULT_ZERO_MODEL }).model,
