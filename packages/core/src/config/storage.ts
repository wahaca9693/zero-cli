...
export class Storage {
  ...
  static getGlobalZERODir(): string {
    const homeDir = homedir();
    if (!homeDir) {
      return path.join(os.tmpdir(), ZERO_DIR);
    }
    return path.join(homeDir, ZERO_DIR);
  }
  ...
  static getMcpOAuthTokensPath(): string {
    return path.join(Storage.getGlobalZERODir(), 'mcp-oauth-tokens.json');
  }

  static getA2AOAuthTokensPath(): string {
    return path.join(Storage.getGlobalZERODir(), 'a2a-oauth-tokens.json');
  }

  static getGlobalSettingsPath(): string {
    return path.join(Storage.getGlobalZERODir(), 'settings.json');
  }

  static getInstallationIdPath(): string {
    return path.join(Storage.getGlobalZERODir(), 'installation_id');
  }

  static getGoogleAccountsPath(): string {
    return path.join(Storage.getGlobalZERODir(), GOOGLE_ACCOUNTS_FILENAME);
  }

  static getTrustedFoldersPath(): string {
    if (process.env['ZERO_CLI_TRUSTED_FOLDERS_PATH']) {
      return process.env['ZERO_CLI_TRUSTED_FOLDERS_PATH'];
    }
    return path.join(Storage.getGlobalZERODir(), TRUSTED_FOLDERS_FILENAME);
  }

  static getUserCommandsDir(): string {
    return path.join(Storage.getGlobalZERODir(), 'commands');
  }

  static getUserSkillsDir(): string {
    return path.join(Storage.getGlobalZERODir(), 'skills');
  }
  ...
  static getUserPoliciesDir(): string {
    return path.join(Storage.getGlobalZERODir(), 'policies');
  }

  static getUserKeybindingsPath(): string {
    return path.join(Storage.getGlobalZERODir(), 'keybindings.json');
  }

  static getUserAgentsDir(): string {
    return path.join(Storage.getGlobalZERODir(), 'agents');
  }
  ...
  static getPolicyIntegrityStoragePath(): string {
    return path.join(Storage.getGlobalZERODir(), 'policy_integrity.json');
  }

  private static getSystemConfigDir(): string {
    if (os.platform() === 'darwin') {
      return '/Library/Application Support/ZEROCli';
    } else if (os.platform() === 'win32') {
      return 'C:\\ProgramData\\zero-cli';
    } else {
      return '/etc/zero-cli';
    }
  }
  ...
  static getSystemSettingsPath(): string {
    if (process.env['ZERO_CLI_SYSTEM_SETTINGS_PATH']) {
      return process.env['ZERO_CLI_SYSTEM_SETTINGS_PATH'];
    }
  ...
  static getGlobalTempDir(): string {
    return path.join(Storage.getGlobalZERODir(), TMP_DIR_NAME);
  }
  ...
  getZERODir(): string {
    return path.join(this.targetDir, ZERO_DIR);
  }
  ...
  getWorkspacePoliciesDir(): string {
    return path.join(this.getZERODir(), 'policies');
  }
  ...
  static getOAuthCredsPath(): string {
    return path.join(Storage.getGlobalZERODir(), OAUTH_FILE);
  }
  ...
  async initialize(): Promise<void> {
    ...
    this.initPromise = (async () => {
      ...
      const registry = new ProjectRegistry(registryPath, [
        ...
        path.join(Storage.getGlobalZERODir(), 'history'),
      ]);
  ...
  private async performMigration(): Promise<void> {
    ...
    // Migrate History Dir
    const historyDir = path.join(Storage.getGlobalZERODir(), 'history');
  ...
  getWorkspaceSettingsPath(): string {
    return path.join(this.getZERODir(), 'settings.json');
  }

  getProjectCommandsDir(): string {
    return path.join(this.getZERODir(), 'commands');
  }

  getProjectSkillsDir(): string {
    return path.join(this.getZERODir(), 'skills');
  }
  ...
  getProjectAgentsDir(): string {
    return path.join(this.getZERODir(), 'agents');
  }
  ...
  getExtensionsDir(): string {
    return path.join(this.getZERODir(), 'extensions');
  }

  getExtensionsConfigPath(): string {
    return path.join(this.getExtensionsDir(), 'zero-extension.json');
  }
