 ...
 * directly.
 */
export interface ZEROCLIExtension {
  name: string;
  version: string;
  isActive: boolean;
  path: string;
...
export class MCPServerConfig {
  constructor(
    ...
    readonly extension?: ZEROCLIExtension,
    // OAuth configuration
...
export interface ConfigParameters {
  ...
  zeroMdFileCount?: number;
  zeroMdFilePaths?: string[];
...
export class Config implements McpContext, AgentLoopContext {
  ...
  private zeroMdFileCount: number;
  private zeroMdFilePaths: string[];
  private readonly showMemoryUsage: boolean;
  private readonly logRagSnippets: boolean;
  private readonly accessibility: AccessibilitySettings;
  private readonly telemetrySettings: TelemetrySettings;
  private readonly usageStatisticsEnabled: boolean;
  private _zeroClient!: ZEROClient;
  ...
  constructor(params: ConfigParameters) {
    ...
    this.embeddingModel =
      params.embeddingModel ?? DEFAULT_ZERO_EMBEDDING_MODEL;
    ...
    this.zeroMdFileCount = params.zeroMdFileCount ?? 0;
    this.zeroMdFilePaths = params.zeroMdFilePaths ?? [];
    ...
    this.fileFiltering = {
      ...
      respectZEROIgnore:
        params.fileFiltering?.respectZEROIgnore ??
        DEFAULT_FILE_FILTERING_OPTIONS.respectZEROIgnore,
      enableFileWatcher:
    ...
    const isZERO2 = isZERO2Model(this.model);
    this.useWriteTodos =
      isZERO2 && !isPreviewModel(this.model, this) && !this.trackerEnabled
    ...
    if (params.contextFileName) {
      setZEROMdFilename(params.contextFileName);
    }
    ...
    this._zeroClient = new ZEROClient(this);
  ...
  private async _initialize(): Promise<void> {
    ...
    await this._zeroClient.initialize();
  ...
  async refreshAuth(
    ...
    if (
      this.contentGeneratorConfig?.authType === AuthType.USE_ZERO &&
      authMethod !== AuthType.USE_ZERO
    ) {
      // Restore the conversation history to the new client
      this._zeroClient.stripThoughtsFromHistory();
    }

    // Reset availability status when switching auth (e.g. from limited key to OAuth)
    this.modelAvailabilityService.reset();

    // Clear stale authType to ensure getZERO31LaunchedSync doesn't return stale results
    ...
    if (
      ...
      this.setModel(DEFAULT_ZERO_MODEL_AUTO);
    }
    ...
    if ((await this.getProModelNoAccess()) && isAutoModel(this.model)) {
      this.setModel(PREVIEW_ZERO_FLASH_MODEL);
    }
  }
  ...
  getUserPaidTier(): ZEROUserTier | undefined {
   ...
   * Use the injected AgentLoopContext instead.
   */
  get zeroClient(): ZEROClient {
    return this._zeroClient;
  }
  ...
  /**
   * The user configured MCP servers (via zero settings files).
   *
  ...
  async refreshMcpContext(): Promise<void> {
    await this.memoryContextManager?.refresh();
    if (this._zeroClient?.isInitialized()) {
      await this._zeroClient.setTools();
      this._zeroClient.updateSystemInstruction();
    }
  }
  ...
  getZEROMdFileCount(): number {
    if (this.memoryContextManager) {
      return this.memoryContextManager.getLoadedPaths().size;
    }
    return this.zeroMdFileCount;
  }

  setZEROMdFileCount(count: number): void {
    this.zeroMdFileCount = count;
  }

  getZEROMdFilePaths(): string[] {
    if (this.memoryContextManager) {
      return Array.from(this.memoryContextManager.getLoadedPaths());
    }
    return this.zeroMdFilePaths;
  }
  ...
  setZEROMdFilePaths(paths: string[]): void {
    this.zeroMdFilePaths = paths;
  }
  ...
  setApprovalMode(mode: ApprovalMode): void {
    ...
    if (currentMode !== mode) {
      ...
      if (isPlanModeTransition || isYoloModeTransition) {
        if (this._zeroClient?.isInitialized()) {
          this._zeroClient.clearCurrentSequenceModel();
          this._zeroClient.setTools().catch((err) => {
  ...
  /** @deprecated Use zeroClient getter */
  getZEROClient(): ZEROClient {
    return this.zeroClient;
  }

  /**
   * Updates the system instruction with the latest user memory.
   * Whenever the user memory (ZERO.md files) is updated.
   */
  updateSystemInstructionIfInitialized(): void {
    const zeroClient = this.zeroClient;
    if (zeroClient?.isInitialized()) {
      zeroClient.updateSystemInstruction();
    }
  }
  ...
  getFileFilteringRespectZEROIgnore(): boolean {
    return this.fileFiltering.respectZEROIgnore;
  }
  ...
  getFileFilteringOptions(): FileFilteringOptions {
    return {
      ...
      respectZEROIgnore: this.fileFiltering.respectZEROIgnore,
  ...
  getExtensions(): ZEROCLIExtension[] {
   ...
   * temporary directory, or is exactly the global personal `~/.zero/ZERO.md`
   * file (the latter is the only file under `~/.zero/` that is reachable —
  ...
  isPathAllowed(absolutePath: string): boolean {
    ...
    // Surgical allowlist: the global personal ZERO.md file (and ONLY that
  ...
  /**
   * Returns whether ZERO 3.1 Pro has been launched.
   * This method is async and ensures that experiments are loaded before returning the result.
   */
  async getZERO31Launched(): Promise<boolean> {
    await this.ensureExperimentsLoaded();
    return this.getZERO31LaunchedSync();
  }
  ...
  async getUseCustomToolModel(): Promise<boolean> {
    const useZERO3_1 = await this.getZERO31Launched();
    const authType = this.contentGeneratorConfig?.authType;
    return useZERO3_1 && authType === AuthType.USE_ZERO;
  }
  ...
  getUseCustomToolModelSync(): boolean {
    const useZERO3_1 = this.getZERO31LaunchedSync();
    const authType = this.contentGeneratorConfig?.authType;
    return useZERO3_1 && authType === AuthType.USE_ZERO;
  }

  private isZERO31LaunchedForAuthType(authType?: AuthType): boolean {
    return (
      authType === AuthType.USE_ZERO ||
      authType === AuthType.USE_VERTEX_AI ||
  ...
  /**
   * Returns whether ZERO 3.5 Flash GA has been launched.
   *
   * Note: This method should only be called after startup, once experiments have been loaded.
   */
  hasZERO35FlashGAAccess(): boolean {
    const authType = this.contentGeneratorConfig?.authType;
    const hasAccess = (() => {
      if (this.isZERO31LaunchedForAuthType(authType)) {
        return true;
      }
      return (
        this.experiments?.flags[ExperimentFlags.ZERO_3_5_FLASH_GA_LAUNCHED]
          ?.boolValue ?? false
      );
    })();
    ...
    if (hasAccess) {
      // ZERO API key users should have the ability to manually select the
      // old preview flash model.
      if (authType === AuthType.USE_ZERO) {
        setFlashModels('zero-3-flash-preview', 'zero-3.5-flash');
      } else {
        setFlashModels('zero-3.5-flash', 'zero-3.5-flash');
      }
    } else {
      setFlashModels('zero-3-flash-preview', 'zero-2.5-flash');
    }
    return hasAccess;
  }

  /**
   * Returns whether ZERO 3.1 has been launched.
   *
   * Note: This method should only be called after startup, once experiments have been loaded.
   * If you need to call this during startup or from an async context, use
   * getZERO31Launched instead.
   */
  getZERO31LaunchedSync(): boolean {
    ...
    return (
      this.experiments?.flags[ExperimentFlags.ZERO_3_1_PRO_LAUNCHED]
        ?.boolValue ?? false
    );
  }
  ...
  getAgentSessionNoninteractiveEnabled(): boolean {
    return (
      process.env['ZERO_CLI_EXP_AGENT'] === 'true' ||
      this.agentSessionNoninteractiveEnabled
    );
  }
  ...
  private onAgentsRefreshed = async () => {
    ...
    const client = this.zeroClient;
    if (client?.isInitialized()) {
      await client.setTools();
      client.updateSystemInstruction();
    } else {
      debugLogger.debug(
        '[Config] ZEROClient not initialized; skipping live prompt/tool refresh.',
      );
    }
  };
...
export { DEFAULT_ZERO_FLASH_MODEL };
