...
import { ZERO_DIR, homedir, resolveToRealPath } from '../utils/paths.js';
...
describe('Storage – initialize', () => {
  ...
  it('sets up the registry and performs migration if `getProjectTempDir` is called', async () => {
    ...
    expect(storage.getProjectTempDir()).toBe(
      path.join(os.homedir(), ZERO_DIR, 'tmp', PROJECT_SLUG),
    );
...
describe('Storage – getGlobalSettingsPath', () => {
  it('returns path to ~/.zero/settings.json', () => {
    const expected = path.join(os.homedir(), ZERO_DIR, 'settings.json');
    expect(Storage.getGlobalSettingsPath()).toBe(expected);
  });
});

describe('Storage - Security', () => {
  it('falls back to tmp for zero but returns empty for agents if the home directory cannot be determined', () => {
    vi.mocked(homedir).mockReturnValue('');

    // .zero falls back for backward compatibility
    expect(Storage.getGlobalZERODir()).toBe(
      path.join(os.tmpdir(), ZERO_DIR),
    );
...
describe('Storage – additional helpers', () => {
  ...
  it('getWorkspaceSettingsPath returns project/.zero/settings.json', () => {
    const expected = path.join(projectRoot, ZERO_DIR, 'settings.json');
    expect(storage.getWorkspaceSettingsPath()).toBe(expected);
  });

  it('getUserCommandsDir returns ~/.zero/commands', () => {
    const expected = path.join(os.homedir(), ZERO_DIR, 'commands');
    expect(Storage.getUserCommandsDir()).toBe(expected);
  });

  it('getProjectCommandsDir returns project/.zero/commands', () => {
    const expected = path.join(projectRoot, ZERO_DIR, 'commands');
    expect(storage.getProjectCommandsDir()).toBe(expected);
  });

  it('getUserSkillsDir returns ~/.zero/skills', () => {
    const expected = path.join(os.homedir(), ZERO_DIR, 'skills');
    expect(Storage.getUserSkillsDir()).toBe(expected);
  });

  it('getProjectSkillsDir returns project/.zero/skills', () => {
    const expected = path.join(projectRoot, ZERO_DIR, 'skills');
    expect(storage.getProjectSkillsDir()).toBe(expected);
  });

  it('getUserAgentsDir returns ~/.zero/agents', () => {
    const expected = path.join(os.homedir(), ZERO_DIR, 'agents');
    expect(Storage.getUserAgentsDir()).toBe(expected);
  });

  it('getProjectAgentsDir returns project/.zero/agents', () => {
    const expected = path.join(projectRoot, ZERO_DIR, 'agents');
    expect(storage.getProjectAgentsDir()).toBe(expected);
  });

  it('getProjectMemoryDir returns ~/.zero/tmp/<identifier>/memory', async () => {
    await storage.initialize();
  ...
  it('getMcpOAuthTokensPath returns ~/.zero/mcp-oauth-tokens.json', () => {
    const expected = path.join(
      os.homedir(),
      ZERO_DIR,
  ...
  it('getGlobalBinDir returns ~/.zero/tmp/bin', () => {
    const expected = path.join(os.homedir(), ZERO_DIR, 'tmp', 'bin');
    expect(Storage.getGlobalBinDir()).toBe(expected);
  });

  it('getProjectTempPlansDir returns ~/.zero/tmp/<identifier>/plans when no sessionId is provided', async () => {
    await storage.initialize();
  ...
  it('getProjectTempPlansDir returns ~/.zero/tmp/<identifier>/<sessionId>/plans when sessionId is provided', async () => {
  ...
  it('getProjectTempTrackerDir returns ~/.zero/tmp/<identifier>/tracker when no sessionId is provided', async () => {
    await storage.initialize();
    const tempDir = storage.getProjectTempDir();
  ...
  it('getProjectTempTrackerDir returns ~/.zero/tmp/<identifier>/<sessionId>/tracker when sessionId is provided', async () => {
    const sessionId = 'test-session-id';
    const storageWithSession = new Storage(projectRoot, sessionId);
    ProjectRegistry.prototype.getShortId = vi
      .fn()
...
describe('Storage - System Paths', () => {
  const originalEnv = process.env['ZERO_CLI_SYSTEM_SETTINGS_PATH'];

  afterEach(() => {
    if (originalEnv !== undefined) {
      process.env['ZERO_CLI_SYSTEM_SETTINGS_PATH'] = originalEnv;
    } else {
      delete process.env['ZERO_CLI_SYSTEM_SETTINGS_PATH'];
    }
  });
  ...
  it('getSystemSettingsPath returns correct path based on platform (default)', () => {
    ...
    if (platform === 'darwin') {
      expect(result).toBe(
        '/Library/Application Support/ZEROCli/settings.json',
      );
    } else if (platform === 'win32') {
      expect(result).toBe('C:\\ProgramData\\zero-cli\\settings.json');
    } else {
      expect(result).toBe('/etc/zero-cli/settings.json');
    }
  });

  it('getSystemSettingsPath follows ZERO_CLI_SYSTEM_SETTINGS_PATH if set', () => {
    const customPath = '/custom/path/settings.json';
    process.env['ZERO_CLI_SYSTEM_SETTINGS_PATH'] = customPath;
  ...
  it('getSystemPoliciesDir returns correct path based on platform and ignores env var', () => {
    process.env['ZERO_CLI_SYSTEM_SETTINGS_PATH'] =
    ...
    if (platform === 'darwin') {
      expect(result).toBe('/Library/Application Support/ZEROCli/policies');
    } else if (platform === 'win32') {
      expect(result).toBe('C:\\ProgramData\\zero-cli\\policies');
    } else {
      expect(result).toBe('/etc/zero-cli/policies');
    }
  });
});
