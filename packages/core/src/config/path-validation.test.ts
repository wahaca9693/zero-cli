...
describe('Config Path Validation', () => {
  ...
  const globalZERODir = path.join(os.homedir(), '.zero');
  ...
  it('should allow access to a file under ~/.zero once that directory is added to the workspace', () => {
    // Use settings.json rather than ZERO.md as the example: the latter is
    ...
    // under ~/.zero/.
    const settingsPath = path.join(globalZERODir, 'settings.json');

    // Before adding, it should be denied
    expect(config.isPathAllowed(settingsPath)).toBe(false);

    // Add to workspace
    config.getWorkspaceContext().addDirectory(globalZERODir);
