...
const createMockConfig = (overrides: Partial<Config> = {}): Config => {
  const config = {
    ...
    getModel: () => 'zero-2.5-pro',
    getZERO31LaunchedSync: () => false,
    getUseCustomToolModelSync: () => {
      const useZERO31 = config.getZERO31LaunchedSync();
      const authType = config.getContentGeneratorConfig().authType;
      return useZERO31 && authType === AuthType.USE_ZERO;
    },
...
describe('policyHelpers', () => {
  describe('resolvePolicyChain', () => {
    ...
    it('leaves catalog order untouched when active model already present', () => {
      ...
      const chain = resolvePolicyChain(config);
      expect(chain[0]?.model).toBe('zero-2.5-pro');
    });

    it('returns the default chain when active model is "auto"', () => {
      const config = createMockConfig({
        getModel: () => DEFAULT_ZERO_MODEL_AUTO,
      });
      const chain = resolvePolicyChain(config);
      ...
      expect(chain[0]?.model).toBe('zero-2.5-pro');
      expect(chain[1]?.model).toBe('zero-2.5-flash');
    });
    ...
    it('uses auto chain when preferred model is auto', () => {
      ...
      const chain = resolvePolicyChain(config, DEFAULT_ZERO_MODEL_AUTO);
      expect(chain).toHaveLength(2);
      expect(chain[0]?.model).toBe('zero-2.5-pro');
      expect(chain[1]?.model).toBe('zero-2.5-flash');
    });
    ...
    it('uses auto chain when configured model is auto even if preferred is concrete', () => {
      ...
      const chain = resolvePolicyChain(config, 'zero-2.5-pro');
      expect(chain).toHaveLength(2);
      expect(chain[0]?.model).toBe('zero-2.5-pro');
      expect(chain[1]?.model).toBe('zero-2.5-flash');
    });
    ...
    it('starts chain from preferredModel when model is "auto"', () => {
      ...
      const chain = resolvePolicyChain(config, 'zero-2.5-flash');
      // Due to ZERO 2.x wrapsAround, the chain will contain both flash and pro
      expect(chain.length).toBeGreaterThanOrEqual(1);
      expect(chain[0]?.model).toBe('zero-2.5-flash');
    });
    ...
    it('returns flash-lite chain when preferred model is flash-lite', () => {
      ...
      const chain = resolvePolicyChain(config, DEFAULT_ZERO_FLASH_LITE_MODEL);
      expect(chain).toHaveLength(3);
      expect(chain[0]?.model).toBe('zero-3.1-flash-lite');
      expect(chain[1]?.model).toBe('zero-2.5-flash');
      expect(chain[2]?.model).toBe('zero-2.5-pro');
    });

    it('returns flash-lite chain when configured model is flash-lite', () => {
      const config = createMockConfig({
        getModel: () => DEFAULT_ZERO_FLASH_LITE_MODEL,
      });
      const chain = resolvePolicyChain(config);
      expect(chain).toHaveLength(3);
      expect(chain[0]?.model).toBe('zero-3.1-flash-lite');
    ...
    it('wraps around the chain when wrapsAround is true', () => {
      ...
      const chain = resolvePolicyChain(config, 'zero-2.5-flash', true);
      expect(chain).toHaveLength(2);
      expect(chain[0]?.model).toBe('zero-2.5-flash');
      expect(chain[1]?.model).toBe('zero-2.5-pro');
    });

    it('proactively returns ZERO 2.5 chain if ZERO 3 requested but user lacks access', () => {
      const config = createMockConfig({
        getModel: () => 'auto-zero-3',
    ...
    it('returns ZERO 3.1 Pro chain when launched and auto-zero-3 requested', () => {
      const config = createMockConfig({
        getModel: () => 'auto-zero-3',
        getZERO31LaunchedSync: () => true,
      });
      const chain = resolvePolicyChain(config);
      expect(chain[0]?.model).toBe(PREVIEW_ZERO_3_1_MODEL);
      expect(chain[1]?.model).toBe('zero-3-flash-preview');
    });

    it('returns ZERO 3.1 Pro Custom Tools chain when launched, auth is ZERO, and auto-zero-3 requested', () => {
      const config = createMockConfig({
        getModel: () => 'auto-zero-3',
        getZERO31LaunchedSync: () => true,
        getContentGeneratorConfig: () => ({ authType: AuthType.USE_ZERO }),
      });
      const chain = resolvePolicyChain(config);
      expect(chain[0]?.model).toBe(PREVIEW_ZERO_3_1_CUSTOM_TOOLS_MODEL);
      expect(chain[1]?.model).toBe('zero-3-flash-preview');
    });
  ...
  describe('resolvePolicyChain behavior is identical between dynamic and legacy implementations', () => {
    const testCases = [
      { name: 'Default Auto', model: DEFAULT_ZERO_MODEL_AUTO },
      { name: 'ZERO 3 Auto', model: 'auto-zero-3' },
      { name: 'Unified Auto', model: 'auto' },
      { name: 'Flash Lite', model: DEFAULT_ZERO_FLASH_LITE_MODEL },
      {
        name: 'ZERO 3 Auto (3.1 Enabled)',
        model: 'auto-zero-3',
        useZERO31: true,
      },
      {
        name: 'ZERO 3 Auto (3.1 + Custom Tools)',
        model: 'auto-zero-3',
        useZERO31: true,
        authType: AuthType.USE_ZERO,
      },
      {
        name: 'ZERO 3 Auto (No Access)',
        model: 'auto-zero-3',
        hasAccess: false,
      },
      { name: 'Concrete Model (2.5 Pro)', model: 'zero-2.5-pro' },
      { name: 'Explicit ZERO 3', model: 'zero-3-pro-preview' },
  ...
  describe('applyModelSelection', () => {
    ...
    it('returns requested model if it is available', () => {
      ...
      expect(result.model).toBe('zero-pro');
      expect(result.maxAttempts).toBeUndefined();
      expect(config.setActiveModel).toHaveBeenCalledWith('zero-pro');
    });
    ...
    it('switches to backup model and updates config if requested is unavailable', () => {
      ...
      expect(result.model).toBe('zero-flash');
      ...
      expect(config.setActiveModel).toHaveBeenCalledWith('zero-flash');
    });
    ...
    it('does not consume sticky attempt if consumeAttempt is false', () => {
      ...
      const result = applyModelSelection(
        config,
        { model: 'zero-pro', isChatModel: true },
        {
