...
describe('ModelAvailabilityService', () => {
  ...
  describe('prefix normalization', () => {
    it('treats prefixed and non-prefixed models as identical when marking terminal', () => {
      service.markTerminal('models/zero-3.1-pro-preview', 'quota');

      // Checking the non-prefixed version should show it as unavailable
      expect(service.snapshot('zero-3.1-pro-preview')).toEqual({
        available: false,
        reason: 'quota',
      });

      // Checking the prefixed version should also show it as unavailable
      expect(service.snapshot('models/zero-3.1-pro-preview')).toEqual({
        available: false,
        reason: 'quota',
      });
    });
    ...
    it('treats prefixed and non-prefixed models as identical when selecting', () => {
      service.markTerminal('zero-3-flash-preview', 'quota');
      ...
      const result = service.selectFirstAvailable([
        'models/zero-3-flash-preview',
        'zero-3.1-pro-preview',
      ]);

      expect(result.selectedModel).toBe('zero-3.1-pro-preview');
      expect(result.skipped).toEqual([
        { model: 'zero-3-flash-preview', reason: 'quota' },
      ]);
    });

    it('treats prefixed and non-prefixed models as identical when marking healthy', () => {
      service.markTerminal('zero-3-flash-preview', 'quota');
      service.markHealthy('models/zero-3-flash-preview');

      expect(service.snapshot('zero-3-flash-preview')).toEqual({
        available: true,
      });
    });
  });
