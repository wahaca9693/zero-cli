...
describe('Fallback Integration', () => {
  ...
  beforeEach(() => {
    ...
    config = {
      getModel: () => PREVIEW_ZERO_MODEL_AUTO,
      getActiveModel: () => PREVIEW_ZERO_MODEL_AUTO,
  ...
  it('should select fallback model when primary model is terminal and config is in AUTO mode', () => {
    ...
    // The policy chain for PREVIEW_ZERO_MODEL_AUTO is [PREVIEW_ZERO_MODEL, PREVIEW_ZERO_FLASH_MODEL]
    availabilityService.markTerminal(PREVIEW_ZERO_MODEL, 'quota');

    // 2. Request "Pro" explicitly (as Agent would)
    const requestedModel = PREVIEW_ZERO_MODEL;
    ...
    expect(result.model).toBe(PREVIEW_ZERO_FLASH_MODEL);
  ...
  it('should fallback for ZERO 3 models even if config is NOT in AUTO mode', () => {
    // 1. Config is explicitly set to Pro, not Auto
    vi.spyOn(config, 'getModel').mockReturnValue(PREVIEW_ZERO_MODEL);
    ...
    // 5. Expect it to fallback to Flash (because ZERO 3 uses PREVIEW_CHAIN)
    expect(result.model).toBe(PREVIEW_ZERO_FLASH_MODEL);
  });
  ...
  it('should fallback to Flash after failures and restore Pro on next turn', () => {
    ...
    expect(result1.model).toBe(PREVIEW_ZERO_MODEL);
    expect(result1.maxAttempts).toBe(3);

    // 2. Simulate failure and transition to sticky_retry with consumed=true
    availabilityService.markRetryOncePerTurn(PREVIEW_ZERO_MODEL, 3);
    availabilityService.consumeStickyAttempt(PREVIEW_ZERO_MODEL);
    ...
    expect(result2.model).toBe(PREVIEW_ZERO_FLASH_MODEL);
    ...
    expect(result3.model).toBe(PREVIEW_ZERO_MODEL);
