...
describe('policyCatalog', () => {
  it('returns preview chain when preview enabled', () => {
    ...
    expect(chain[0]?.model).toBe(PREVIEW_ZERO_MODEL);
    expect(chain).toHaveLength(2);
  });

  it('returns ZERO 3.1 chain when useZERO31 is true', () => {
  ...
  it('returns ZERO 3.1 Custom Tools chain when useZERO31 and useCustomToolModel are true', () => {
    const chain = getModelPolicyChain({
      previewEnabled: true,
      useZERO31: true,
  ...
  it('returns default chain when preview disabled', () => {
    ...
    expect(chain[0]?.model).toBe(DEFAULT_ZERO_MODEL);
    expect(chain).toHaveLength(2);
  });
  ...
  it('marks preview transients as sticky retries when auto-selected', () => {
    ...
    expect(previewPolicy.model).toBe(PREVIEW_ZERO_MODEL);
