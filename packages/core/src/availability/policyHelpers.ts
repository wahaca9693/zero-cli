...
export function resolvePolicyChain(
  ...
  const isOriginallyZERO3 = isZERO3Model(modelFromConfig, config);
  ...
  // We always wrap around for ZERO 3 chains to ensure maximum availability
  // between models in the same family (e.g. fallback to Pro if Flash is exhausted).
  const effectiveWrapsAround =
    wrapsAround || isAutoPreferred || isAutoConfigured || isOriginallyZERO3;
  ...
  if (config.getExperimentalDynamicModelConfiguration?.() === true) {
    ...
    if (resolvedModel === DEFAULT_ZERO_FLASH_LITE_MODEL) {
      chain = config.modelConfigService.resolveChain('lite', context);
    } else if (isOriginallyZERO3 || isAutoPreferred || isAutoConfigured) {
      ...
      if (!chain) {
        ...
        const previewEnabled =
          hasAccessToPreview &&
          (isZERO3Model(resolvedModel, config) ||
            normalizedPreferredModel === PREVIEW_ZERO_MODEL_AUTO ||
            configuredModel === PREVIEW_ZERO_MODEL_AUTO);
    ...
    if (resolvedModel === DEFAULT_ZERO_FLASH_LITE_MODEL) {
      ...
      if (hasAccessToPreview) {
        ...
        // User requested ZERO 3 but has no access. Proactively downgrade
        // to the stable ZERO 2.5 chain.
        chain = getModelPolicyChain({
...
export function selectModelForAvailability(
  ...
  const backupModel =
    chain.find((p) => p.isLastResort)?.model ?? DEFAULT_ZERO_MODEL;
