...
export function logConsecaPolicyGeneration(
  ...
  if (clearcutLogger) {
    ...
    if (config.getTelemetryLogPromptsEnabled()) {
      data.push(
        {
          zero_cli_key: EventMetadataKey.CONSECA_USER_PROMPT,
          value: safeJsonStringify(event.user_prompt),
        },
        {
          zero_cli_key: EventMetadataKey.CONSECA_TRUSTED_CONTENT,
          value: safeJsonStringify(event.trusted_content),
        },
        {
          zero_cli_key: EventMetadataKey.CONSECA_GENERATED_POLICY,
    ...
    if (event.error) {
      data.push({
        zero_cli_key: EventMetadataKey.CONSECA_ERROR,
        value: event.error,
      });
    }
...
export function logConsecaVerdict(
  ...
  if (clearcutLogger) {
    const data: EventValue[] = [
      {
        zero_cli_key: EventMetadataKey.CONSECA_VERDICT_RESULT,
    ...
    if (config.getTelemetryLogPromptsEnabled()) {
      data.push(
        ...
        {
          zero_cli_key: EventMetadataKey.CONSECA_VERDICT_RATIONALE,
