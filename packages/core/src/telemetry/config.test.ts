...
describe('telemetry/config helpers', () => {
  ...
  describe('resolveTelemetrySettings', () => {
    ...
    it('uses env over settings and argv over env', async () => {
      ...
      const env = {
        ZERO_TELEMETRY_ENABLED: '1',
        ZERO_TELEMETRY_TARGET: 'gcp',
        ZERO_TELEMETRY_OTLP_ENDPOINT: 'http://env:4317',
        ZERO_TELEMETRY_OTLP_PROTOCOL: 'http',
        ZERO_TELEMETRY_LOG_PROMPTS: 'true',
        ZERO_TELEMETRY_OUTFILE: 'env.log',
        ZERO_TELEMETRY_USE_COLLECTOR: 'true',
    ...
    it('resolves useCliAuth from env', async () => {
      const env = {
        ZERO_TELEMETRY_USE_CLI_AUTH: 'true',
      };
    ...
    it('falls back to OTEL_EXPORTER_OTLP_ENDPOINT when ZERO var is missing', async () => {
      const settings = {};
      const env = {
    ...
    it('throws on unknown protocol values', async () => {
      const env = { ZERO_TELEMETRY_OTLP_PROTOCOL: 'unknown' } as Record<
        string,
        string
      >;
    ...
    it('throws on unknown target values', async () => {
      const env = { ZERO_TELEMETRY_TARGET: 'unknown' } as Record<
        string,
        string
      >;
      await expect(resolveTelemetrySettings({ env })).rejects.toThrow(
