...
export interface EventValue {
  zero_cli_key: EventMetadataKey;
  value: string;
}

export interface LogEvent {
  console_type: 'ZERO_CLI';
  application: number;
  event_name: string;
 ...
 * distribution channel in which the user is using ZERO CLI.  ZERO CLI comes bundled
...
export class ClearcutLogger {
      ...
      this.config?.getExperimentsAsync().then((experiments) => {
        if (experiments) {
          const exp_id_data: EventValue[] = [
            {
              zero_cli_key: EventMetadataKey.ZERO_CLI_EXPERIMENT_IDS,
  ...
  createBasicLogEvent(
    ...
    const baseMetadata: EventValue[] = [
      ...data,
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_SURFACE,
        value: surface,
      },
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_VERSION,
        value: CLI_VERSION,
      },
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_GIT_COMMIT_HASH,
        value: GIT_COMMIT_INFO,
      },
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_OS,
    ...
    if (ghWorkflowName) {
      baseMetadata.push({
        zero_cli_key: EventMetadataKey.ZERO_CLI_GH_WORKFLOW_NAME,
    ...
    if (this.hashedGHRepositoryName) {
      baseMetadata.push({
        zero_cli_key: EventMetadataKey.ZERO_CLI_GH_REPOSITORY_NAME_HASH,
    ...
    if (ghEventName) {
      baseMetadata.push({
        zero_cli_key: EventMetadataKey.ZERO_CLI_GH_EVENT_NAME,
        value: ghEventName,
      });
    }
    ...
    if (ghPRNumber) {
      baseMetadata.push({
        zero_cli_key: EventMetadataKey.ZERO_CLI_GH_PR_NUMBER,
        value: ghPRNumber,
      });
    }
    ...
    if (ghIssueNumber) {
      baseMetadata.push({
        zero_cli_key: EventMetadataKey.ZERO_CLI_GH_ISSUE_NUMBER,
        value: ghIssueNumber,
      });
    }
    ...
    if (ghCustomTrackingId) {
      baseMetadata.push({
        zero_cli_key: EventMetadataKey.ZERO_CLI_GH_CUSTOM_TRACKING_ID,
  ...
  async logStartSessionEvent(event: StartSessionEvent): Promise<void> {
    const data: EventValue[] = [
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_START_SESSION_MODEL,
        value: event.model,
      },
      {
        zero_cli_key:
          EventMetadataKey.ZERO_CLI_START_SESSION_EMBEDDING_MODEL,
        value: event.embedding_model,
      },
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_START_SESSION_SANDBOX,
        value: event.sandbox_enabled.toString(),
      },
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_START_SESSION_CORE_TOOLS,
        value: event.core_tools_enabled,
      },
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_START_SESSION_APPROVAL_MODE,
        value: event.approval_mode,
      },
      {
        zero_cli_key:
          EventMetadataKey.ZERO_CLI_START_SESSION_API_KEY_ENABLED,
        value: event.api_key_enabled.toString(),
      },
      {
        zero_cli_key:
          EventMetadataKey.ZERO_CLI_START_SESSION_VERTEX_API_ENABLED,
        value: event.vertex_ai_enabled.toString(),
      },
      {
        zero_cli_key:
          EventMetadataKey.ZERO_CLI_START_SESSION_DEBUG_MODE_ENABLED,
      ...
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_START_SESSION_MCP_SERVERS,
      ...
      {
        zero_cli_key:
          EventMetadataKey.ZERO_CLI_START_SESSION_TELEMETRY_ENABLED,
        value: event.telemetry_enabled.toString(),
      },
      {
        zero_cli_key:
          EventMetadataKey.ZERO_CLI_START_SESSION_TELEMETRY_LOG_USER_PROMPTS_ENABLED,
        value: event.telemetry_log_user_prompts_enabled.toString(),
      },
      {
        zero_cli_key:
          EventMetadataKey.ZERO_CLI_START_SESSION_MCP_SERVERS_COUNT,
      ...
      {
        zero_cli_key:
          EventMetadataKey.ZERO_CLI_START_SESSION_MCP_TOOLS_COUNT,
        value: event.mcp_tools_count?.toString() ?? '',
      },
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_START_SESSION_MCP_TOOLS,
        value: event.mcp_tools ? event.mcp_tools : '',
      },
      {
        zero_cli_key:
          EventMetadataKey.ZERO_CLI_START_SESSION_EXTENSIONS_COUNT,
        value: event.extensions_count.toString(),
      },
      // We deliberately do not log the names of extensions here, to be safe.
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_START_SESSION_EXTENSION_IDS,
        value: event.extension_ids.toString(),
      },
      {
        zero_cli_key:
          EventMetadataKey.ZERO_CLI_START_SESSION_WORKTREE_ACTIVE,
    ...
    if (cpus && cpus.length > 0) {
      data.push({
        zero_cli_key: EventMetadataKey.ZERO_CLI_CPU_INFO,
        value: cpus[0].model,
      });
    }
    ...
    data.push(
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_CPU_CORES,
        value: os.availableParallelism().toString(),
      },
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_RAM_TOTAL_GB,
    ...
    data.push({
      zero_cli_key: EventMetadataKey.ZERO_CLI_GPU_INFO,
      value: gpuInfo,
    });
  ...
  logNewPromptEvent(event: UserPromptEvent): void {
    ...
    const data: EventValue[] = [
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_USER_PROMPT_LENGTH,
  ...
  logToolCallEvent(event: ToolCallEvent): void {
    const data: EventValue[] = [
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_TOOL_CALL_NAME,
        value: JSON.stringify(event.function_name),
      },
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_TOOL_CALL_DECISION,
        value: JSON.stringify(event.decision),
      },
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_TOOL_CALL_SUCCESS,
        value: JSON.stringify(event.success),
      },
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_TOOL_CALL_DURATION_MS,
        value: JSON.stringify(event.duration_ms),
      },
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_TOOL_CALL_ERROR_TYPE,
        value: JSON.stringify(event.error_type),
      },
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_TOOL_TYPE,
        value: JSON.stringify(event.tool_type),
      },
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_TOOL_CALL_CONTENT_LENGTH,
        value: JSON.stringify(event.content_length),
      },
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_TOOL_CALL_MCP_SERVER_NAME,
    ...
    if (event.metadata) {
      const metadataMapping: { [key: string]: EventMetadataKey } = {
        model_added_lines: EventMetadataKey.ZERO_CLI_AI_ADDED_LINES,
        model_removed_lines: EventMetadataKey.ZERO_CLI_AI_REMOVED_LINES,
        model_added_chars: EventMetadataKey.ZERO_CLI_AI_ADDED_CHARS,
        model_removed_chars: EventMetadataKey.ZERO_CLI_AI_REMOVED_CHARS,
        user_added_lines: EventMetadataKey.ZERO_CLI_USER_ADDED_LINES,
        user_removed_lines: EventMetadataKey.ZERO_CLI_USER_REMOVED_LINES,
        user_added_chars: EventMetadataKey.ZERO_CLI_USER_ADDED_CHARS,
        user_removed_chars: EventMetadataKey.ZERO_CLI_USER_REMOVED_CHARS,
      };
      ...
      if (
        ...
        const askUserMapping: { [key: string]: EventMetadataKey } = {
          question_types: EventMetadataKey.ZERO_CLI_ASK_USER_QUESTION_TYPES,
          dismissed: EventMetadataKey.ZERO_CLI_ASK_USER_DISMISSED,
          empty_submission:
            EventMetadataKey.ZERO_CLI_ASK_USER_EMPTY_SUBMISSION,
          answer_count: EventMetadataKey.ZERO_CLI_ASK_USER_ANSWER_COUNT,
        };

        for (const [key, zero_cli_key] of Object.entries(askUserMapping)) {
      ...
      for (const [key, zero_cli_key] of Object.entries(metadataMapping)) {
    ...
    if (event.extension_id) {
      data.push({
        zero_cli_key: EventMetadataKey.ZERO_CLI_EXTENSION_ID,
  ...
  logFileOperationEvent(event: FileOperationEvent): void {
    const data: EventValue[] = [
      ...
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_FILE_OPERATION_TYPE,
        value: JSON.stringify(event.operation),
      },
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_FILE_OPERATION_LINES,
        value: JSON.stringify(event.lines),
      },
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_FILE_OPERATION_MIMETYPE,
        value: JSON.stringify(event.mimetype),
      },
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_FILE_OPERATION_EXTENSION,
    ...
    if (event.programming_language) {
      data.push({
        zero_cli_key: EventMetadataKey.ZERO_CLI_PROGRAMMING_LANGUAGE,
  ...
  logApiRequestEvent(event: ApiRequestEvent): void {
    const data: EventValue[] = [
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_API_REQUEST_MODEL,
  ...
  logApiResponseEvent(event: ApiResponseEvent): void {
    const data: EventValue[] = [
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_API_RESPONSE_MODEL,
        value: JSON.stringify(event.model),
      },
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_API_RESPONSE_STATUS_CODE,
        value: JSON.stringify(event.status_code),
      },
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_API_RESPONSE_DURATION_MS,
        value: JSON.stringify(event.duration_ms),
      },
      {
        zero_cli_key:
          EventMetadataKey.ZERO_CLI_API_RESPONSE_INPUT_TOKEN_COUNT,
        value: JSON.stringify(event.usage.input_token_count),
      },
      {
        zero_cli_key:
          EventMetadataKey.ZERO_CLI_API_RESPONSE_OUTPUT_TOKEN_COUNT,
        value: JSON.stringify(event.usage.output_token_count),
      },
      {
        zero_cli_key:
          EventMetadataKey.ZERO_CLI_API_RESPONSE_CACHED_TOKEN_COUNT,
        value: JSON.stringify(event.usage.cached_content_token_count),
      },
      {
        zero_cli_key:
          EventMetadataKey.ZERO_CLI_API_RESPONSE_THINKING_TOKEN_COUNT,
        value: JSON.stringify(event.usage.thoughts_token_count),
      },
      {
        zero_cli_key:
          EventMetadataKey.ZERO_CLI_API_RESPONSE_TOOL_TOKEN_COUNT,
      ...
      {
        zero_cli_key:
          EventMetadataKey.ZERO_CLI_API_RESPONSE_CONTEXT_BREAKDOWN_SYSTEM_INSTRUCTIONS,
      ...
      {
        zero_cli_key:
          EventMetadataKey.ZERO_CLI_API_RESPONSE_CONTEXT_BREAKDOWN_TOOL_DEFINITIONS,
        value: JSON.stringify(
      ...
      {
        zero_cli_key:
          EventMetadataKey.ZERO_CLI_API_RESPONSE_CONTEXT_BREAKDOWN_HISTORY,
        value: JSON.stringify(event.usage.context_breakdown?.history ?? 0),
      },
      {
        zero_cli_key:
          EventMetadataKey.ZERO_CLI_API_RESPONSE_CONTEXT_BREAKDOWN_TOOL_CALLS,
        value: JSON.stringify(event.usage.context_breakdown?.tool_calls ?? {}),
      },
      {
        zero_cli_key:
          EventMetadataKey.ZERO_CLI_API_RESPONSE_CONTEXT_BREAKDOWN_MCP_SERVERS,
  ...
  logApiErrorEvent(event: ApiErrorEvent): void {
    const data: EventValue[] = [
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_API_ERROR_MODEL,
        value: JSON.stringify(event.model),
      },
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_API_ERROR_TYPE,
        value: JSON.stringify(event.error_type),
      },
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_API_ERROR_STATUS_CODE,
        value: JSON.stringify(event.status_code),
      },
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_API_ERROR_DURATION_MS,
        value: JSON.stringify(event.duration_ms),
      },
    ];
  ...
  logChatCompressionEvent(event: ChatCompressionEvent): void {
    const data: EventValue[] = [
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_COMPRESSION_TOKENS_BEFORE,
        value: `${event.tokens_before}`,
      },
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_COMPRESSION_TOKENS_AFTER,
  ...
  logLoopDetectedEvent(event: LoopDetectedEvent): void {
    const data: EventValue[] = [
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_LOOP_DETECTED_TYPE,
    ...
    if (event.confirmed_by_model) {
      data.push({
        zero_cli_key:
          EventMetadataKey.ZERO_CLI_LOOP_DETECTED_CONFIRMED_BY_MODEL,
  ...
  logNextSpeakerCheck(event: NextSpeakerCheckEvent): void {
    const data: EventValue[] = [
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_RESPONSE_FINISH_REASON,
        value: JSON.stringify(event.finish_reason),
      },
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_NEXT_SPEAKER_CHECK_RESULT,
  ...
  logSlashCommandEvent(event: SlashCommandEvent): void {
    const data: EventValue[] = [
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_SLASH_COMMAND_NAME,
    ...
    if (event.subcommand) {
      data.push({
        zero_cli_key: EventMetadataKey.ZERO_CLI_SLASH_COMMAND_SUBCOMMAND,
    ...
    if (event.status) {
      data.push({
        zero_cli_key: EventMetadataKey.ZERO_CLI_SLASH_COMMAND_STATUS,
  ...
  logRewindEvent(event: RewindEvent): void {
    const data: EventValue[] = [
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_REWIND_OUTCOME,
        value: event.outcome,
      },
    ];
  ...
  logMalformedJsonResponseEvent(event: MalformedJsonResponseEvent): void {
    const data: EventValue[] = [
      {
        zero_cli_key:
          EventMetadataKey.ZERO_CLI_MALFORMED_JSON_RESPONSE_MODEL,
        value: JSON.stringify(event.model),
      },
    ];
  ...
  logIdeConnectionEvent(event: IdeConnectionEvent): void {
    const data: EventValue[] = [
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_IDE_CONNECTION_TYPE,
  ...
  logConversationFinishedEvent(event: ConversationFinishedEvent): void {
    const data: EventValue[] = [
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_SESSION_ID,
        value: this.config?.getSessionId() ?? '',
      },
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_CONVERSATION_TURN_COUNT,
        value: JSON.stringify(event.turnCount),
      },
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_APPROVAL_MODE,
  ...
  logInvalidChunkEvent(event: InvalidChunkEvent): void {
    ...
    if (event.error_message) {
      data.push({
        zero_cli_key: EventMetadataKey.ZERO_CLI_INVALID_CHUNK_ERROR_MESSAGE,
  ...
  logContentRetryEvent(event: ContentRetryEvent): void {
    const data: EventValue[] = [
      {
        zero_cli_key:
          EventMetadataKey.ZERO_CLI_CONTENT_RETRY_ATTEMPT_NUMBER,
        value: String(event.attempt_number),
      },
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_CONTENT_RETRY_ERROR_TYPE,
        value: event.error_type,
      },
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_CONTENT_RETRY_DELAY_MS,
  ...
  logContentRetryFailureEvent(event: ContentRetryFailureEvent): void {
    const data: EventValue[] = [
      {
        zero_cli_key:
          EventMetadataKey.ZERO_CLI_CONTENT_RETRY_FAILURE_TOTAL_ATTEMPTS,
        value: String(event.total_attempts),
      },
      {
        zero_cli_key:
          EventMetadataKey.ZERO_CLI_CONTENT_RETRY_FAILURE_FINAL_ERROR_TYPE,
    ...
    if (event.total_duration_ms) {
      data.push({
        zero_cli_key:
          EventMetadataKey.ZERO_CLI_CONTENT_RETRY_FAILURE_TOTAL_DURATION_MS,
  ...
  logNetworkRetryAttemptEvent(event: NetworkRetryAttemptEvent): void {
    // This event is generic for any retry attempt (ZERO, WebFetch, etc.)
    const data: EventValue[] = [
      {
        zero_cli_key:
          EventMetadataKey.ZERO_CLI_NETWORK_RETRY_ATTEMPT_NUMBER,
        value: String(event.attempt),
      },
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_NETWORK_RETRY_DELAY_MS,
        value: String(event.delay_ms),
      },
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_NETWORK_RETRY_ERROR_TYPE,
        value: event.error_type,
      },
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_API_REQUEST_MODEL,
  ...
  async logExtensionInstallEvent(event: ExtensionInstallEvent): Promise<void> {
    const data: EventValue[] = [
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_EXTENSION_NAME,
      ...
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_EXTENSION_VERSION,
        value: event.extension_version,
      },
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_EXTENSION_SOURCE,
        value: event.extension_source,
      },
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_EXTENSION_INSTALL_STATUS,
        value: event.status,
      },
    ];
  ...
  async logExtensionUninstallEvent(
    ...
    const data: EventValue[] = [
      ...
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_EXTENSION_UNINSTALL_STATUS,
        value: event.status,
      },
    ];
  ...
  async logExtensionUpdateEvent(event: ExtensionUpdateEvent): Promise<void> {
    const data: EventValue[] = [
      ...
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_EXTENSION_PREVIOUS_VERSION,
      ...
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_EXTENSION_UPDATE_STATUS,
        value: event.status,
      },
    ];
  ...
  logToolOutputTruncatedEvent(event: ToolOutputTruncatedEvent): void {
    const data: EventValue[] = [
      ...
      {
        zero_cli_key:
          EventMetadataKey.ZERO_CLI_TOOL_OUTPUT_TRUNCATED_ORIGINAL_LENGTH,
        value: JSON.stringify(event.original_content_length),
      },
      {
        zero_cli_key:
          EventMetadataKey.ZERO_CLI_TOOL_OUTPUT_TRUNCATED_TRUNCATED_LENGTH,
        value: JSON.stringify(event.truncated_content_length),
      },
      {
        zero_cli_key:
          EventMetadataKey.ZERO_CLI_TOOL_OUTPUT_TRUNCATED_THRESHOLD,
  ...
  logToolOutputMaskingEvent(event: ToolOutputMaskingEvent): void {
    const data: EventValue[] = [
      {
        zero_cli_key:
          EventMetadataKey.ZERO_CLI_TOOL_OUTPUT_MASKING_TOKENS_BEFORE,
        value: event.tokens_before.toString(),
      },
      {
        zero_cli_key:
          EventMetadataKey.ZERO_CLI_TOOL_OUTPUT_MASKING_TOKENS_AFTER,
        value: event.tokens_after.toString(),
      },
      {
        zero_cli_key:
          EventMetadataKey.ZERO_CLI_TOOL_OUTPUT_MASKING_MASKED_COUNT,
        value: event.masked_count.toString(),
      },
      {
        zero_cli_key:
          EventMetadataKey.ZERO_CLI_TOOL_OUTPUT_MASKING_TOTAL_PRUNABLE_TOKENS,
  ...
  logModelRoutingEvent(event: ModelRoutingEvent): void {
    const data: EventValue[] = [
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_ROUTING_DECISION,
        value: event.decision_model,
      },
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_ROUTING_DECISION_SOURCE,
        value: event.decision_source,
      },
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_ROUTING_LATENCY_MS,
        value: event.routing_latency_ms.toString(),
      },
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_ROUTING_FAILURE,
    ...
    if (event.error_message) {
      data.push({
        zero_cli_key: EventMetadataKey.ZERO_CLI_ROUTING_FAILURE_REASON,
        value: event.error_message,
      });
    }
    ...
    if (event.reasoning && this.config?.getTelemetryLogPromptsEnabled()) {
      data.push({
        zero_cli_key: EventMetadataKey.ZERO_CLI_ROUTING_REASONING,
    ...
    if (event.enable_numerical_routing !== undefined) {
      data.push({
        zero_cli_key: EventMetadataKey.ZERO_CLI_ROUTING_NUMERICAL_ENABLED,
    ...
    if (event.classifier_threshold) {
      data.push({
        zero_cli_key:
          EventMetadataKey.ZERO_CLI_ROUTING_CLASSIFIER_THRESHOLD,
  ...
  async logExtensionEnableEvent(event: ExtensionEnableEvent): Promise<void> {
    const data: EventValue[] = [
      ...
      {
        zero_cli_key:
          EventMetadataKey.ZERO_CLI_EXTENSION_ENABLE_SETTING_SCOPE,
  ...
  logModelSlashCommandEvent(event: ModelSlashCommandEvent): void {
    const data: EventValue[] = [
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_MODEL_SLASH_COMMAND,
  ...
  async logExtensionDisableEvent(event: ExtensionDisableEvent): Promise<void> {
    const data: EventValue[] = [
      ...
      {
        zero_cli_key:
          EventMetadataKey.ZERO_CLI_EXTENSION_DISABLE_SETTING_SCOPE,
        value: event.setting_scope,
      },
    ];
  ...
  logEditStrategyEvent(event: EditStrategyEvent): void {
    const data: EventValue[] = [
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_EDIT_STRATEGY,
  ...
  logEditCorrectionEvent(event: EditCorrectionEvent): void {
    const data: EventValue[] = [
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_EDIT_CORRECTION,
  ...
  logAgentStartEvent(event: AgentStartEvent): void {
    const data: EventValue[] = [
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_AGENT_ID,
        value: event.agent_id,
      },
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_AGENT_NAME,
  ...
  logAgentFinishEvent(event: AgentFinishEvent): void {
    const data: EventValue[] = [
      ...
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_AGENT_DURATION_MS,
        value: event.duration_ms.toString(),
      },
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_AGENT_TURN_COUNT,
        value: event.turn_count.toString(),
      },
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_AGENT_TERMINATE_REASON,
  ...
  logRecoveryAttemptEvent(event: RecoveryAttemptEvent): void {
    const data: EventValue[] = [
      ...
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_AGENT_RECOVERY_REASON,
        value: event.reason,
      },
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_AGENT_RECOVERY_DURATION_MS,
        value: event.duration_ms.toString(),
      },
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_AGENT_RECOVERY_SUCCESS,
  ...
  logWebFetchFallbackAttemptEvent(event: WebFetchFallbackAttemptEvent): void {
    const data: EventValue[] = [
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_WEB_FETCH_FALLBACK_REASON,
        value: event.reason,
      },
    ];
  ...
  logLlmLoopCheckEvent(event: LlmLoopCheckEvent): void {
    const data: EventValue[] = [
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_PROMPT_ID,
        value: event.prompt_id,
      },
      {
        zero_cli_key:
          EventMetadataKey.ZERO_CLI_LLM_LOOP_CHECK_FLASH_CONFIDENCE,
        value: event.flash_confidence.toString(),
      },
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_LLM_LOOP_CHECK_MAIN_MODEL,
        value: event.main_model,
      },
      {
        zero_cli_key:
          EventMetadataKey.ZERO_CLI_LLM_LOOP_CHECK_MAIN_MODEL_CONFIDENCE,
  ...
  logHookCallEvent(event: HookCallEvent): void {
    const data: EventValue[] = [
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_HOOK_EVENT_NAME,
        value: event.hook_event_name,
      },
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_HOOK_DURATION_MS,
        value: event.duration_ms.toString(),
      },
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_HOOK_SUCCESS,
        value: event.success.toString(),
      },
    ];
    ...
    if (event.exit_code !== undefined) {
      data.push({
        zero_cli_key: EventMetadataKey.ZERO_CLI_HOOK_EXIT_CODE,
  ...
  logApprovalModeSwitchEvent(event: ApprovalModeSwitchEvent): void {
    const data: EventValue[] = [
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_ACTIVE_APPROVAL_MODE,
        value: event.from_mode,
      },
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_APPROVAL_MODE_TO,
        value: event.to_mode,
      },
    ];
  ...
  logApprovalModeDurationEvent(event: ApprovalModeDurationEvent): void {
    const data: EventValue[] = [
      ...
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_APPROVAL_MODE_DURATION_MS,
        value: event.duration_ms.toString(),
      },
    ];
  ...
  logKeychainAvailabilityEvent(event: KeychainAvailabilityEvent): void {
    const data: EventValue[] = [
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_KEYCHAIN_AVAILABLE,
  ...
  logTokenStorageInitializationEvent(
    ...
    const data: EventValue[] = [
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_TOKEN_STORAGE_TYPE,
        value: event.type,
      },
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_TOKEN_STORAGE_FORCED,
  ...
  logOnboardingStartEvent(_event: OnboardingStartEvent): void {
    const data: EventValue[] = [
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_ONBOARDING_START,
        value: 'true',
      },
    ];
    this.enqueueLogEvent(
  ...
  logOnboardingSuccessEvent(event: OnboardingSuccessEvent): void {
    ...
    if (event.userTier) {
      data.push({
        zero_cli_key: EventMetadataKey.ZERO_CLI_ONBOARDING_USER_TIER,
        value: event.userTier,
      });
    }
    if (event.duration_ms !== undefined) {
      data.push({
        zero_cli_key: EventMetadataKey.ZERO_CLI_ONBOARDING_DURATION_MS,
        value: event.duration_ms.toString(),
      });
    }
    this.enqueueLogEvent(
  ...
  logStartupStatsEvent(event: StartupStatsEvent): void {
    const data: EventValue[] = [
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_STARTUP_PHASES,
        value: JSON.stringify(event.phases),
      },
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_STARTUP_OS_PLATFORM,
        value: event.os_platform,
      },
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_STARTUP_OS_RELEASE,
        value: event.os_release,
      },
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_STARTUP_IS_DOCKER,
  ...
  logCreditsUsedEvent(event: CreditsUsedEvent): void {
    const data: EventValue[] = [
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_BILLING_MODEL,
        value: JSON.stringify(event.model),
      },
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_BILLING_CREDITS_CONSUMED,
        value: JSON.stringify(event.credits_consumed),
      },
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_BILLING_CREDITS_REMAINING,
  ...
  logOverageOptionSelectedEvent(event: OverageOptionSelectedEvent): void {
    const data: EventValue[] = [
      ...
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_BILLING_SELECTED_OPTION,
        value: JSON.stringify(event.selected_option),
      },
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_BILLING_CREDIT_BALANCE,
  ...
  logCreditPurchaseClickEvent(event: CreditPurchaseClickEvent): void {
    const data: EventValue[] = [
      ...
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_BILLING_PURCHASE_SOURCE,
  ...
  logBrowserAgentConnectionEvent(attrs: {
    ...
    const data: EventValue[] = [
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_BROWSER_AGENT_SESSION_MODE,
        value: attrs.session_mode,
      },
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_BROWSER_AGENT_HEADLESS,
        value: attrs.headless.toString(),
      },
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_BROWSER_AGENT_SUCCESS,
        value: attrs.success.toString(),
      },
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_BROWSER_AGENT_DURATION_MS,
    ...
    if (attrs.error_type) {
      data.push({
        zero_cli_key: EventMetadataKey.ZERO_CLI_BROWSER_AGENT_ERROR_TYPE,
    ...
    if (attrs.tool_count !== undefined) {
      data.push({
        zero_cli_key: EventMetadataKey.ZERO_CLI_BROWSER_AGENT_TOOL_COUNT,
  ...
  logBrowserAgentVisionStatusEvent(attrs: {
    ...
    const data: EventValue[] = [
      {
        zero_cli_key:
          EventMetadataKey.ZERO_CLI_BROWSER_AGENT_VISION_ENABLED,
    ...
    if (attrs.disabled_reason) {
      data.push({
        zero_cli_key:
          EventMetadataKey.ZERO_CLI_BROWSER_AGENT_VISION_DISABLED_REASON,
  ...
  addDefaultFields(data: EventValue[], totalAccounts: number): EventValue[] {
    const defaultLogMetadata: EventValue[] = [
      ...
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_AUTH_TYPE,
        value: JSON.stringify(
          this.config?.getContentGeneratorConfig()?.authType,
        ),
      },
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_GOOGLE_ACCOUNTS_COUNT,
        value: `${totalAccounts}`,
      },
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_PROMPT_ID,
      ...
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_NODE_VERSION,
        value: process.versions.node,
      },
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_USER_SETTINGS,
        value: this.getConfigJson(),
      },
      {
        zero_cli_key: EventMetadataKey.ZERO_CLI_INTERACTIVE,
