...
describe('Telemetry Metrics', () => {
  ...
  describe('recordTokenUsageMetrics', () => {
    ...
    it.each([
      { type: 'input', tokens: 100, model: 'zero-pro' },
      { type: 'output', tokens: 50, model: 'zero-pro' },
      { type: 'thought', tokens: 25, model: 'zero-pro' },
      { type: 'cache', tokens: 75, model: 'zero-pro' },
      { type: 'tool', tokens: 125, model: 'zero-pro' },
      { type: 'input', tokens: 200, model: 'zero-different-model' },
    ])(
  ...
  describe('recordModelRoutingMetrics', () => {
    ...
    it('should record latency for a successful routing decision', () => {
      ...
      expect(mockHistogramRecordFn).toHaveBeenCalledWith(150, {
        ...
        'installation.id': 'test-installation-id',
        'user.email': 'test@example.com',
        'routing.decision_model': 'zero-pro',
    ...
    it('should truncate long reasoning and error_message when ZERO_STRICT_TELEMETRY_LIMITS is true', () => {
      vi.stubEnv('ZERO_STRICT_TELEMETRY_LIMITS', 'true');
      initializeMetricsModule(mockConfig);
    ...
    it('should NOT truncate long reasoning and error_message when ZERO_STRICT_TELEMETRY_LIMITS is false or unset', () => {
      initializeMetricsModule(mockConfig);
      const longReason = 'a'.repeat(2000);
      const longError = 'b'.repeat(2000);
      const event = new ModelRoutingEvent(
    ...
    describe('recordGenAiClientTokenUsage', () => {
      ...
      it('should record input token usage with correct attributes', () => {
        ...
        recordGenAiClientTokenUsageModule(mockConfig, 150, {
          ...
          'gen_ai.provider.name': 'gcp.gen_ai',
          'gen_ai.token.type': 'input',
          'gen_ai.request.model': 'zero-2.0-flash',
          'gen_ai.response.model': 'zero-2.0-flash',
        });
    ...
    describe('recordGenAiClientOperationDuration', () => {
      ...
      it('should record operation duration with server details', () => {
        ...
        recordGenAiClientOperationDurationModule(mockConfig, 0.95, {
          'gen_ai.operation.name': 'generate_content',
          'gen_ai.provider.name': 'gcp.vertex_ai',
          'gen_ai.request.model': 'zero-1.5-pro',
          'gen_ai.response.model': 'zero-1.5-pro-001',
    ...
    describe('recordHookCallMetrics', () => {
      ...
      it('should always sanitize hook names regardless of content', () => {
        ...
        recordHookCallMetricsModule(
          mockConfig,
          'BeforeTool',
          '/path/to/.zero/hooks/check-secrets.sh --api-key=abc123',
          150,
          true,
        );
