...
export async function resolveTelemetrySettings(options: {
  ...
  const enabled =
    argv.telemetry ??
    parseBooleanEnvFlag(env['ZERO_TELEMETRY_ENABLED']) ??
    settings.enabled;

  const traces =
    parseBooleanEnvFlag(env['ZERO_TELEMETRY_TRACES_ENABLED']) ??
    settings.traces;

  const rawTarget =
    argv.telemetryTarget ??
    env['ZERO_TELEMETRY_TARGET'] ??
  ...
  const otlpEndpoint =
    ...
    env['ZERO_TELEMETRY_OTLP_ENDPOINT'] ??
    env['OTEL_EXPORTER_OTLP_ENDPOINT'] ??
    settings.otlpEndpoint;

  const rawProtocol =
    argv.telemetryOtlpProtocol ??
    env['ZERO_TELEMETRY_OTLP_PROTOCOL'] ??
  ...
  const logPrompts =
    ...
    parseBooleanEnvFlag(env['ZERO_TELEMETRY_LOG_PROMPTS']) ??
    settings.logPrompts;

  const outfile =
    argv.telemetryOutfile ??
    env['ZERO_TELEMETRY_OUTFILE'] ??
    settings.outfile;

  const useCollector =
    parseBooleanEnvFlag(env['ZERO_TELEMETRY_USE_COLLECTOR']) ??
  ...
  return {
    ...
    useCollector,
    useCliAuth:
      parseBooleanEnvFlag(env['ZERO_TELEMETRY_USE_CLI_AUTH']) ??
      settings.useCliAuth,
  };
}
