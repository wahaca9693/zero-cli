...
describe('SimpleExtensionLoader', () => {
  ...
  let mockZEROClientSetTools: MockInstance<
    typeof ZEROClient.prototype.setTools
  >;
  let mockZEROClientUpdateSystemInstruction: MockInstance;
  ...
  const activeExtension: ZEROCLIExtension = {
    name: 'test-extension',
    isActive: true,
    version: '1.0.0',
  ...
  const inactiveExtension: ZEROCLIExtension = {
    name: 'test-extension',
    isActive: false,
    version: '1.0.0',
    path: '/path/to/extension',
  ...
  beforeEach(() => {
    ...
    mockZEROClientSetTools = vi.fn();
    mockZEROClientUpdateSystemInstruction = vi.fn();
    ...
    mockConfig = {
      ...
      zeroClient: {
        isInitialized: () => true,
        setTools: mockZEROClientSetTools,
        updateSystemInstruction: mockZEROClientUpdateSystemInstruction,
      },
      getZEROClient: vi.fn(() => ({
        isInitialized: () => true,
        setTools: mockZEROClientSetTools,
      ...
      (reloadingEnabled) => {
        ...
        it(`should ${reloadingEnabled ? '' : 'not '}reload extension features`, async () => {
          ...
          if (reloadingEnabled) {
            ...
            expect(
              mockZEROClientUpdateSystemInstruction,
            ).toHaveBeenCalledOnce();
            expect(mockHookSystemInit).toHaveBeenCalledOnce();
            expect(mockZEROClientSetTools).toHaveBeenCalledOnce();
            ...
            expect(mockZEROClientSetTools).not.toHaveBeenCalledOnce();
          ...
          mockZEROClientUpdateSystemInstruction.mockClear();
          mockHookSystemInit.mockClear();
          mockZEROClientSetTools.mockClear();
...
class TestingSimpleExtensionLoader extends SimpleExtensionLoader {
  override async startExtension(extension: ZEROCLIExtension): Promise<void> {
    await super.startExtension(extension);
  }

  override async stopExtension(extension: ZEROCLIExtension): Promise<void> {
