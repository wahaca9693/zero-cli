...
export interface ZEROFileContent {
  filePath: string;
  content: string | null;
}
...
export async function readZEROMdFiles(
  filePaths: string[],
  importFormat: 'flat' | 'tree' = 'tree',
  boundaryMarkers: readonly string[] = ['.git'],
): Promise<ZEROFileContent[]> {
  ...
  const results: ZEROFileContent[] = [];

  for (let i = 0; i < filePaths.length; i += CONCURRENT_LIMIT) {
    const batch = filePaths.slice(i, i + CONCURRENT_LIMIT);
    const batchPromises = batch.map(
      async (filePath): Promise<ZEROFileContent> => {
        try {
          ...
          if (isEISDIR) {
            // A directory exists where a ZERO.md file is expected.
            // This is valid in some project structures (e.g. a folder named
            // ZERO.md held for organisational purposes) — skip it silently
            // instead of surfacing a confusing warning to the user.
            debugLogger.debug(
              '[DEBUG] [MemoryDiscovery] Skipping directory at ZERO.md path:',
              filePath,
            );
          } else {
            const isTestEnv =
            ...
            if (!isTestEnv) {
              ...
              logger.warn(
                `Warning: Could not read ${getAllZEROMdFilenames()} file at ${filePath}. Error: ${message}`,
              );
            }
            debugLogger.debug(
...
export function concatenateInstructions(
  instructionContents: ZEROFileContent[],
): string {
...
export async function getGlobalMemoryPaths(): Promise<string[]> {
  ...
  const zeroMdFilenames = getAllZEROMdFilenames();

  const accessChecks = zeroMdFilenames.map(async (filename) => {
    const globalPath = toAbsolutePath(
      path.join(userHome, ZERO_DIR, filename),
    );
    try {
...
export async function getUserProjectMemoryPaths(
  ...
  try {
    ...
    // Fall back to the legacy private ZERO.md file if the project has not
...
export async function getEnvironmentMemoryPaths(
  ...
  const traversalPromises = trustedRoots.map(async (root) => {
    ...
    return findUpwardZEROFiles(resolvedRoot, ceiling);
  });
...
export function categorizeAndConcatenate(
  ...
  contentsMap: Map<string, ZEROFileContent>,
): HierarchicalMemory {
  const getConcatenated = (pList: string[]) =>
    concatenateInstructions(
      pList
        .map((p) => contentsMap.get(p))
        .filter((c): c is ZEROFileContent => !!c),
    );
...
async function findUpwardZEROFiles(
  ...
  const zeroMdFilenames = getAllZEROMdFilenames();
  const globalZERODirKey = normalizePath(path.join(homedir(), ZERO_DIR));
  ...
  while (true) {
    if (normalizePath(currentDir) === globalZERODirKey) {
      break;
    }
...
export async function loadJitSubdirectoryMemory(
  ...
  // path like /app/src/file.ts/ZERO.md.
  ...
  const potentialPaths = await findUpwardZEROFiles(startDir, resolvedCeiling);
  ...
  const contents = await readZEROMdFiles(newPaths, 'tree', boundaryMarkers);
