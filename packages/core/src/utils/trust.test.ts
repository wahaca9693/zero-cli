...
describe('Trust Utility (Core)', () => {
  const tempDir = path.join(
    os.tmpdir(),
    'zero-trust-test-' + Math.random().toString(36).slice(2),
  );
  ...
  it('should handle TRUST_PARENT', () => {
    const config = {
      [path.resolve('/project/.zero')]: TrustLevel.TRUST_PARENT,
    };
    fs.writeFileSync(trustedFoldersPath, JSON.stringify(config));
    ...
    expect(
      folders.isPathTrusted(path.resolve('/project/.zero/config.yaml')),
    ).toBe(true);
  });
