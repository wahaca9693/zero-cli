...
import {
  ...
  unescapeStringForZEROBug,
  resetEditCorrectorCaches_TEST_ONLY,
} from './editCorrector.js';

describe('editCorrector', () => {
  describe('unescapeStringForZEROBug', () => {
    it('should unescape common sequences', () => {
      expect(unescapeStringForZEROBug('\\n')).toBe('\n');
      expect(unescapeStringForZEROBug('\\t')).toBe('\t');
      expect(unescapeStringForZEROBug("\\'")).toBe("'");
      expect(unescapeStringForZEROBug('\\"')).toBe('"');
      expect(unescapeStringForZEROBug('\\`')).toBe('`');
    });
    it('should handle multiple escaped sequences', () => {
      expect(unescapeStringForZEROBug('Hello\\nWorld\\tTest')).toBe(
        'Hello\nWorld\tTest',
      );
    });
    it('should not alter already correct sequences', () => {
      expect(unescapeStringForZEROBug('\n')).toBe('\n');
      expect(unescapeStringForZEROBug('Correct string')).toBe(
        'Correct string',
      );
    });
    ...
    it('should handle backslash followed by actual newline character', () => {
      expect(unescapeStringForZEROBug('\\\n')).toBe('\n');
      expect(unescapeStringForZEROBug('First line\\\nSecond line')).toBe(
        'First line\nSecond line',
      );
    });
    ...
    it('should not alter strings with no targeted escape sequences', () => {
      expect(unescapeStringForZEROBug('abc def')).toBe('abc def');
      expect(unescapeStringForZEROBug('C:\\Folder\\File')).toBe(
        'C:\\Folder\\File',
      );
    });
    it('should correctly process strings with some targeted escapes', () => {
      expect(unescapeStringForZEROBug('C:\\Users\\name')).toBe(
        'C:\\Users\name',
      );
    });
    ...
    it('should handle escaped backslashes', () => {
      expect(unescapeStringForZEROBug('\\\\')).toBe('\\');
      expect(unescapeStringForZEROBug('C:\\\\Users')).toBe('C:\\Users');
      expect(unescapeStringForZEROBug('path\\\\to\\\\file')).toBe(
        'path\to\\file',
      );
    });
    it('should handle escaped backslashes mixed with other escapes (aggressive unescaping)', () => {
      expect(unescapeStringForZEROBug('line1\\\\\\nline2')).toBe(
        'line1\nline2',
      );
      expect(unescapeStringForZEROBug('quote\\\\"text\\\\nline')).toBe(
        'quote"text\nline',
      );
    });
  });
  ...
  describe('ensureCorrectFileContent', () => {
    ...
    it('should return unescaped content when LLM is disabled and aggressiveUnescape is true', async () => {
      ...
      // unescapeStringForZEROBug would change \\\\title to \title (literal tab and "itle")
