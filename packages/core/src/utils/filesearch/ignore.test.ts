...
describe('loadIgnoreRules', () => {
  ...
  it('should load rules from .zeroignore', async () => {
    tmpDir = await createTmpDir({
      [ZERO_IGNORE_FILE_NAME]: '*.log',
    });
    const service = new FileDiscoveryService(tmpDir, {
  ...
  it('should combine rules from .gitignore and .zeroignore', async () => {
    tmpDir = await createTmpDir({
      '.git': {},
      '.gitignore': '*.log',
      [ZERO_IGNORE_FILE_NAME]: '*.txt',
    });
    const service = new FileDiscoveryService(tmpDir, {
      respectGitIgnore: true,
      respectZEROIgnore: true,
