...
describe('modelUtils', () => {
  describe('normalizeModelId', () => {
    it('should strip "models/" prefix if present', () => {
      expect(normalizeModelId('models/zero-3.1-pro-preview')).toBe(
        'zero-3.1-pro-preview',
      );
      expect(normalizeModelId('models/zero-1.5-flash')).toBe(
        'zero-1.5-flash',
      );
    });

    it('should leave model ID untouched if prefix is not present', () => {
      expect(normalizeModelId('zero-3.1-pro-preview')).toBe(
        'zero-3.1-pro-preview',
      );
