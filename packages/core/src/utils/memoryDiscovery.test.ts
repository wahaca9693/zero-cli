...
import type { ZEROCLIExtension } from '../config/config.js';
...
describe('memoryDiscovery', () => {
  ...
  describe('EISDIR handling for ZERO.md as a directory', () => {
    it('readZEROMdFiles returns null content (without throwing) when path is a directory', async () => {
      const dirAsFilePath = await createEmptyDir(
        path.join(projectRoot, DEFAULT_CONTEXT_FILENAME),
      );

      const results = await readZEROMdFiles([dirAsFilePath]);
  ...
  describe('getGlobalMemoryPaths', () => {
    it('should find global memory file if it exists', async () => {
      const globalMemoryFile = await createTestFile(
        path.join(homedir, ZERO_DIR, DEFAULT_CONTEXT_FILENAME),
  ...
  describe('getUserProjectMemoryPaths', () => {
    ...
    it('should fall back to legacy ZERO.md when MEMORY.md is absent', async () => {
    ...
    it('should return empty array when neither MEMORY.md nor ZERO.md exists', async () => {
  ...
  describe('getExtensionMemoryPaths', () => {
    it('should return active extension context files', async () => {
      const extFile = await createTestFile(
        path.join(testRootDir, 'ext', 'ZERO.md'),
        'Extension content',
      );
  ...
  describe('getEnvironmentMemoryPaths', () => {
    ...
    it('should fall back to trusted root as ceiling when no .git exists', async () => {
      ...
      // notesDir has no ZERO.md and won't traverse up to docsDir.
      const resultNotes = await getEnvironmentMemoryPaths([notesDir]);
      expect(resultNotes).toHaveLength(0);

      // docsDir has a ZERO.md at the trusted root itself, so it's found.
    ...
    it('should preserve case-distinct files before identity deduplication', async () => {
      ...
      vi.doMock('node:fs/promises', async () => {
        ...
        return {
          ...
          stat: vi.fn(async (filePath) => {
            ...
            return {
              dev: 1,
              ino: normalizedPath.endsWith('/ZERO.md') ? 101 : 202,
            };
          }),
        };
      });
      ...
      try {
        ...
        memoryTool.setZEROMdFilename(['ZERO.md', 'zero.md']);
        ...
        expect(result).toEqual([
          paths.toAbsolutePath('/case-root/ZERO.md'),
          paths.toAbsolutePath('/case-root/zero.md'),
        ]);
      } finally {
    ...
    it('should keep multiple memory files from the same directory adjacent and in order', async () => {
      ...
      setZEROMdFilename(['PRIMARY.md', 'SECONDARY.md']);
  ...
  describe('file identity deduplication', () => {
    it('should deduplicate files that point to the same inode (same physical file)', async () => {
      const zeroFile = await createTestFile(
        path.join(projectRoot, 'zero.md'),
        'Project root memory',
      );

      // create hard link to simulate case-insensitive filesystem behavior
      const zeroFileLink = path.join(projectRoot, 'ZERO.md');
      try {
        await fsPromises.link(zeroFile, zeroFileLink);
      } catch (error) {
        const errorMessage =
      ...
      const stats1 = await fsPromises.lstat(zeroFile);
      const stats2 = await fsPromises.lstat(zeroFileLink);
      ...
      expect(result.identityMap.get(zeroFile)).toBe(
        result.identityMap.get(zeroFileLink),
      );

      try {
        await fsPromises.unlink(zeroFileLink);
      } catch {
    ...
    it('should handle case where files have different inodes (different files)', async () => {
      const zeroFileLower = await createTestFile(
        path.join(projectRoot, 'zero.md'),
        'Lowercase file content',
      );
      const zeroFileUpper = await createTestFile(
        path.join(projectRoot, 'ZERO.md'),
        'Uppercase file content',
      );

      const stats1 = await fsPromises.lstat(zeroFileLower);
      const stats2 = await fsPromises.lstat(zeroFileUpper);
      ...
      if (stats1.ino !== stats2.ino || stats1.dev !== stats2.dev) {
        ...
        expect(result.paths).toContain(zeroFileLower);
        expect(result.paths).toContain(zeroFileUpper);
      }
    });
    ...
    it("should handle files that cannot be stat'd (missing files)", async () => {
      ...
      expect(result.paths).toEqual([zeroFile, missingFile]);
    ...
    it('should deduplicate multiple paths pointing to same file (3+ duplicates)', async () => {
      ...
      const link1 = path.join(projectRoot, 'ZERO.md');
      const link2 = path.join(projectRoot, 'ZERO.md');

      try {
        await fsPromises.link(zeroFile, link1);
        await fsPromises.link(zeroFile, link2);
      } catch (error) {
        const errorMessage =
          error instanceof Error ? error.message : String(error);
        if (
  ...
  describe('loadJitSubdirectoryMemory', () => {
    ...
    it('should deduplicate files in JIT memory loading (same inode)', async () => {
      ...
      const zeroFile = await createTestFile(
        path.join(subDir, 'zero.md'),
        'JIT memory content',
      );

      const zeroFileLink = path.join(subDir, 'ZERO.md');
      try {
        await fsPromises.link(zeroFile, zeroFileLink);
      } catch (error) {
        const errorMessage =
      ...
      setZEROMdFilename(['zero.md', 'ZERO.md']);
    ...
    it('should resolve file target to its parent directory for traversal', async () => {
      ...
      // Should find the ZERO.md in the same directory as the file
      expect(result.files).toHaveLength(1);
      expect(result.files[0].path).toBe(subDirMemory);
    ...
    it('should fall back to trusted root as ceiling when no git root exists', async () => {
      ...
      // subDir is within the trusted root, so its ZERO.md is found
      expect(result.files).toHaveLength(1);
      expect(result.files[0].path).toBe(subDirMemory);
