...
import { ZERO_DIR } from './paths.js';
import { ZERO_IGNORE_FILE_NAME } from 'src/config/constants.js';
...
`.trim();
  ...
  describe('with gitignore', () => {
    ...
    it('should ignore files and folders specified in .gitignore', async () => {
      ...
      await createTestFile(ZERO_DIR, 'config.yaml');
      await createTestFile(ZERO_DIR, 'logs.json');
  ...
  describe('with zeroignore', () => {
    it('should ignore zeroignore files by default', async () => {
      await fsPromises.writeFile(
    ...
    it('should not ignore files if respectZEROIgnore is false', async () => {
      await fsPromises.writeFile(
        path.join(testRootDir, ZERO_IGNORE_FILE_NAME),
        'ignored.txt\nnode_modules/\n.zero/\n!/.zero/config.yaml',
      );
      ...
      const structure = await getFolderStructure(testRootDir, {
        ...
        fileFilteringOptions: {
          respectZEROIgnore: false,
          respectGitIgnore: true, // Explicitly disable zero ignore only
          customIgnoreFilePaths: [],
        },
      });
      expect(structure).toContain('ignored.txt');
