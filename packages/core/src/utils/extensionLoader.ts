...
import type { Config, ZEROCLIExtension } from '../config/config.js';
...
export abstract class ExtensionLoader {
   ...
   */
  protected async startExtension(extension: ZEROCLIExtension) {
    if (!this.config) {
    ...
    try {
      ...
      await this.maybeRefreshZEROTools(extension);
  ...
  private async maybeRefreshMemories(): Promise<void> {
    if (!this.config) {
      throw new Error(
        'Cannot refresh zero memories prior to calling `start`.',
      );
    }
    if (
      ...
      this.config.getZEROClient().updateSystemInstruction();
  ...
  /**
   * Refreshes the zero tools list if it is initialized and the extension has
   * any excludeTools settings.
   */
  private async maybeRefreshZEROTools(
    extension: ZEROCLIExtension,
  ): Promise<void> {
    if (extension.excludeTools && extension.excludeTools.length > 0) {
      const zeroClient = this.config?.zeroClient;
      if (zeroClient?.isInitialized()) {
        await zeroClient.setTools();
      }
    }
  }
   ...
   * initialized.
   */
  protected async stopExtension(extension: ZEROCLIExtension) {
    if (!this.config) {
  ...
  async restartExtension(extension: ZEROCLIExtension): Promise<void> {
    await this.stopExtension(extension);
    await this.startExtension(extension);
  }
}
...
export class SimpleExtensionLoader extends ExtensionLoader {
  constructor(
    protected readonly extensions: ZEROCLIExtension[],
  ...
  async loadExtension(extension: ZEROCLIExtension) {
  ...
  async unloadExtension(extension: ZEROCLIExtension) {
