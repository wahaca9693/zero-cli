...
import { getCurrentZEROMdFilename } from '../tools/memoryTool.js';
...
export interface ExcludeOptions {
  ...
  /**
   * Whether to include dynamic patterns like the current ZERO MD filename. Defaults to true.
   */
...
export class FileExclusions {
  ...
  getDefaultExcludePatterns(options: ExcludeOptions = {}): string[] {
    ...
    // Add dynamic patterns (like current ZERO MD filename)
    if (includeDynamicPatterns) {
      patterns.push(`**/${getCurrentZEROMdFilename()}`);
    }
