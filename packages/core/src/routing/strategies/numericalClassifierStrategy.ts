...
import { resolveClassifierModel, isZERO3Model } from '../../config/models.js';
...
export class NumericalClassifierStrategy implements RoutingStrategy {
  ...
  async route(
    ...
    try {
      ...
      const [useZERO3_1, useCustomToolModel] = await Promise.all([
        config.getZERO31Launched(),
