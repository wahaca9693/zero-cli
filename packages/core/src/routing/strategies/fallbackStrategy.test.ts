...
const createMockConfig = (overrides: Partial<Config> = {}): Config =>
  ({
    ...
    getModel: vi.fn().mockReturnValue(DEFAULT_ZERO_MODEL),
    ...overrides,
...
describe('FallbackStrategy', () => {
  ...
  it('should return null if the requested model is available', async () => {
    ...
    // Should check availability of the resolved model (DEFAULT_ZERO_MODEL)
    expect(mockService.snapshot).toHaveBeenCalledWith(DEFAULT_ZERO_MODEL);
  });
  ...
  it('should return null if fallback selection is same as requested model', async () => {
    ...
    vi.mocked(selectModelForAvailability).mockReturnValue({
      selectedModel: DEFAULT_ZERO_MODEL,
      skipped: [],
    });
  ...
  it('should return fallback decision if model is unavailable and fallback found', async () => {
    ...
    vi.mocked(selectModelForAvailability).mockReturnValue({
      selectedModel: DEFAULT_ZERO_FLASH_MODEL,
      skipped: [{ model: DEFAULT_ZERO_MODEL, reason: 'quota' }],
    });
    ...
    expect(decision?.metadata.reasoning).toContain(
      `Model ${DEFAULT_ZERO_MODEL} is unavailable`,
    );
  });
