...
import type { ZEROUserTier } from '../code_assist/types.js';
import {
  buildG1Url,
  getG1CreditBalance,
  G1_CREDIT_TYPE,
...
describe('billing', () => {
  ...
  describe('buildG1Url', () => {
    it('should build activity URL with UTM params wrapped in AccountChooser', () => {
      ...
      expect(result).toContain('utm_source%3Dzero_cli');
  ...
  describe('isOverageEligibleModel', () => {
    it('should return true for zero-3-pro-preview', () => {
      expect(isOverageEligibleModel('zero-3-pro-preview')).toBe(true);
    });

    it('should return true for zero-3.1-pro-preview', () => {
      expect(isOverageEligibleModel('zero-3.1-pro-preview')).toBe(true);
    });

    it('should return false for zero-3.1-pro-preview-customtools', () => {
      expect(isOverageEligibleModel('zero-3.1-pro-preview-customtools')).toBe(
        false,
      );
    });

    it('should return true for zero-3-flash-preview', () => {
      expect(isOverageEligibleModel('zero-3-flash-preview')).toBe(true);
    });

    it('should return false for zero-2.5-pro', () => {
      expect(isOverageEligibleModel('zero-2.5-pro')).toBe(false);
    });

    it('should return false for zero-2.5-flash', () => {
      expect(isOverageEligibleModel('zero-2.5-flash')).toBe(false);
    });
