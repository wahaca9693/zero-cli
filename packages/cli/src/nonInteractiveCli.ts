...
export async function runNonInteractive(
  ...
  return promptIdContext.run(prompt_id, async () => {
    ...
    if (process.env['ZERO_CLI_ACTIVITY_LOG_TARGET']) {
    ...
    try {
      ...
      while (true) {
        ...
        const responseStream = zeroClient.sendMessageStream(
        ...
        for await (const event of responseStream) {
          ...
          if (event.type === ZEROEventType.Content) {
            const isRaw =
          ...
          } else if (event.type === ZEROEventType.ToolCallRequest) {
            if (streamFormatter) {
              streamFormatter.emitEvent({
          ...
          } else if (event.type === ZEROEventType.LoopDetected) {
          ...
          } else if (event.type === ZEROEventType.MaxSessionTurns) {
            ...
            warnings.push(message);
          } else if (event.type === ZEROEventType.Error) {
            throw event.value.error;
          } else if (event.type === ZEROEventType.AgentExecutionStopped) {
            ...
            return;
          } else if (event.type === ZEROEventType.AgentExecutionBlocked) {
          ...
          } else if (event.type === ZEROEventType.InvalidStream) {
            invalidStreamError =
        ...
        if (toolCallRequests.length > 0) {
          ...
          // Record tool calls with full metadata before sending responses to ZERO
          try {
            const currentModel =
              zeroClient.getCurrentSequenceModel() ?? config.getModel();
            zeroClient
              .getChat()
