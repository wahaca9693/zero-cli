...
describe('migrate command', () => {
  ...
  beforeEach(() => {
    ...
    mockedLoadSettings.mockReturnValue({
      ...
      workspace: { path: '/test/project/.zero' },
    });
  });
  ...
  it('should migrate hooks from settings.json when it exists', async () => {
    ...
    expect(debugLoggerLogSpy).toHaveBeenCalledWith(
      '✓ Hooks successfully migrated to .zero/settings.json',
    );
  });
  ...
  it('should replace $CLAUDE_PROJECT_DIR with $ZERO_PROJECT_DIR', async () => {
    const claudeSettings = {
      hooks: {
        PreToolUse: [
          {
  ...
  it('should merge with existing ZERO hooks', async () => {
    const claudeSettings = {
      hooks: {
        PreToolUse: [
          {
