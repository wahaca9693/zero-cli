...
import { debugLogger, getErrorMessage } from '@allhands/zero-cli-core';
...
/**
 * Mapping from Claude Code event names to ZERO event names
 */
const EVENT_MAPPING: Record<string, string> = {
  ...
  Stop: 'AfterAgent',
  SubAgentStop: 'AfterAgent', // ZERO doesn't have sub-agents, map to AfterAgent
...
/**
 * Mapping from Claude Code tool names to ZERO tool names
 */
...
/**
 * Transform a matcher regex to update tool names from Claude to ZERO
 */
function transformMatcher(matcher: string | undefined): string | undefined {
  if (!matcher) return matcher;

  let transformed = matcher;
  for (const [claudeName, zeroName] of Object.entries(TOOL_NAME_MAPPING)) {
...
/**
 * Migrate a Claude Code hook configuration to ZERO format
 */
function migrateClaudeHook(claudeHook: unknown): unknown {
  ...
  if ('command' in hook) {
    ...
    // Replace CLAUDE_PROJECT_DIR with ZERO_PROJECT_DIR in command
  ...
  // Map timeout field (Claude uses seconds, ZERO uses seconds)
  // eslint-disable-next-line no-restricted-syntax
...
/**
 * Migrate Claude Code hooks configuration to ZERO format
 */
function migrateClaudeHooks(claudeConfig: unknown): Record<string, unknown> {
  ...
  const zeroHooks: Record<string, unknown> = {};
  ...
  for (const [eventName, eventConfig] of Object.entries(hooksSection)) {
    // Map event name
    const zeroEventName = EVENT_MAPPING[eventName] || eventName;
    ...
    zeroHooks[zeroEventName] = migratedDefinitions;
  }
...
export async function handleMigrateFromClaude() {
  ...
  try {
    ...
    debugLogger.log('✓ Hooks successfully migrated to .zero/settings.json');
    debugLogger.log(
      '\nMigration complete! Please review the migrated hooks in .zero/settings.json',
    );
  } catch (error) {
...
export const migrateCommand: CommandModule = {
  command: 'migrate',
  describe: 'Migrate hooks from Claude Code to ZERO CLI',
  builder: (yargs) =>
  ...
  handler: async (argv) => {
    ...
    if (args.fromClaude) {
      ...
      debugLogger.log(
        'Usage: zero hooks migrate --from-claude\n\nMigrate hooks from Claude Code to ZERO CLI format.',
      );
    }
    await exitCli();
  },
