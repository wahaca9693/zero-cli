...
- `zero-extension.json`: The manifest file.
- `policies/`: Contains the `.toml` policy files.

## How to use

1.  Link this extension to your local ZERO CLI installation:

    ```bash
    zero extensions link packages/cli/src/commands/extensions/examples/policies
    ```

2.  Restart your ZERO CLI session.
...
For security, ZERO CLI ignores any `allow` decisions or `yolo` mode
