...
async function handleSetup(argv: SetupArgs): Promise<number> {
  ...
  try {
    ...
    logError(
      '  You can manually add the configuration to ~/.zero/settings.json',
    );
  }
  ...
  if (argv.start) {
    ...
    if (serverStarted) {
      ...
      log(
        chalk.yellow(
          `  ! Server may not have started correctly. Check: zero gemma status`,
        ),
      );
    }
  }
  ...
  if (!settingsUpdated) {
    log(
      chalk.yellow(
        '  Fix the settings update above, then rerun "zero gemma status".',
      ),
    );
    log('');
  } else if (!argv.start) {
    log(chalk.yellow('  Note: Run "zero gemma start" to start the server.'));
    if (autoStartServer) {
      log(
        chalk.yellow(
    ...
    log(
      chalk.yellow(
        '  Review the server logs and rerun "zero gemma start" after fixing the issue.',
      ),
    );
    log('');
  }
  log('  Useful commands:');
  log(chalk.dim('    zero gemma status   Check routing status'));
  log(chalk.dim('    zero gemma start    Start the LiteRT server'));
  log(chalk.dim('    zero gemma stop     Stop the LiteRT server'));
