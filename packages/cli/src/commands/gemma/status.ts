...
export function formatGemmaStatus(status: GemmaStatusResult): string {
  ...
  if (status.binaryInstalled) {
    ...
    if (platform) {
      ...
      lines.push(chalk.dim(`             Run: zero gemma setup`));
    } else {
      lines.push(
  ...
  if (status.serverRunning) {
    ...
    lines.push(chalk.dim(`             Run: zero gemma start`));
  }
  ...
  if (status.settingsEnabled) {
    ...
    lines.push(
      chalk.dim(
        `             Run: zero gemma setup (auto-configures settings)`,
      ),
    );
  }
  ...
  if (status.allPassing) {
    ...
    lines.push(
      chalk.yellow(
        '  Some checks failed. Run "zero gemma setup" for guided installation.',
      ),
    );
  }
