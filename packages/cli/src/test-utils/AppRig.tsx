...
// Mock ZERORespondingSpinner to disable animations (avoiding 'act()' warnings) without triggering screen reader mode.
vi.mock('../ui/components/ZERORespondingSpinner.js', async () => {
  const React = await import('react');
  const { Text } = await import('ink');
  return {
    ZEROSpinner: () => React.createElement(Text, null, '...'),
    ZERORespondingSpinner: ({
...
export class AppRig {
  ...
  constructor(private options: AppRigOptions = {}) {
    ...
    this.testDir = fs.mkdtempSync(
      path.join(os.tmpdir(), `zero-app-rig-${uniqueId.slice(0, 8)}-`),
    );
    ...
    const zeroDir = path.join(this.testDir, '.zero');
    if (!fs.existsSync(zeroDir)) {
      fs.mkdirSync(zeroDir, { recursive: true });
    }
    fs.writeFileSync(
      path.join(zeroDir, 'state.json'),
  ...
  async initialize() {
    ...
    await act(async () => {
      ...
      await this.config!.refreshAuth(AuthType.USE_ZERO);
    });
  }

  private setupEnvironment() {
    // Stub environment variables to avoid interference from developer's machine
    vi.stubEnv('ZERO_CLI_HOME', this.testDir);
    ...
    if (this.options.fakeResponsesPath) {
      ...
      if (!process.env['ZERO_API_KEY']) {
        throw new Error(
          'ZERO_API_KEY must be set in the environment for live model tests.',
        );
      }
    ...
    vi.stubEnv('ZERO_DEFAULT_AUTH_TYPE', AuthType.USE_ZERO);
  }

  private createRigSettings(): LoadedSettings {
    return createMockSettings({
      user: {
        path: path.join(this.testDir, '.zero', 'user_settings.json'),
        settings: {
          security: {
            auth: {
              selectedType: AuthType.USE_ZERO,
              useExternal: true,
            },
            folderTrust: {
              enabled: true,
  ...
  private stubRefreshAuth() {
    ...
    gcConfig.refreshAuth = async (authMethod: AuthType) => {
      ...
      const newContentGeneratorConfig = {
        ...
        apiKey: process.env['ZERO_API_KEY'] || 'test-api-key',
      };
