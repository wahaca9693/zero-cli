...
for the ZERO CLI. The ACP allows external clients (like IDE extensions) to
communicate with the ZERO CLI agent over a structured JSON-RPC based protocol.
...
- **[acpRpcDispatcher.ts](./acpRpcDispatcher.ts)**: Contains the `ZEROAgent`
...
```bash
# General pattern
npm test -w @google/zero-cli -- src/acp/<test-file-name>.ts

# Example
npm test -w @google/zero-cli -- src/acp/acpRpcDispatcher.test.ts
```
...
```bash
source ~/.nvm/nvm.sh && nvm use default && npm test -w @google/zero-cli -- src/acp/acpSession.test.ts
```

### Adding New Features

- **New RPC Method**: Add the method to `ZEROAgent` in `acpRpcDispatcher.ts`
