...
import type { FileSystemService } from '@allhands/zero-cli-core';
...
describe('AcpFileSystemService', () => {
  ...
  describe('readTextFile', () => {
    it.each([
      ...
      {
        capability: true,
        path: '/home/user/.zero/tmp/file.md',
        root: '/home/user',
        desc: 'fallback if file is inside global zero dir, even if root overlaps',
        setup: () => {
          mockFallback.readTextFile.mockResolvedValue('content');
        },
        verify: () => {
          expect(mockFallback.readTextFile).toHaveBeenCalledWith(
            '/home/user/.zero/tmp/file.md',
          );
          expect(mockConnection.readTextFile).not.toHaveBeenCalled();
        },
      },
