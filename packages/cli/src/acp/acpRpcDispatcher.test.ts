...
import { ZEROAgent } from './acpRpcDispatcher.js';
...
describe('ZEROAgent - RPC Dispatcher', () => {
  ...
  beforeEach(() => {
    mockConfig = {
      ...
      getZEROClient: vi.fn().mockReturnValue({
      ...
      getZERO31LaunchedSync: vi.fn().mockReturnValue(false),
    ...
    agent = new ZEROAgent(mockConfig, mockSettings, mockArgv, mockConnection);
  });
  ...
  it('should initialize correctly', async () => {
    ...
    const zeroAuth = response.authMethods?.find(
      (m) => m.id === AuthType.USE_ZERO,
    );
    expect(zeroAuth?._meta).toEqual({
      'api-key': {
        provider: 'google',
      },
    });
  ...
  it('should delegate setModel to session (unstable)', async () => {
    ...
    const result = await agent.unstable_setSessionModel({
      sessionId: 'test-session-id',
      modelId: 'zero-2.0-pro-exp',
    });

    expect(mockSession.setModel).toHaveBeenCalledWith('zero-2.0-pro-exp');
    expect(result).toEqual({});
  });
