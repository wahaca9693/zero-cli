...
export class AcpSessionManager {
  ...
  async newSession(
    ...
    const authType =
      ...
      (authDetails.baseUrl || process.env['GOOGLE_ZERO_BASE_URL']
        ? AuthType.GATEWAY
        : AuthType.USE_ZERO);
    ...
    try {
      ...
      // Extra validation for ZERO API key
      ...
      if (
        ...
        authErrorMessage = 'ZERO API key is missing or not configured.';
      }
    } catch (e) {
      isAuthenticated = false;
    ...
    const chat = await zeroClient.startChat();
  ...
  async loadSession(
    ...
    const zeroClient = config.getZEROClient();
    await zeroClient.initialize();
    await zeroClient.resumeChat(clientHistory, {
