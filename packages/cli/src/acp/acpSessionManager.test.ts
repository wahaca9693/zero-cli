...
describe('AcpSessionManager', () => {
  ...
  it('should create a new session', async () => {
    ...
    expect(mockConfig.getZEROClient).toHaveBeenCalled();
  ...
  it('should include preview models when user has access', async () => {
    ...
    mockConfig.getZERO31LaunchedSync = vi.fn().mockReturnValue(true);
  ...
  it('should fail session creation if ZERO API key is missing', async () => {
    (loadSettings as unknown as Mock).mockImplementation(() => ({
      merged: {
        security: { auth: { selectedType: AuthType.USE_ZERO } },
        mcpServers: {},
      },
      setValue: vi.fn(),
    }));
    ...
    await expect(
      ...
      message: 'ZERO API key is missing or not configured.',
    });
  });
