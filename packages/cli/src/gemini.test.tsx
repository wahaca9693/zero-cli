...
describe('zero.tsx main function', () => {
  ...
  beforeEach(() => {
    ...
    vi.stubEnv('ZERO_SANDBOX', '');
    vi.stubEnv('SANDBOX', '');
    vi.stubEnv('SHPOOL_SESSION_NAME', '');
    vi.stubEnv('ZERO_CLI_TRUST_WORKSPACE', 'true');
...
describe('getNodeMemoryArgs', () => {
  ...
  beforeEach(() => {
    ...
    delete process.env['ZERO_CLI_NO_RELAUNCH'];
  ...
  it('should return empty array if ZERO_CLI_NO_RELAUNCH is set', () => {
    process.env['ZERO_CLI_NO_RELAUNCH'] = 'true';
...
describe('zero.tsx main function kitty protocol', () => {
  ...
  beforeEach(() => {
    ...
    originalEnvNoRelaunch = process.env['ZERO_CLI_NO_RELAUNCH'];
    process.env['ZERO_CLI_NO_RELAUNCH'] = 'true';
  ...
  afterEach(() => {
    ...
    if (originalEnvNoRelaunch !== undefined) {
      process.env['ZERO_CLI_NO_RELAUNCH'] = originalEnvNoRelaunch;
    } else {
      delete process.env['ZERO_CLI_NO_RELAUNCH'];
    }
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  it.each([
    ...
    process.env['ZERO_API_KEY'] = 'test-key';
    try {
      await main();
    } catch (e) {
...
describe('zero.tsx main function exit codes', () => {
  let originalEnvNoRelaunch: string | undefined;
  let originalIsTTY: boolean | undefined;
  ...
  it('should validate and refresh auth in non-interactive mode when no auth type is selected but env var is present', async () => {
    ...
    expect(refreshAuthSpy).toHaveBeenCalledWith(AuthType.USE_ZERO);
  });
});
