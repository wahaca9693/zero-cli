...
import * as coreFunctions from '@allhands/zero-cli-core';
...
describe('setupWorktree', () => {
  ...
  beforeEach(() => {
    ...
    mockService.setup.mockResolvedValue({
      name: 'my-feature',
      path: '/mock/project/.zero/worktrees/my-feature',
      baseSha: 'base-sha',
    });
  });
  ...
  it('should create and switch to a new worktree', async () => {
    ...
    expect(process.chdir).toHaveBeenCalledWith(
      '/mock/project/.zero/worktrees/my-feature',
    );
    expect(process.env['ZERO_CLI_WORKTREE_HANDLED']).toBe('1');
  });

  it('should generate a name if worktreeName is undefined', async () => {
    mockService.setup.mockResolvedValue({
      name: 'generated-name',
      path: '/mock/project/.zero/worktrees/generated-name',
      baseSha: 'base-sha',
    });
  ...
  it('should skip worktree creation if ZERO_CLI_WORKTREE_HANDLED is set', async () => {
