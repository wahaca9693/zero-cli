...
import { debugLogger, writeToStdout } from '@allhands/zero-cli-core';
...
function sanitizeNotificationContent(
  ...
  return {
    title: title || 'ZERO CLI',
    subtitle: subtitle || undefined,
    body: body || 'Open ZERO CLI for details.',
  };
}
...
export function buildRunEventNotificationContent(
  ...
  if (event.type === 'attention') {
    return sanitizeNotificationContent({
      title: 'ZERO CLI needs your attention',
      subtitle: event.heading ?? 'Action required',
      body: event.detail ?? 'Open ZERO CLI to continue.',
    });
  }

  return sanitizeNotificationContent({
    title: 'ZERO CLI session complete',
