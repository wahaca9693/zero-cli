...
describe('SessionSelector', () => {
  ...
  it('should list session with zero message even without user message', async () => {
    const sessionIdZEROOnly = randomUUID();

    // Create test session files
    const chatsDir = path.join(tmpDir, 'chats');
    await fs.mkdir(chatsDir, { recursive: true });

    // Session with only zero message - should be listed
    const sessionZEROOnly = {
      sessionId: sessionIdZEROOnly,
      projectHash: 'test-hash',
      startTime: '2024-01-01T10:00:00.000Z',
      lastUpdated: '2024-01-01T10:30:00.000Z',
      messages: [
    ...
    await fs.writeFile(
      path.join(
        chatsDir,
        `${SESSION_FILE_PREFIX}2024-01-01T10-00-${sessionIdZEROOnly.slice(0, 8)}.json`,
      ),
      JSON.stringify(sessionZEROOnly, null, 2),
    );

    const sessionSelector = new SessionSelector(storage);
    const sessions = await sessionSelector.listSessions();

    // Should list the session with zero message
    expect(sessions.length).toBe(1);
    expect(sessions[0].id).toBe(sessionIdZEROOnly);
  });
