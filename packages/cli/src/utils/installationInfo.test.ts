...
describe('getInstallationInfo', () => {
  ...
  it('should detect running via npx', () => {
    const npxPath = `/Users/test/.npm/_npx/12345/bin/zero`;
  ...
  it('should detect running via pnpx', () => {
    const pnpxPath = `/Users/test/.pnpm/_pnpx/12345/bin/zero`;
  ...
  it('should detect running via bunx', () => {
    const bunxPath = `/Users/test/.bun/install/cache/12345/bin/zero`;
  ...
  it('should detect Homebrew installation via execSync', () => {
    ...
    const cliPath = '/opt/homebrew/Cellar/zero-cli/1.0.0/bin/zero';
    process.argv[1] = cliPath;

    mockedExecSync.mockImplementation((cmd) => {
      if (typeof cmd === 'string' && cmd.includes('brew --prefix zero-cli')) {
        return '/opt/homebrew/opt/zero-cli';
      }
    ...
    mockedRealPathSync.mockImplementation((p) => {
      ...
      if (p === '/opt/homebrew/opt/zero-cli') {
        return '/opt/homebrew/Cellar/zero-cli/1.0.0';
      }
      return String(p);
    });
    ...
    expect(mockedExecSync).toHaveBeenCalledWith(
      expect.stringContaining('brew --prefix zero-cli'),
      expect.anything(),
    );
    expect(info.packageManager).toBe(PackageManager.HOMEBREW);
    expect(info.isGlobal).toBe(true);
    expect(info.updateMessage).toBe(
      'Installed via Homebrew. Please update with "brew upgrade zero-cli".',
    );
  });
  ...
  it('should fall through if brew command fails', () => {
    ...
    const cliPath = '/usr/local/bin/zero';
    process.argv[1] = cliPath;
  ...
  it('should detect local installation and identify yarn from lockfile', () => {
    const localPath = `${projectRoot}/node_modules/.bin/zero`;
  ...
  it('should default to global npm installation for unrecognized paths', () => {
    const globalPath = `/usr/local/bin/zero`;
