...
export async function start_sandbox(
  ...
  try {
    if (config.command === 'sandbox-exec') {
      ...
      // if profile name is not recognized, look in user-level ~/.zero first,
      // then fall back to project-level .zero. path.basename() strips any
      ...
      if (!BUILTIN_SEATBELT_PROFILES.includes(profile)) {
        ...
        const userProfileFile = path.join(homedir(), ZERO_DIR, fileName);
        const projectProfileFile = path.join(ZERO_DIR, fileName);
      ...
      // start and set up proxy if ZERO_SANDBOX_PROXY_COMMAND is set
      const proxyCommand = process.env['ZERO_SANDBOX_PROXY_COMMAND'];
    ...
    // if BUILD_SANDBOX is set, then call scripts/build_sandbox.js under zero-cli repo
    //
    // note this can only be done with binary linked from zero-cli repo
    if (process.env['BUILD_SANDBOX']) {
      if (!gcPath.includes('zero-cli/packages/')) {
        throw new FatalSandboxError(
          'Cannot build sandbox using installed zero binary; ' +
            'run `npm link ./packages/cli` under zero-cli repo to switch to linked binary.',
        );
      } else {
        ...
        execSync(
          `cd ${gcRoot} && node scripts/build_sandbox.js -s ${buildArgs}`,
          {
            ...
            env: {
              ...process.env,
              ZERO_SANDBOX: command, // in case sandbox is enabled via flags (see config.ts under cli package)
            },
          },
        );
      }
    ...
    if (!(await ensureSandboxImageIsPresent(command, image, cliConfig))) {
      const remedy =
        image === LOCAL_DEV_SANDBOX_IMAGE_NAME
          ? 'Try running `npm run build:all` or `npm run build:sandbox` under the zero-cli repo to build it locally, or check the image name and your network connection.'
          : 'Please check the image name, your network connection, or notify zero-cli-dev@google.com if the issue persists.';
      throw new FatalSandboxError(
    ...
    const userSettingsDirOnHost = path.join(userHomeDirOnHost, ZERO_DIR);
    ...
    // ZERO_SANDBOX_PROXY_COMMAND implies HTTPS_PROXY unless HTTP_PROXY is set
    const proxyCommand = process.env['ZERO_SANDBOX_PROXY_COMMAND'];
    ...
    const isIntegrationTest =
      process.env['ZERO_CLI_INTEGRATION_TEST'] === 'true';
    const containerNamePrefix = isIntegrationTest
      ? 'zero-cli-integration-test'
      : imageName;
    ...
    // copy ZERO_CLI_TEST_VAR for integration tests
    if (process.env['ZERO_CLI_TEST_VAR']) {
      args.push(
        '--env',
        `ZERO_CLI_TEST_VAR=${process.env['ZERO_CLI_TEST_VAR']}`,
      );
    }

    // copy ZERO_API_KEY(s)
    if (process.env['ZERO_API_KEY']) {
      args.push('--env', `ZERO_API_KEY=${process.env['ZERO_API_KEY']}`);
    }
    if (process.env['GOOGLE_API_KEY']) {
      args.push('--env', `GOOGLE_API_KEY=${process.env['GOOGLE_API_KEY']}`);
    }

    // copy GOOGLE_ZERO_BASE_URL and GOOGLE_VERTEX_BASE_URL
    if (process.env['GOOGLE_ZERO_BASE_URL']) {
      args.push(
        '--env',
        `GOOGLE_ZERO_BASE_URL=${process.env['GOOGLE_ZERO_BASE_URL']}`,
      );
    }
    ...
    // copy ZERO_MODEL
    if (process.env['ZERO_MODEL']) {
      args.push('--env', `ZERO_MODEL=${process.env['ZERO_MODEL']}`);
    }
    ...
    if (
      ...
      const sandboxVenvPath = path.resolve(ZERO_DIR, 'sandbox.venv');
    ...
    if (process.env['ZERO_CLI_INTEGRATION_TEST'] === 'true') {
      args.push('--user', 'root');
      userFlag = '--user root';
...
// image. The user creates and manages their own LXC container; ZERO runs
// inside it via `lxc exec`. The container name is stored in config.image
// (default: "zero-sandbox"). The workspace is bind-mounted into the
...
async function start_lxc_sandbox(
  ...
  const containerName = config.image || 'zero-sandbox';
  const workdir = path.resolve(process.cwd());
  ...
  try {
    ...
    const workspaceDeviceName = `zero-workspace-${randomBytes(4).toString(
      'hex',
    )}`;
    ...
    if (config.allowedPaths) {
      for (const hostPath of config.allowedPaths) {
        if (hostPath && path.isAbsolute(hostPath) && fs.existsSync(hostPath)) {
          const allowedDeviceName = `zero-allowed-${randomBytes(4).toString(
            'hex',
          )}`;
    ...
    const envVarsToForward: Record<string, string | undefined> = {
      ...
      GOOGLE_CLOUD_PROJECT: process.env['GOOGLE_CLOUD_PROJECT'],
      GOOGLE_CLOUD_LOCATION: process.env['GOOGLE_CLOUD_LOCATION'],
      ZERO_MODEL: process.env['ZERO_MODEL'],
