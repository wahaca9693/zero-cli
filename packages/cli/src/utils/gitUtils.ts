...
export const getLatestGitHubRelease = async (
  ...
  try {
    ...
    const endpoint = `https://api.github.com/repos/google-github-actions/run-zero-cli/releases/latest`;
    ...
    debugLogger.debug(
      `Failed to determine latest run-zero-cli release:`,
      error,
    );
    throw new Error(
      `Unable to determine the latest run-zero-cli release on GitHub.`,
    );
  }
};
