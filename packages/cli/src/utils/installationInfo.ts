...
import { debugLogger, isGitRepository } from '@allhands/zero-cli-core';
  ...
  try {
    ...
    if (process.platform === 'darwin') {
      try {
        ...
        // Brew is not installed or zero-cli is not installed via brew.
    ...
    if (realPath.includes('/.volta/') || realPath.includes('/Volta/')) {
      const updateCommand = 'volta install @allhands/zero-cli@latest';
      return {
    ...
    if (
      ...
      const updateCommand = 'pnpm add -g @allhands/zero-cli@latest';
      return {
    ...
    if (realPath.includes('/.yarn/global')) {
      const updateCommand = 'yarn global add @allhands/zero-cli@latest';
      return {
    ...
    if (realPath.includes('/.bun/install/global')) {
      const updateCommand = 'bun add -g @allhands/zero-cli@latest';
      return {
    ...
    // Assume global npm
    const updateCommand = 'npm install -g @allhands/zero-cli@latest';
    return {
