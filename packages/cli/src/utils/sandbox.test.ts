  ...
  describe('start_sandbox', () => {
    ...
    it('should resolve custom seatbelt profile from user home directory', async () => {
      ...
      vi.mocked(fs.existsSync).mockImplementation((p) =>
        String(p).includes(
          path.join(homedir(), '.zero', 'sandbox-macos-custom-test.sb'),
        ),
      );
      const config: SandboxConfig = createMockSandboxConfig({
        command: 'sandbox-exec',
    ...
    it('should fall back to project .zero directory when user profile is missing', async () => {
      vi.mocked(os.platform).mockReturnValue('darwin');
      vi.stubEnv('SEATBELT_PROFILE', 'custom-test');
      vi.mocked(fs.existsSync).mockImplementation((p) => {
        const s = String(p);
        return (
          s.includes(path.join('.zero', 'sandbox-macos-custom-test.sb')) &&
          !s.includes(path.join(homedir(), '.zero'))
        );
      });
      const config: SandboxConfig = createMockSandboxConfig({
        command: 'sandbox-exec',
      ...
      expect(profileArg).toEqual(
        expect.stringContaining(
          path.join('.zero', 'sandbox-macos-custom-test.sb'),
        ),
      );
    ...
    it('should handle Docker execution', async () => {
      ...
      const containerName = 'zero-cli-sandbox-a1b2c3d4e5f6';
    ...
    it('should preserve the integration-test prefix for random container names', async () => {
      ...
      const containerName = 'zero-cli-integration-test-a1b2c3d4e5f6';
      expect(randomBytes).toHaveBeenCalledWith(6);
      expect(spawn).toHaveBeenNthCalledWith(
        2,
        'docker',
    ...
    it('should mount volumes correctly', async () => {
      ...
      expect(spawn).toHaveBeenNthCalledWith(
        ...
        expect.arrayContaining([
          ...
          '--volume',
          expect.stringMatching(/[\\/]home[\\/]user[\\/]\.zero/),
        ]),
        expect.any(Object),
      );
    });
    ...
    it('should handle networkAccess: false in Docker', async () => {
      ...
      expect(execSync).toHaveBeenCalledWith(
        expect.stringContaining('network create --internal zero-cli-sandbox'),
        expect.any(Object),
      );
      expect(spawn).toHaveBeenCalledWith(
        'docker',
        expect.arrayContaining(['--network', 'zero-cli-sandbox']),
        expect.any(Object),
      );
    });
    ...
    it('should pass through GOOGLE_ZERO_BASE_URL and GOOGLE_VERTEX_BASE_URL', async () => {
      const config: SandboxConfig = createMockSandboxConfig({
        command: 'docker',
        image: 'zero-cli-sandbox',
      });
      process.env['GOOGLE_ZERO_BASE_URL'] = 'http://zero.proxy';
      ...
      expect(spawn).toHaveBeenCalledWith(
        ...
        expect.arrayContaining([
          '--env',
          'GOOGLE_ZERO_BASE_URL=http://zero.proxy',
          '--env',
    ...
    it('should handle user creation on Linux if needed', async () => {
      ...
      expect(entrypointCmd).toContain('groupadd -g 1000 -o zero');
    ...
    it('should register and unregister proxy exit handlers', async () => {
      vi.stubEnv('ZERO_SANDBOX_PROXY_COMMAND', 'some-proxy-cmd');
      const config: SandboxConfig = createMockSandboxConfig({
        command: 'docker',
        image: 'zero-cli-sandbox',
      });
      ...
      vi.mocked(spawn).mockImplementation((cmd, args) => {
        ...
        if (cmd === 'docker' && a && a[0] === 'run') {
          ...
          mockSpawnProcess.on = vi.fn().mockImplementation((event, cb) => {
            if (event === 'close') {
              if (a.includes('zero-cli-sandbox-proxy')) {
    ...
    describe('LXC sandbox', () => {
      const LXC_RUNNING = JSON.stringify([
        { name: 'zero-sandbox', status: 'Running' },
      ]);
      const LXC_STOPPED = JSON.stringify([
        { name: 'zero-sandbox', status: 'Stopped' },
      ]);
      ...
      it('should run lxc exec with correct args for a running container', async () => {
        ...
        expect(spawn).toHaveBeenCalledWith(
          'lxc',
          expect.arrayContaining(['exec', 'zero-sandbox', '--cwd']),
          expect.objectContaining({ stdio: 'inherit' }),
        );
      });
  ...
  describe('gVisor (runsc)', () => {
    it('should use docker with --runtime=runsc on Linux', async () => {
      ...
      expect(spawn).toHaveBeenNthCalledWith(
        1,
        'docker',
        expect.arrayContaining(['images', '-q', 'zero-cli-sandbox']),
      );
