...
export const SANDBOX_PROXY_NAME = 'zero-cli-sandbox-proxy';
...
export function entrypoint(workdir: string, cliArgs: string[]): string[] {
  ...
  const projectSandboxBashrc = `${ZERO_DIR}/sandbox.bashrc`;
  ...
  const cliCmd =
    process.env['NODE_ENV'] === 'development'
      ...
      : isDebugMode
        ? `node --inspect-brk=0.0.0.0:${process.env['DEBUG_PORT'] || '9229'} $(which zero)`
        : 'zero';
