...
vi.mock('@google/zero-cli-core', async (importOriginal) => {
  ...
  return {
    ...
    zeroPartsToContentParts: original.zeroPartsToContentParts,
...
describe('runNonInteractive', () => {
  ...
  beforeEach(async () => {
    ...
    mockConfig = {
      ...
      getZEROClient: vi.fn().mockReturnValue(mockZEROClient),
      getToolRegistry: vi.fn().mockReturnValue(mockToolRegistry),
      getMaxSessionTurns: vi.fn().mockReturnValue(10),
      getSessionId: vi.fn().mockReturnValue('test-session-id'),
      getProjectRoot: vi.fn().mockReturnValue('/test/project'),
      storage: {
        getProjectTempDir: vi.fn().mockReturnValue('/test/project/.zero/tmp'),
      },
  ...
  it('should process input and write text output', async () => {
    const events: ServerZEROStreamEvent[] = [
      { type: ZEROEventType.Content, value: 'Hello' },
      { type: ZEROEventType.Content, value: ' World' },
      {
        type: ZEROEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 10 } },
      },
    ];
    mockZEROClient.sendMessageStream.mockReturnValue(
    ...
    expect(mockZEROClient.sendMessageStream).toHaveBeenCalledWith(
      [{ text: 'Test input' }],
  ...
  it('should register activity logger when ZERO_CLI_ACTIVITY_LOG_TARGET is set', async () => {
    vi.stubEnv('ZERO_CLI_ACTIVITY_LOG_TARGET', '/tmp/test.jsonl');
    const events: ServerZEROStreamEvent[] = [
      {
        type: ZEROEventType.Finished,
  ...
  it('should not register activity logger when ZERO_CLI_ACTIVITY_LOG_TARGET is not set', async () => {
    vi.stubEnv('ZERO_CLI_ACTIVITY_LOG_TARGET', '');
    const events: ServerZEROStreamEvent[] = [
      {
        type: ZEROEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 0 } },
  ...
  it('should handle a single tool call and respond', async () => {
    const toolCallEvent: ServerZEROStreamEvent = {
    ...
    const firstCallEvents: ServerZEROStreamEvent[] = [toolCallEvent];
    const secondCallEvents: ServerZEROStreamEvent[] = [
      { type: ZEROEventType.Content, value: 'Final answer' },
      {
        type: ZEROEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 10 } },
      },
    ...
    mockZEROClient.sendMessageStream
    ...
    expect(mockZEROClient.sendMessageStream).toHaveBeenCalledTimes(2);
    expect(mockSchedulerSchedule).toHaveBeenCalledWith(
      [expect.objectContaining({ name: 'testTool' })],
      expect.any(AbortSignal),
    );
    expect(mockZEROClient.sendMessageStream).toHaveBeenNthCalledWith(
      2,
      [{ text: 'Tool response' }],
      expect.any(AbortSignal),
      'prompt-id-2',
  ...
  it('should write a single newline between sequential text outputs from the model', async () => {
    ...
    const modelTurn1: ServerZEROStreamEvent[] = [
      { type: ZEROEventType.Content, value: 'Use mock tool' },
      toolCallEvent,
    ];
    // Turn 2: Model outputs more text, then requests another tool call.
    const modelTurn2: ServerZEROStreamEvent[] = [
      { type: ZEROEventType.Content, value: 'Use mock tool again' },
      toolCallEvent,
    ];
    // Turn 3: Model outputs a final answer.
    const modelTurn3: ServerZEROStreamEvent[] = [
      { type: ZEROEventType.Content, value: 'Finished.' },
      {
        type: ZEROEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 10 } },
      },
  ...
  it('should handle error during tool execution and should send error back to the model', async () => {
    ...
    const finalResponse: ServerZEROStreamEvent[] = [
      {
        type: ZEROEventType.Content,
  ...
  it('should exit with error if sendMessageStream throws initially', async () => {
    ...
    mockZEROClient.sendMessageStream.mockImplementation(() => {
      throw apiError;
    });
  ...
  it('should preprocess @include commands before sending to the model', async () => {
    ...
    // Mock a simple stream response from the ZERO client
    const events: ServerZEROStreamEvent[] = [
      { type: ZEROEventType.Content, value: 'Summary complete.' },
      {
        type: ZEROEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 10 } },
      },
  ...
  it('should process input and write JSON output with stats', async () => {
    const events: ServerZEROStreamEvent[] = [
      { type: ZEROEventType.Content, value: 'Hello World' },
      {
        type: ZEROEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 10 } },
      },
  ...
  it('should write JSON output with stats for tool-only commands (no text response)', async () => {
    ...
    const firstCallEvents: ServerZEROStreamEvent[] = [
      toolCallEvent,
      {
        type: ZEROEventType.Finished,
    ...
    mockZEROClient.sendMessageStream
      .mockReturnValueOnce(
        createStreamFromEvents([
          { type: ZEROEventType.Content, value: 'Let me check that...' },
          toolCallEvent,
          {
            type: ZEROEventType.Finished,
            value: { reason: undefined, usageMetadata: { totalTokenCount: 5 } },
  ...
  it('should execute a slash command that returns a prompt', async () => {
    ...
    const events: ServerZEROStreamEvent[] = [
      { type: ZEROEventType.Content, value: 'Response from command' },
      {
        type: ZEROEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 5 } },
      },
  ...
  it('should handle slash commands', async () => {
    ...
    const events: ServerZEROStreamEvent[] = [
      { type: ZEROEventType.Content, value: 'Response to slash command' },
      {
        type: ZEROEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 10 } },
      },
  ...
  it('should handle cancellation (Ctrl+C)', async () => {
    ...
    const events: ServerZEROStreamEvent[] = [
      { type: ZEROEventType.Content, value: 'Thinking...' },
    ];
  ...
  it('should treat an unknown slash command as a regular prompt', async () => {
    ...
    const events: ServerZEROStreamEvent[] = [
      { type: ZEROEventType.Content, value: 'Response to unknown' },
      {
        type: ZEROEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 5 } },
      },
  ...
  it('should pass arguments to the slash command action', async () => {
    ...
    const events: ServerZEROStreamEvent[] = [
      { type: ZEROEventType.Content, value: 'Acknowledged' },
      {
        type: ZEROEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 1 } },
      },
  ...
  it('should allow a normally-excluded tool when --allowed-tools is set', async () => {
    ...
    const firstCallEvents: ServerZEROStreamEvent[] = [toolCallEvent];
    const secondCallEvents: ServerZEROStreamEvent[] = [
      { type: ZEROEventType.Content, value: 'file.txt' },
      {
        type: ZEROEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 10 } },
      },
  ...
  it('should resume chat when resumedSessionData is provided', async () => {
    const events: ServerZEROStreamEvent[] = [
      { type: ZEROEventType.Content, value: 'Resumed' },
      {
        type: ZEROEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 5 } },
      },
    ...
    expect(mockZEROClient.resumeChat).toHaveBeenCalledWith(
      expect.any(Array),
      resumedSessionData,
    );
  ...
  it.each([
    ...
    },
    {
      ...
      events: [
        { type: ZEROEventType.MaxSessionTurns },
      ] as ServerZEROStreamEvent[],
    ...
    async ({ events, input, promptId }) => {
      ...
      const streamEvents: ServerZEROStreamEvent[] = [
        ...events,
        {
          type: ZEROEventType.Finished,
          value: { reason: undefined, usageMetadata: { totalTokenCount: 0 } },
  ...
  it('should log error when tool recording fails', async () => {
    ...
    const events: ServerZEROStreamEvent[] = [
      toolCallEvent,
      { type: ZEROEventType.Content, value: 'Done' },
      {
        type: ZEROEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 5 } },
      },
  ...
  it('should stop agent execution immediately when a tool call returns STOP_EXECUTION error', async () => {
    ...
    const firstCallEvents: ServerZEROStreamEvent[] = [
      { type: ZEROEventType.Content, value: 'Executing tool...' },
      toolCallEvent,
    ];
    ...
    expect(mockZEROClient.sendMessageStream).toHaveBeenCalledTimes(1);
  ...
  it('should write JSON output when a tool call returns STOP_EXECUTION error', async () => {
    ...
    const firstCallEvents: ServerZEROStreamEvent[] = [
      { type: ZEROEventType.Content, value: 'Partial content' },
      toolCallEvent,
    ];
  ...
  describe('Agent Execution Events', () => {
    it('should handle AgentExecutionStopped event', async () => {
      const events: ServerZEROStreamEvent[] = [
        {
          type: ZEROEventType.AgentExecutionStopped,
    ...
    it('should handle AgentExecutionBlocked event', async () => {
      const allEvents: ServerZEROStreamEvent[] = [
        {
          type: ZEROEventType.AgentExecutionBlocked,
    ...
    it('should not include warnings field in JSON output if no blocks occur', async () => {
      const allEvents: ServerZEROStreamEvent[] = [
        { type: ZEROEventType.Content, value: 'Clean answer' },
        {
          type: ZEROEventType.Finished,
          value: { reason: undefined, usageMetadata: { totalTokenCount: 10 } },
        },
  ...
  describe('Output Sanitization', () => {
    ...
    it('should sanitize ANSI output by default', async () => {
      const events: ServerZEROStreamEvent[] = [
        { type: ZEROEventType.Content, value: ANSI_SEQUENCE },
        { type: ZEROEventType.Content, value: ' ' },
        { type: ZEROEventType.Content, value: OSC_HYPERLINK },
        {
          type: ZEROEventType.Finished,
          value: { reason: undefined, usageMetadata: { totalTokenCount: 10 } },
        },
    ...
    it('should emit warning event for loop_detected in streaming JSON mode', async () => {
      ...
      const streamEvents: ServerZEROStreamEvent[] = [
        { type: ZEROEventType.LoopDetected } as ServerZEROStreamEvent,
        { type: ZEROEventType.Content, value: 'Continuing after loop' },
        {
          type: ZEROEventType.Finished,
          value: { reason: undefined, usageMetadata: { totalTokenCount: 5 } },
        },
