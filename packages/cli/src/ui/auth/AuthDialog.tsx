...
export function AuthDialog({
  ...
  let items = [
    ...
    ...(process.env['CLOUD_SHELL'] === 'true'
      ...
      : process.env['ZERO_CLI_USE_COMPUTE_ADC'] === 'true'
        ...
        : []),
    {
      label: 'Use ZERO API Key',
      value: AuthType.USE_ZERO,
      key: AuthType.USE_ZERO,
    },
    {
      label: 'Vertex AI',
  ...
  const defaultAuthTypeEnv = process.env['ZERO_DEFAULT_AUTH_TYPE'];
  if (
    defaultAuthTypeEnv &&
  ...
  let initialAuthIndex = items.findIndex((item) => {
    ...
    if (process.env['ZERO_API_KEY']) {
      return item.value === AuthType.USE_ZERO;
    }
    ...
    async (authType: AuthType | undefined, scope: LoadableSettingScope) => {
      ...
      if (authType) {
        ...
        if (authType === AuthType.USE_ZERO) {
          ...
          // whether ZERO_API_KEY env var or a stored key exists.
  ...
  return (
    ...
    >
      ...
      <Box flexDirection="column" flexGrow={1}>
        ...
        <Box marginTop={1}>
          <Text color={theme.text.primary}>
            Terms of Services and Privacy Notice for ZERO CLI
          </Text>
        </Box>
        <Box marginTop={1}>
          <Text color={theme.text.link}>
            {'https://zerocli.com/docs/resources/tos-privacy/'}
          </Text>
        </Box>
      </Box>
    </Box>
