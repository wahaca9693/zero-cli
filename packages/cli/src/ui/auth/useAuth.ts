...
export async function validateAuthMethodWithSettings(
  ...
  // If using ZERO API key, we don't validate it here as we might need to prompt for it.
...
export const useAuthCommand = (
  ...
  const reloadApiKey = useCallback(async () => {
    const envKey = process.env['ZERO_API_KEY'];
    if (envKey !== undefined) {
    ...
    (async () => {
      ...
      if (!authType) {
        if (process.env['ZERO_API_KEY']) {
          onAuthError(
            'Existing API key detected (ZERO_API_KEY). Select "ZERO API Key" option to use it.',
          );
        } else {
      ...
      const defaultAuthType = process.env['ZERO_DEFAULT_AUTH_TYPE'];
      if (
        defaultAuthType &&
        ...
        onAuthError(
          `Invalid value for ZERO_DEFAULT_AUTH_TYPE: "${defaultAuthType}". ` +
