...
describe('AuthDialog', () => {
  ...
  beforeEach(() => {
    ...
    vi.stubEnv('ZERO_CLI_USE_COMPUTE_ADC', undefined as unknown as string);
    vi.stubEnv('ZERO_DEFAULT_AUTH_TYPE', undefined as unknown as string);
    vi.stubEnv('ZERO_API_KEY', undefined as unknown as string);
  ...
  it('filters auth types when enforcedType is set', async () => {
    props.settings.merged.security.auth.enforcedType = AuthType.USE_ZERO;
    const { unmount } = await renderWithProviders(<AuthDialog {...props} />);
    const items = mockedRadioButtonSelect.mock.calls[0][0].items;
    expect(items).toHaveLength(1);
    expect(items[0].value).toBe(AuthType.USE_ZERO);
    unmount();
  });
  ...
  describe('Initial Auth Type Selection', () => {
    it.each([
      ...
      },
      {
        ...
        expected: AuthType.USE_ZERO,
        desc: 'from ZERO_DEFAULT_AUTH_TYPE env var',
      },
      {
        setup: () => {
          vi.stubEnv('ZERO_API_KEY', 'test-key');
        },
        expected: AuthType.USE_ZERO,
        desc: 'from ZERO_API_KEY env var',
      },
      {
        setup: () => {},
  ...
  describe('handleAuthSelect', () => {
    it('calls onAuthError if validation fails', async () => {
      ...
      await handleAuthSelect(AuthType.USE_ZERO);
    ...
    it('always shows API key dialog even when env var is present', async () => {
      mockedValidateAuthMethod.mockResolvedValue(null);
      vi.stubEnv('ZERO_API_KEY', 'test-key-from-env');
    ...
    it('always shows API key dialog even when env var is empty string', async () => {
      mockedValidateAuthMethod.mockResolvedValue(null);
      vi.stubEnv('ZERO_API_KEY', ''); // Empty string
    ...
    it('shows API key dialog on initial setup if no env var is present', async () => {
      mockedValidateAuthMethod.mockResolvedValue(null);
      // process.env['ZERO_API_KEY'] is not set
      // props.settings.merged.security.auth.selectedType is undefined here, simulating initial setup
