...
export function ModelDialog({ onClose }: ModelDialogProps): React.JSX.Element {
  ...
  const preferredModel = config?.getModel() || ZERO_MODEL_ALIAS_AUTO;

  const shouldShowPreviewModels = config?.getHasAccessToPreviewModel() ?? false;
  const useZERO31 = config?.getZERO31LaunchedSync?.() ?? false;
  const useZERO3_5Flash = config?.hasZERO35FlashGAAccess?.() ?? false;
  const selectedAuthType = settings.merged.security.auth.selectedType;
  const useCustomToolModel =
    useZERO31 && selectedAuthType === AuthType.USE_ZERO;
  ...
  const mainOptions = useMemo(() => {
    ...
    const list = [
      {
        value: ZERO_MODEL_ALIAS_AUTO,
        title: getDisplayString(ZERO_MODEL_ALIAS_AUTO),
        description: getAutoModelDescription(
          shouldShowPreviewModels,
          useZERO31,
          useZERO3_5Flash,
        ),
        key: ZERO_MODEL_ALIAS_AUTO,
      },
      {
        value: 'Manual',
        title: manualModelSelected
  ...
  const manualOptions = useMemo(() => {
    ...
    const options = [
      {
        value: DEFAULT_ZERO_MODEL,
        title: getDisplayString(DEFAULT_ZERO_MODEL),
        key: DEFAULT_ZERO_MODEL,
      },
      {
        value: DEFAULT_ZERO_FLASH_LITE_MODEL,
        title: getDisplayString(DEFAULT_ZERO_FLASH_LITE_MODEL),
        key: DEFAULT_ZERO_FLASH_LITE_MODEL,
      },
      {
        value: DEFAULT_ZERO_FLASH_MODEL,
        title: getDisplayString(DEFAULT_ZERO_FLASH_MODEL),
        key: DEFAULT_ZERO_FLASH_MODEL,
      },
    ];
    ...
    if (shouldShowPreviewModels) {
      const previewProModel = useZERO31
        ? PREVIEW_ZERO_3_1_MODEL
        : PREVIEW_ZERO_MODEL;
      ...
      const previewOptions = [
        ...
        {
          value: PREVIEW_ZERO_FLASH_MODEL,
          title: getDisplayString(PREVIEW_ZERO_FLASH_MODEL),
          key: PREVIEW_ZERO_FLASH_MODEL,
        },
      ];

      if (PREVIEW_ZERO_FLASH_LITE_MODEL !== 'none') {
        previewOptions.push({
          value: PREVIEW_ZERO_FLASH_LITE_MODEL,
          title: getDisplayString(PREVIEW_ZERO_FLASH_LITE_MODEL),
          key: PREVIEW_ZERO_FLASH_LITE_MODEL,
        });
      }
    ...
    >
      ...
      <Box flexDirection="column">
        <Text color={theme.text.secondary}>
          {'> To use a specific ZERO model on startup, use the --model flag.'}
        </Text>
      </Box>
      <ModelQuotaDisplay
