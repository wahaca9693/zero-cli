...
describe('VoiceModelDialog', () => {
  it('should display a privacy warning when ZERO Live API (Cloud) is selected', async () => {
    const onClose = vi.fn();
    ...
    expect(frame).toContain('ZERO Live API (Cloud)');
    expect(frame).toContain('When using the ZERO Live backend');
  });
  ...
  it('should NOT display a privacy warning when Whisper (Local) is highlighted', async () => {
    ...
    // Verify warning is present for default (ZERO Live)
    expect(lastFrame()).toContain('When using the ZERO Live backend');
    ...
    await waitFor(() => {
      ...
      expect(frame).not.toContain('When using the ZERO Live backend');
    });
  });
  ...
  it('should update settings and close dialog when a backend is selected', async () => {
    ...
    // Select ZERO Live (it's already highlighted, just press Enter)
    await act(async () => {
      stdin.write('\r');
    });
