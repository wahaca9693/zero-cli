...
describe('<HistoryItemDisplay />', () => {
  ...
  it('renders AboutBox for "about" type', async () => {
    ...
    expect(lastFrame()).toContain('About ZERO CLI');
    unmount();
  });
  ...
  describe.each([true, false])(
    ...
    (useAlternateBuffer) => {
      ...
      it('should render a truncated zero item', async () => {
        const item: HistoryItem = {
          id: 1,
          type: 'zero',
          text: longCode,
      ...
      it('should render a full zero item when using availableTerminalHeightZERO', async () => {
        const item: HistoryItem = {
          id: 1,
          type: 'zero',
          text: longCode,
        ...
        const { lastFrame, unmount } = await renderWithProviders(
          <HistoryItemDisplay
            ...
            terminalWidth={80}
            availableTerminalHeight={10}
            availableTerminalHeightZERO={Number.MAX_SAFE_INTEGER}
          />,
          {
            config: makeFakeConfig({ useAlternateBuffer }),
            settings: createMockSettings({ ui: { useAlternateBuffer } }),
      ...
      it('should render a truncated zero_content item', async () => {
        const item: HistoryItem = {
          id: 1,
          type: 'zero_content',
          text: longCode,
      ...
      it('should render a full zero_content item when using availableTerminalHeightZERO', async () => {
        const item: HistoryItem = {
          id: 1,
          type: 'zero_content',
          text: longCode,
