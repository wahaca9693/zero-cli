...
const SKILL_DESTINATION_CHOICES: DestinationChoice[] = [
  {
    destination: 'global',
    label: 'Global',
    description: '~/.zero/skills — available in all projects',
  },
  {
    destination: 'project',
    label: 'Project',
    description: '.zero/skills — available in this workspace',
  },
];
...
// project (private MEMORY.md and sibling files, plus ~/.zero/ZERO.md),
...
function getSkillOriginTag(filePath: string): string {
  ...
  if (normalizedPath.includes('/.zero/skills/')) {
    ...
    if (
      homeDirs.some((homeDir) =>
        normalizedPath.startsWith(`${homeDir}/.zero/skills/`),
      )
    ) {
      return 'Global';
    }
