...
import { isBinaryAvailable } from '@allhands/zero-cli-core';
import {
  WhisperModelManager,
...
export function VoiceModelDialog({
  ...
  const currentBackend =
    settings.merged.experimental.voice?.backend ?? 'zero-live';
  ...
  const backendOptions = useMemo(
    () => [
      {
        value: 'zero-live',
        title: 'ZERO Live API (Cloud)',
        description: 'Real-time cloud transcription via ZERO Live API.',
        key: 'zero-live',
      },
      {
        value: 'whisper',
          ...
          {view === 'backend' ? (
            <>
              <DescriptiveRadioButtonSelect
                ...
                showNumbers={true}
              />
              {highlightedBackend === 'zero-live' && (
                <Box marginTop={1}>
                  <WarningMessage text="When using the ZERO Live backend, voice recordings are sent to Google Cloud for transcription. Enterprise users should verify this aligns with their data privacy and compliance requirements." />
                </Box>
              )}
            </>
          ) : (
