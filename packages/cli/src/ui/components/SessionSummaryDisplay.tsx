...
export const SessionSummaryDisplay: React.FC<SessionSummaryDisplayProps> = ({
  ...
  const footerSessionId =
    ...
    !escapedSessionId.startsWith("'")
      ? `"${escapedSessionId}"`
      : escapedSessionId;
  let footer = `To resume this session: zero --resume ${footerSessionId}`;

  if (worktreeSettings) {
    footer =
      `To resume work in this worktree: cd ${escapeShellArg(worktreeSettings.path, shell)} && zero --resume ${footerSessionId}\n` +
