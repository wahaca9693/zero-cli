...
import { ToolCallDecision, LlmRole } from '@allhands/zero-cli-core';
...
const renderWithMockedStats = async (
  ...
  width?: number,
  currentModel: string = 'zero-2.5-pro',
) => {
...
describe('<ModelStatsDisplay />', () => {
  ...
  it('should display stats for multiple models correctly', async () => {
    ...
    const output = lastFrame();
    expect(output).toContain('zero-2.5-pro');
    expect(output).toContain('zero-2.5-flash');
    expect(output).toMatchSnapshot();
    unmount();
  });
  ...
  it('should display a single model correctly', async () => {
    ...
    const output = lastFrame();
    expect(output).toContain('zero-2.5-pro');
    expect(output).not.toContain('zero-2.5-flash');
    expect(output).toMatchSnapshot();
    unmount();
  });

  it('should resolve zero-3-flash to zero-3.5-flash via getDisplayString', async () => {
    const { lastFrame, unmount } = await renderWithMockedStats({
      models: {
        'zero-3-flash': {
          api: { totalRequests: 1, totalErrors: 0, totalLatencyMs: 100 },
    ...
    const output = lastFrame();
    expect(output).toContain('zero-3.5-flash');
    expect(output).not.toContain('zero-3-flash');
    expect(output).toMatchSnapshot();
    unmount();
  });

  it('should handle models with long names (zero-3-*-preview) without layout breaking', async () => {
    const { lastFrame, unmount } = await renderWithMockedStats(
      {
        models: {
          'zero-3-pro-preview': {
    ...
    const output = lastFrame();
    expect(output).toContain('zero-3-pro-');
    expect(output).toContain('zero-3-flash-');
    expect(output).toMatchSnapshot();
    unmount();
  });
