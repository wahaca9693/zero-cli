...
vi.mock('../ZERORespondingSpinner.js', () => ({
  ZERORespondingSpinner: () => <Text>Spinner</Text>,
}));
...
describe('AlternateBufferQuittingDisplay', () => {
  ...
  it('renders with user and zero messages', async () => {
    persistentStateMock.setData({ tipsShown: 0 });
    const history: HistoryItem[] = [
      { id: 1, type: 'user', text: 'Hello ZERO' },
      { id: 2, type: 'zero', text: 'Hello User!' },
    ];
    const { lastFrame, unmount } = await renderWithProviders(
      <AlternateBufferQuittingDisplay />,
      {
    ...
    expect(lastFrame()).toMatchSnapshot('with_user_zero_messages');
    unmount();
  });
});
