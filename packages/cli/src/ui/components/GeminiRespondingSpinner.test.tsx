...
import { ZERORespondingSpinner } from './ZERORespondingSpinner.js';
...
vi.mock('./ZEROSpinner.js', () => ({
  ZEROSpinner: ({ altText }: { altText?: string }) => (
    <Text>ZEROSpinner {altText}</Text>
  ),
}));

describe('ZERORespondingSpinner', () => {
  ...
  it('renders spinner when responding', async () => {
    ...
    const { lastFrame, unmount } = await render(<ZERORespondingSpinner />);
    expect(lastFrame()).toContain('ZEROSpinner');
    unmount();
  });
  ...
  it('renders non-responding display when provided', async () => {
    ...
    const { lastFrame, unmount } = await render(
      <ZERORespondingSpinner nonRespondingDisplay="Waiting..." />,
    );
