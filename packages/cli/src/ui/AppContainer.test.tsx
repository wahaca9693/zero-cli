...
vi.mock('./hooks/useZEROStream.js');
...
import { useZEROStream } from './hooks/useZEROStream.js';
...
describe('AppContainer State Management', () => {
  ...
  const mockedUseZEROStream = useZEROStream as Mock;
  ...
  const DEFAULT_ZERO_STREAM_MOCK = {
    streamingState: 'idle',
    submitQuery: vi.fn(),
    initError: null,
    pendingHistoryItems: [],
  ...
  beforeEach(() => {
    ...
    mockedUseZEROStream.mockReturnValue(DEFAULT_ZERO_STREAM_MOCK);
  ...
  describe('State Initialization', () => {
    ...
    it('sends a macOS notification when confirmation is pending and terminal is unfocused', async () => {
      ...
      mockedUseZEROStream.mockReturnValue({
        ...DEFAULT_ZERO_STREAM_MOCK,
        pendingHistoryItems: [
          {
            type: 'tool_group',
            tools: [
    ...
    it('sends a macOS notification when a response completes while unfocused', async () => {
      ...
      mockedUseZEROStream.mockImplementation(() => ({
        ...DEFAULT_ZERO_STREAM_MOCK,
      ...
      expect(
        ...
        expect.objectContaining({
          ...
          detail: 'ZERO CLI finished responding.',
        }),
      );
      expect(terminalNotificationsMocks.notifyViaTerminal).toHaveBeenCalled();
  ...
  describe('Session Resumption', () => {
    ...
    it('initializes chat recording service when config has it', async () => {
      ...
      vi.spyOn(configWithRecording, 'getZEROClient').mockReturnValue(
        mockZEROClient as unknown as ReturnType<Config['getZEROClient']>,
      );
  ...
  describe('Session Recording Integration', () => {
    it('provides chat recording service configuration', async () => {
      ...
      expect(configWithRecording.getZEROClient).toBeDefined();
      expect(mockZEROClient.getChatRecordingService).toBeDefined();
  ...
  describe('Session Resume Flow', () => {
    it('accepts resumed session data', async () => {
      ...
      vi.spyOn(configWithClient, 'getZEROClient').mockReturnValue(
        mockZEROClient as unknown as ReturnType<Config['getZEROClient']>,
      );
      ...
      expect(mockZEROClient.resumeChat).toBeDefined();
  ...
  describe('Quota and Fallback Integration', () => {
    ...
    it('passes a valid proQuotaRequest to QuotaContext when provided by the hook', async () => {
      ...
      const mockRequest = {
        failedModel: 'zero-pro',
        fallbackModel: 'zero-flash',
        resolve: vi.fn(),
      };
      mockedUseQuotaAndFallback.mockReturnValue({
  ...
  describe('Terminal Title Update Feature', () => {
    ...
    it('should use legacy terminal title when dynamicWindowTitle is false', async () => {
      ...
      expect(titleWrites).toHaveLength(1);
      expect(titleWrites[0][0]).toBe(
        `\x1b]0;${'ZERO CLI (workspace)'.padEnd(80, ' ')}\x07`,
      );
      unmount();
    });
    ...
    it('should use CLI_TITLE environment variable when set', async () => {
      ...
      vi.stubEnv('CLI_TITLE', 'Custom ZERO Title');
      ...
      expect(titleWrites).toHaveLength(1);
      expect(titleWrites[0][0]).toBe(
        `\x1b]0;${'✦  Working… (Custom ZERO Title)'.padEnd(80, ' ')}\x07`,
      );
      unmount();
    });
  });
  ...
  describe('Banner Text', () => {
    it('should render placeholder banner text for USE_ZERO auth type', async () => {
  ...
  describe('onCancelSubmit Behavior', () => {
    ...
    // Helper to extract arguments from the useZEROStream hook call
    // This isolates the positional argument dependency to a single location
    const extractUseZEROStreamArgs = (args: unknown[]) => ({
    ...
    it('preserves buffer when cancelling, even if empty (user is in control)', async () => {
      ...
      const { onCancelSubmit } = extractUseZEROStreamArgs(
        mockedUseZEROStream.mock.lastCall!,
      );
  ...
  describe('Permission Handling', () => {
    ...
    it.each([true, false])(
      ...
      async (allowed) => {
        ...
        const { submitQuery } = mockedUseZEROStream();
