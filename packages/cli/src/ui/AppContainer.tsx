...
export const AppContainer = (props: AppContainerProps) => {
  ...
  const historyManager = useHistory({
    chatRecordingService: config.getZEROClient()?.getChatRecordingService(),
  });
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (result) {
        ...
        const zeroClient = config.getZEROClient();
        if (additionalContext && zeroClient) {
  ...
  const isAuthenticating =
    ...
    settings.merged.security.auth.selectedType !== AuthType.USE_ZERO;

  // Session browser and resume functionality
  const isZEROClientInitialized = config.getZEROClient()?.isInitialized();

  const { loadHistoryForResume, isResuming } = useSessionResume({
    config,
    historyManager,
    refreshStatic,
    isZEROClientInitialized,
    setQuittingMessages,
    resumedSessionData,
    isAuthenticating,
  });
    ...
    async (authType: AuthType | undefined, scope: LoadableSettingScope) => {
      if (authType) {
        ...
        if (
          ...
          writeToStdout(`
----------------------------------------------------------------
Logging in with Google... Restarting ZERO CLI to continue.
----------------------------------------------------------------
          `);
          await relaunchApp();
        }
  ...
  useEffect(() => {
    if (
      ...
      // We skip validation for ZERO API key here because it might be stored
      // in the keychain, which we can't check synchronously.
      // The useAuth hook handles validation for this case.
      if (settings.merged.security.auth.selectedType === AuthType.USE_ZERO) {
        return;
      }
    ...
    historyManager.addItem(
      {
        ...
        text: 'Refreshing hierarchical memory (ZERO.md or other context files)...',
      },
      Date.now(),
    );
    try {
      ...
      const fileCount = config.getZEROMdFileCount();
  ...
  const {
    ...
    submitQuery,
    initError,
    pendingHistoryItems: pendingZEROHistoryItems,
    thought,
  ...
  const pendingHistoryItems = useMemo(
    () => [...pendingSlashCommandHistoryItems, ...pendingZEROHistoryItems],
    [pendingSlashCommandHistoryItems, pendingZEROHistoryItems],
  );
  ...
  useEffect(() => {
    ...
    // Note: We don't need to reset the window title on exit because ZERO CLI is already doing that elsewhere
  }, [
    streamingState,
    thought,
  ...
  const [zeroMdFileCount, setZEROMdFileCount] = useState<number>(
    config.getZEROMdFileCount(),
  );
  useEffect(() => {
    const handleMemoryChanged = (result: MemoryChangedPayload) => {
      setZEROMdFileCount(result.fileCount);
    };
