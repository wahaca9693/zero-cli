...
describe('modelCommand', () => {
  ...
  describe('set subcommand', () => {
    it('should set the model and log the command', async () => {
      ...
      await setCommand!.action!(mockContext, 'zero-pro');

      expect(mockSetModel).toHaveBeenCalledWith('zero-pro', true);
      expect(mockContext.ui.addItem).toHaveBeenCalledWith(
        expect.objectContaining({
          type: MessageType.INFO,
          text: expect.stringContaining('Model set to zero-pro'),
        }),
      );
    });
    ...
    it('should set the model with persistence when --persist is used', async () => {
      ...
      await setCommand!.action!(mockContext, 'zero-pro --persist');

      expect(mockSetModel).toHaveBeenCalledWith('zero-pro', false);
      expect(mockContext.ui.addItem).toHaveBeenCalledWith(
        expect.objectContaining({
          type: MessageType.INFO,
          text: expect.stringContaining('Model set to zero-pro (persisted)'),
        }),
      );
    });
