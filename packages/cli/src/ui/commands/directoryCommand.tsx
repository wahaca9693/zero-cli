...
async function finishAddingDirectories(
  ...
  if (added.length > 0) {
    try {
      ...
      addItem({
        ...
        text: `Successfully added ZERO.md files from the following directories if there are:\n- ${added.join('\n- ')}`,
      });
    } catch (error) {
  ...
  if (added.length > 0) {
    const zero = config.zeroClient;
    if (zero) {
      await zero.addDirectoryContext();

      // Persist directories to session file for resume support
      const chatRecordingService = zero.getChatRecordingService();
