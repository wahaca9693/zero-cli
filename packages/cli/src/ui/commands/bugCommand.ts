...
export const bugCommand: SlashCommand = {
  ...
  action: async (context: CommandContext, args?: string): Promise<void> => {
    ...
    if (process.env['SANDBOX'] && process.env['SANDBOX'] !== 'sandbox-exec') {
      sandboxEnv = process.env['SANDBOX'].replace(/^zero-(?:code-)?/, '');
...
`;
    ...
    const chat = agentContext?.zeroClient?.getChat();
    ...
    let bugReportUrl =
      'https://github.com/google-zero/zero-cli/issues/new?template=bug_report.yml&title={title}&info={info}&problem={problem}';
