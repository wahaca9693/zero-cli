...
const authCommand: SlashCommand = {
  ...
  action: async (
    ...
    try {
      ...
      const zeroClient = context.services.agentContext?.zeroClient;
...
const reloadCommand: SlashCommand = {
  ...
  action: async (
    ...
    // Update the client with the new tools
    const zeroClient = agentContext.zeroClient;
    if (zeroClient?.isInitialized()) {
      await zeroClient.setTools();
    }
...
async function handleEnableDisable(
  ...
  if (agentContext.zeroClient?.isInitialized())
    await agentContext.zeroClient.setTools();
  context.ui.reloadCommands();
