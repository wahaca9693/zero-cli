...
import { convertToRestPayload } from '@allhands/zero-cli-core';
...
const getSavedChatTags = async (
  ...
  const zeroDir = cfg?.storage?.getProjectTempDir();
  if (!zeroDir) {
    return [];
  }
  try {
    ...
    const files = await fsPromises.readdir(zeroDir);
    const chatDetails: ChatDetail[] = [];

    for (const file of files) {
      if (file.startsWith(file_head) && file.endsWith(file_tail)) {
        const filePath = path.join(zeroDir, file);
...
const saveCommand: SlashCommand = {
  ...
  action: async (context, args): Promise<SlashCommandActionReturn | void> => {
    ...
    const chat = context.services.agentContext?.zeroClient?.getChat();
    if (!chat) {
      return {
        type: 'message',
        messageType: 'error',
...
const resumeCheckpointCommand: SlashCommand = {
  ...
  action: async (context, args) => {
    ...
    for (const item of conversation.slice(INITIAL_HISTORY_LENGTH)) {
      ...
      uiHistory.push({
        type: (item.role && rolemap[item.role]) || MessageType.ZERO,
        text,
...
const shareCommand: SlashCommand = {
  ...
  action: async (context, args): Promise<MessageActionReturn> => {
    ...
    if (!filePathArg) {
      filePathArg = `zero-conversation-${Date.now()}.json`;
    }
