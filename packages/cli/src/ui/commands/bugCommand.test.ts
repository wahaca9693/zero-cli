...
import { getVersion, type Config } from '@allhands/zero-cli-core';
...
* **IDE Client:** VSCode
`;
    const expectedUrl = `https://github.com/google-zero/zero-cli/issues/new?template=bug_report.yml&title=A%20test%20bug&info=${encodeURIComponent(expectedInfo)}&problem=A%20test%20bug`;
...
`;
  ...
  it('captures a heap snapshot AFTER opening the bug URL when RSS exceeds 2 GB', async () => {
    ...
    const tempDir = path.join('/tmp', 'zero-test');
  ...
  it('skips auto-capture when RSS is below the 2 GB threshold', async () => {
    ...
    const context = buildHighMemoryContext('/tmp/zero-test');
