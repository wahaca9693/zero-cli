...
describe('handleAtCommand', () => {
  ...
  beforeEach(async () => {
    ...
    mockConfig = {
      ...
      getFileFilteringRespectZEROIgnore: () => true,
      ...
      storage: {
        getProjectTempDir: () => path.join(os.tmpdir(), 'zero-cli-temp'),
      },
  ...
  describe('zero-ignore filtering', () => {
    it('should skip zero-ignored files in @ commands', async () => {
      await createTestFile(
        path.join(testRootDir, ZERO_IGNORE_FILE_NAME),
        'build/output.js',
      );
      const zeroIgnoredFile = await createTestFile(
        path.join(testRootDir, 'build', 'output.js'),
        'console.log("Hello");',
      );
      const query = `@${zeroIgnoredFile}`;
      ...
      expect(mockOnDebugMessage).toHaveBeenCalledWith(
        `Path ${zeroIgnoredFile} is zero-ignored and will be skipped.`,
      );
      expect(mockOnDebugMessage).toHaveBeenCalledWith(
        `Ignored 1 files:\nZERO-ignored: ${zeroIgnoredFile}`,
      );
    });
  });
  it('should process non-ignored files when .zeroignore is present', async () => {
    await createTestFile(
      path.join(testRootDir, ZERO_IGNORE_FILE_NAME),
      'build/output.js',
    );
  ...
  it('should handle mixed zero-ignored and valid files', async () => {
    await createTestFile(
      path.join(testRootDir, ZERO_IGNORE_FILE_NAME),
      'dist/bundle.js',
    );
    ...
    const query = `@${validFile} @${zeroIgnoredFile}`;
    ...
    expect(result).toEqual({
      processedQuery: [
        { text: `@${getRelativePath(validFile)} @${zeroIgnoredFile}` },
        { text: '\n--- Content from referenced files ---' },
        { text: `\nContent from @${getRelativePath(validFile)}:\n` },
