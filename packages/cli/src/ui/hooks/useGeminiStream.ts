...
import {
  ZEROEventType as ServerZEROEventType,
  getErrorMessage,
  isNodeError,
  MessageSenderType,
  logUserPrompt,
...
import type {
  ...
  EditorType,
  ZEROClient,
  ServerZEROChatCompressedEvent,
  ServerZEROContentEvent as ContentEvent,
  ServerZEROFinishedEvent,
  ServerZEROStreamEvent as ZEROEvent,
  ThoughtSummary,
  ToolCallRequestInfo,
...
function calculateStreamingState(
  ...
  const isAnyToolActive = toolCalls.some((tc) => {
    ...
    // if the result hasn't been submitted back to ZERO yet.
    if (
...
/**
 * Manages the ZERO stream, including user input, command processing,
 * API interaction, and tool call lifecycle.
 */
export const useZEROStream = (
  ...
  const [
    ...
    async (completedToolCallsFromScheduler) => {
      ...
      if (completedToolCallsFromScheduler.length > 0) {
        ...
        try {
          const currentModel =
            config.getZEROClient().getCurrentSequenceModel() ??
            config.getModel();
          config
            .getZEROClient()
            .getChat()
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < toolCalls.length; i++) {
      ...
      if (
        ...
        // Architecturally, `useZEROStream` should not be responsible for stitching
  ...
  const prepareQueryForZERO = useCallback(
    async (
      query: PartListUnion,
      userMessageTimestamp: number,
      ...
      let localQueryToSendToZERO: PartListUnion | null = null;
      ...
      if (typeof query === 'string') {
          ...
          if (slashCommandResult) {
            switch (slashCommandResult.type) {
              case 'schedule_tool': {
                ...
                if (postSubmitPrompt) {
                  localQueryToSendToZERO = postSubmitPrompt;
                  return {
                    queryToSend: localQueryToSendToZERO,
                    shouldProceed: true,
                  };
                }
              ...
              case 'submit_prompt': {
                localQueryToSendToZERO = slashCommandResult.content;
        ...
        if (isAtCommand(trimmedQuery)) {
          ...
          localQueryToSendToZERO = atCommandResult.processedQuery;
        } else {
          // Normal query for ZERO
          addItem(
            { type: MessageType.USER, text: trimmedQuery },
            userMessageTimestamp,
          );
          localQueryToSendToZERO = trimmedQuery;
        }
      } else {
        // It's a function response (PartListUnion that isn't a string)
        localQueryToSendToZERO = query;
      }

      if (localQueryToSendToZERO === null) {
        onDebugMessage(
          'Query processing resulted in null, not sending to ZERO.',
        );
        return { queryToSend: null, shouldProceed: false };
      }
      return { queryToSend: localQueryToSendToZERO, shouldProceed: true };
    },
    [
      config,
      addItem,
  ...
  const handleContentEvent = useCallback(
    (
      ...
      currentZEROMessageBuffer: string,
      userMessageTimestamp: number,
    ): string => {
      setRetryStatus(null);
      if (turnCancelledRef.current) {
      ...
      let newZEROMessageBuffer = currentZEROMessageBuffer + eventValue;
      if (
        pendingHistoryItemRef.current?.type !== 'zero' &&
        pendingHistoryItemRef.current?.type !== 'zero_content'
      ) {
        // Flush any pending item before starting zero content
        if (pendingHistoryItemRef.current) {
          addItem(pendingHistoryItemRef.current, userMessageTimestamp);
        }
        setPendingHistoryItem({ type: 'zero', text: '' });
        newZEROMessageBuffer = eventValue;
      }
      // Split large messages for better rendering performance. Ideally,
      // we should maximize the amount of output sent to <Static />.
      const splitPoint = findLastSafeSplitPoint(newZEROMessageBuffer);
      if (splitPoint === newZEROMessageBuffer.length) {
        // Update the existing message with accumulated content
        setPendingHistoryItem((item) => ({
          // eslint-disable-next-line @typescript-eslint/no-unsafe-type-assertion
          type: item?.type as 'zero' | 'zero_content',
          text: newZEROMessageBuffer,
        }));
      } else {
        // This indicates that we need to split up this ZERO Message.
        ...
        const beforeText = newZEROMessageBuffer.substring(0, splitPoint);
        const afterText = newZEROMessageBuffer.substring(splitPoint);
        ...
        setPendingHistoryItem({ type: 'zero_content', text: afterText });
        newZEROMessageBuffer = afterText;
      }
      return newZEROMessageBuffer;
    },
  ...
  const handleErrorEvent = useCallback(
    (eventValue: ZEROErrorEventValue, userMessageTimestamp: number) => {
      if (pendingHistoryItemRef.current) {
        addItem(pendingHistoryItemRef.current, userMessageTimestamp);
        setPendingHistoryItem(null);
      }
  ...
  const handleFinishedEvent = useCallback(
    (event: ServerZEROFinishedEvent, userMessageTimestamp: number) => {
  ...
  const handleChatCompressionEvent = useCallback(
    (
      eventValue: ServerZEROChatCompressedEvent['value'],
      userMessageTimestamp: number,
    ) => {
      if (pendingHistoryItemRef.current) {
        addItem(pendingHistoryItemRef.current, userMessageTimestamp);
  ...
  const processZEROStreamEvents = useCallback(
    async (
      stream: AsyncIterable<ZEROEvent>,
      userMessageTimestamp: number,
      signal: AbortSignal,
    ): Promise<StreamProcessingStatus> => {
      let zeroMessageBuffer = '';
      const toolCallRequests: ToolCallRequestInfo[] = [];
      for await (const event of stream) {
        if (
          event.type !== ServerZEROEventType.Thought &&
        ...
        switch (event.type) {
          case ServerZEROEventType.Thought:
            setLastZEROActivityTime(Date.now());
            handleThoughtEvent(event.value, userMessageTimestamp);
            break;
          case ServerZEROEventType.Content:
            setLastZEROActivityTime(Date.now());
            zeroMessageBuffer = handleContentEvent(
              event.value,
              zeroMessageBuffer,
              userMessageTimestamp,
            );
            break;
          case ServerZEROEventType.ToolCallRequest:
            toolCallRequests.push(event.value);
            break;
          case ServerZEROEventType.UserCancelled:
            handleUserCancelledEvent(userMessageTimestamp);
            break;
          case ServerZEROEventType.Error:
            handleErrorEvent(event.value, userMessageTimestamp);
            break;
          case ServerZEROEventType.AgentExecutionStopped:
            ...
            break;
          case ServerZEROEventType.AgentExecutionBlocked:
            ...
            break;
          case ServerZEROEventType.ChatCompressed:
            handleChatCompressionEvent(event.value, userMessageTimestamp);
            break;
          case ServerZEROEventType.ToolCallConfirmation:
          case ServerZEROEventType.ToolCallResponse:
            // do nothing
            break;
          case ServerZEROEventType.MaxSessionTurns:
            handleMaxSessionTurnsEvent();
            break;
          case ServerZEROEventType.ContextWindowWillOverflow:
            handleContextWindowWillOverflowEvent(
              event.value.estimatedRequestTokenCount,
              event.value.remainingTokenCount,
            );
            break;
          case ServerZEROEventType.Finished:
            handleFinishedEvent(event, userMessageTimestamp);
            break;
          case ServerZEROEventType.Citation:
            handleCitationEvent(event.value, userMessageTimestamp);
            break;
          case ServerZEROEventType.ModelInfo:
            handleChatModelEvent(event.value, userMessageTimestamp);
            break;
          case ServerZEROEventType.LoopDetected:
            // handle later because we want to move pending history to history
            // before we add loop detected message to history
            loopDetectedRef.current = true;
            break;
          case ServerZEROEventType.Retry:
          case ServerZEROEventType.InvalidStream:
      ...
      runInDevTraceSpan(
        {
          operation: options?.isContinuation
            ? ZEROCliOperation.SystemPrompt
            : ZEROCliOperation.UserPrompt,
        ...
        async ({ metadata: spanMetadata }) => {
          ...
          return promptIdContext.run(prompt_id, async () => {
            const { queryToSend, shouldProceed } = await prepareQueryForZERO(
              query,
              userMessageTimestamp,
              abortSignal,
              prompt_id!,
            ...
            try {
              const stream = zeroClient.sendMessageStream(
                queryToSend,
                abortSignal,
                prompt_id!,
                undefined,
              ...
              const processingStatus = await processZEROStreamEvents(
                stream,
                userMessageTimestamp,
                abortSignal,
              );
  ...
  const handleApprovalModeChange = useCallback(
    ...
    [config, toolCalls, zeroClient, streamingState, addItem, onDebugMessage],
  );
  ...
  const handleCompletedTools = useCallback(
    async (completedToolCallsFromScheduler: TrackedToolCall[]) => {
      ...
      const zeroTools = completedAndReadyToSubmitTools.filter(
        (t) => !t.request.isClientInitiated,
      );

      if (isLowErrorVerbosity) {
        // Low-mode suppression applies only to model-initiated tool failures.
        suppressedToolErrorCountRef.current += zeroTools.filter(
          (tc) => tc.status === CoreToolCallStatus.Error,
        ).length;
      }

      if (zeroTools.length === 0) {
        return;
      }

      // Check if any tool requested to stop execution immediately
      const stopExecutionTool = zeroTools.find(
      ...
      if (stopExecutionTool && stopExecutionTool.response.error) {
        ...
        const callIdsToMarkAsSubmitted = zeroTools.map(
      ...
      // If all the tools were cancelled, don't submit a response to ZERO.
      // Note: we ignore the topic tool because the user doesn't have a chance to decline it.
      const declinableTools = zeroTools.filter(
      ...
      const allToolsCancelled =
        zeroTools.length > 0 &&
        zeroTools.every((tc) => tc.status === CoreToolCallStatus.Cancelled);
      ...
      if (allDeclinableToolsCancelled || allToolsCancelled) {
        ...
        if (zeroClient) {
          ...
          const combinedParts = zeroTools.flatMap(
            (toolCall) => toolCall.response.responseParts,
          );
          // eslint-disable-next-line @typescript-eslint/no-floating-promises
          zeroClient.addHistory({
            role: 'user',
            parts: combinedParts,
          });
        }
      ...
      const responsesToSend: Part[] = zeroTools.flatMap(
        (toolCall) => toolCall.response.responseParts,
      );
      ...
      const prompt_ids = zeroTools.map(
  ...
  }, [toolCalls, config, onDebugMessage, gitService, history, zeroClient]);

  const lastOutputTime = Math.max(
    lastToolOutputTime,
    lastShellOutputTime,
    lastZEROActivityTime,
  );
