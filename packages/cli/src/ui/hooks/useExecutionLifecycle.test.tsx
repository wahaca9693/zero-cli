...
import { NoopSandboxManager, escapeShellArg } from '@allhands/zero-cli-core';
...
describe('useExecutionLifecycle', () => {
  ...
  beforeEach(() => {
    ...
    mockZEROClient = { addHistory: vi.fn() } as unknown as ZEROClient;
    ...
    vi.mocked(fs.mkdtempSync).mockReturnValue('/tmp/zero-shell-abcdef');
  ...
  it('should initiate command execution and set pending state', async () => {
    ...
    const tmpFile = path.join('/tmp/zero-shell-abcdef', 'pwd.tmp');
  ...
  it('should handle successful execution and update history correctly', async () => {
    ...
    expect(mockZEROClient.addHistory).toHaveBeenCalled();
  ...
  it('should handle command abort and display cancelled status', async () => {
    ...
    // to avoid duplication/flickering, as they are handled by useZEROStream.
