...
interface UseSessionResumeParams {
  ...
  refreshStatic: () => void;
  isZEROClientInitialized: boolean;
  ...
  const loadHistoryForResume = useCallback(
    async (
      ...
      try {
        ...
        // Give the history to the ZERO client.
        await config.getZEROClient()?.resumeChat(clientHistory, resumedData);
      } catch (error) {
    ...
    [config, isZEROClientInitialized, setQuittingMessages],
  );
