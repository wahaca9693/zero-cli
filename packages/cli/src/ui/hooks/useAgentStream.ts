...
export const useAgentStream = ({
  ...
  const zeroMessageBufferRef = useRef<string>('');
        ...
        case 'message':
          if (event.role === 'agent') {
            for (const part of event.content) {
              if (part.type === 'text') {
                zeroMessageBufferRef.current += part.text;
                // Update pending history item with incremental text
                const splitPoint = findLastSafeSplitPoint(
                  zeroMessageBufferRef.current,
                );
                if (splitPoint === zeroMessageBufferRef.current.length) {
                  setPendingHistoryItem({
                    type: 'zero',
                    text: zeroMessageBufferRef.current,
                  });
                } else {
                  const before = zeroMessageBufferRef.current.substring(
                    0,
                    splitPoint,
                  );
                  const after =
                    zeroMessageBufferRef.current.substring(splitPoint);
                  addItem(
                    { type: 'zero', text: before },
                    userMessageTimestampRef.current,
                  );
                  zeroMessageBufferRef.current = after;
                  setPendingHistoryItem({
                    type: 'zero_content',
                    text: after,
                  });
  ...
  const submitQuery = useCallback(
    async (
      ...
      const parts = zeroPartsToContentParts(
