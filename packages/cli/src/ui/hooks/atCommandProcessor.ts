...
import type { AnyToolInvocation, Config } from '@allhands/zero-cli-core';
import {
  debugLogger,
  getErrorMessage,
  unescapePath,
...
async function resolveFilePaths(
  ...
  for (const part of fileParts) {
    ...
    const zeroIgnored =
      respectFileIgnore.respectZEROIgnore &&
      fileDiscovery.shouldIgnoreFile(pathName, {
        respectGitIgnore: false,
        respectZEROIgnore: true,
      });

    if (gitIgnored || zeroIgnored) {
      const reason =
        gitIgnored && zeroIgnored ? 'both' : gitIgnored ? 'git' : 'zero';
      ignoredFiles.push({ path: pathName, reason });
      const reasonText =
        reason === 'both'
          ? 'ignored by both git and zero'
          : reason === 'git'
            ? 'git-ignored'
            : 'zero-ignored';
...
async function readLocalFiles(
  ...
  const toolArgs = {
    ...
    file_filtering_options: {
      ...
      respect_zero_ignore: respectFileIgnore.respectZEROIgnore,
    },
  };
...
function reportIgnoredFiles(
  ...
  if (ignoredByReason['zero'].length) {
    messages.push(`ZERO-ignored: ${ignoredByReason['zero'].join(', ')}`);
  }
