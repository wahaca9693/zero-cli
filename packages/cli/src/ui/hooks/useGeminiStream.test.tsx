...
import { useZEROStream } from './useZEROStream.js';
...
const MockedZEROClientClass = vi.hoisted(() =>
  vi.fn().mockImplementation(function (this: any, _config: any) {
    ...
    this.getCurrentSequenceModel = vi
      .fn()
      .mockReturnValue('zero-2.0-flash-exp');
  }),
);
...
vi.mock('@google/zero-cli-core', async (importOriginal) => {
  ...
  return {
    ...
    GitService: vi.fn(),
    ZEROClient: MockedZEROClientClass,
...
// --- Tests for useZEROStream Hook ---
describe('useZEROStream', () => {
  ...
  const mockGetZEROClient = vi.fn().mockImplementation(() => {
    const clientInstance = new MockedZEROClientClass(mockConfig);
  ...
  const mockConfig: Config = {
    ...
    getZEROClient: mockGetZEROClient,
  ...
  beforeEach(() => {
    ...
    // Reset mocks for ZEROClient instance methods (startChat and sendMessageStream)
    // The ZEROClient constructor itself is mocked at the module level.
    mockStartChat.mockClear().mockResolvedValue({
      sendMessageStream: mockSendMessageStream,
    } as unknown as any); // ZEROChat -> any
  ...
  const mockLoadedSettings: LoadedSettings = {
    ...
    workspace: { path: '/workspace/.zero/settings.json', settings: {} },
    errors: [],
    forScope: vi.fn(),
    setValue: vi.fn(),
  ...
  const renderTestHook = async (
    ...
    const client = zeroClient || mockConfig.getZEROClient();
  ...
  const renderHookWithDefaults = async (
    ...
    return renderHookWithProviders(() =>
      useZEROStream(
        new MockedZEROClientClass(mockConfig),
        [],
        mockAddItem,
        mockConfig,
        mockLoadedSettings,
  ...
  it('should inject steering hint prompt for continuation', async () => {
    ...
    mockSendMessageStream.mockReturnValue(
      (async function* () {
        yield {
          type: ServerZEROEventType.Content,
    ...
    expect(mockRunInDevTraceSpan).toHaveBeenCalledWith(
      expect.objectContaining({
        operation: ZEROCliOperation.SystemPrompt,
      }),
      expect.any(Function),
    );
  ...
  it('should handle all tool calls being cancelled', async () => {
    ...
    const client = new MockedZEROClientClass(mockConfig);
  ...
  describe('Retry Handling', () => {
    ...
    it('should reset retryStatus when isResponding becomes false', async () => {
      ...
      const mockStream = (async function* () {
        yield { type: ServerZEROEventType.Content, value: 'Part 1' };
        await new Promise(() => {}); // Keep stream open
      })();
      mockSendMessageStream.mockReturnValue(mockStream);
  ...
  describe('Slash Command Handling', () => {
    ...
    it('should stop processing and not call ZERO when a command is handled without a tool call', async () => {
    ...
    it('should call ZERO with prompt content when slash command returns a `submit_prompt` action', async () => {
    ...
    it('should record client-initiated tool calls in ZEROChat history', async () => {
      const { result, client: mockZEROClient } = await renderTestHook();
      ...
      // Verify that the tool call and response were added to ZEROChat history
      expect(mockZEROClient.addHistory).toHaveBeenCalledWith({
        role: 'model',
        parts: [
          {
            functionCall: {
    ...
    it('should NOT record other client-initiated tool calls in history', async () => {
      ...
      expect(mockZEROClient.addHistory).not.toHaveBeenCalled();
    });
  });
  ...
  describe('Error Handling', () => {
    it('should call parseAndFormatApiError with the correct authType on stream initialization failure', async () => {
      ...
      const { result } = await renderHookWithProviders(() =>
        useZEROStream(
          new MockedZEROClientClass(testConfig),
          [],
          mockAddItem,
          testConfig,
          mockLoadedSettings,
  ...
  describe('handleFinishedEvent', () => {
    it('should add info message for MAX_TOKENS finish reason', async () => {
      ...
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          ...
          yield {
            type: ServerZEROEventType.Finished,
      ...
      it.each([
        ...
        async ({ requestTokens, remainingTokens, expectedMessage }) => {
          mockSendMessageStream.mockReturnValue(
            (async function* () {
              yield {
                type: ServerZEROEventType.ContextWindowWillOverflow,
                value: {
    ...
    it('should add informational messages when ChatCompressed event is received', async () => {
      ...
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          yield {
            type: ServerZEROEventType.ChatCompressed,
            value: {
  ...
  it('should flush pending text rationale before scheduling tool calls to ensure correct history order', async () => {
    ...
    const mockStream = (async function* () {
      ...
      yield {
        type: ServerZEROEventType.ToolCallRequest,
    ...
    // Expectation: addItem:zero (rationale) MUST happen before scheduleToolCalls_START
    const rationaleIndex = addItemOrder.indexOf('addItem:zero');
  ...
  it('should display user query, then tool execution, then model response', async () => {
    ...
    // Mock the ZERO stream to return a model response after the tool
    mockSendMessageStream.mockReturnValue(
      (async function* () {
        yield {
          type: ServerZEROEventType.Content,
  ...
  describe('Thought Reset', () => {
    it('should keep full thinking entries in history when mode is full', async () => {
      ...
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          yield {
            type: ServerZEROEventType.Thought,
            value: {
    ...
    it('should reset thought to null when user cancels', async () => {
      ...
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          ...
          yield { type: ServerZEROEventType.UserCancelled };
        })(),
      );
    ...
    it('should reset thought to null when there is an error', async () => {
      ...
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          ...
          yield {
            type: ServerZEROEventType.Error,
    ...
    it('should update lastOutputTime on ZERO thought and content events', async () => {
      vi.useFakeTimers();
  ...
  describe('Loop Detection Confirmation', () => {
    beforeEach(() => {
      ...
      mockConfig.getZEROClient = vi.fn().mockReturnValue({
        ...new MockedZEROClientClass(mockConfig),
    ...
    it('should set loopDetectionConfirmationRequest when LoopDetected event is received', async () => {
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          ...
          yield {
            type: ServerZEROEventType.LoopDetected,
          };
        })(),
      );
    ...
    it('should disable loop detection and show message when user selects "disable"', async () => {
      ...
      mockConfig.getZEROClient = vi.fn().mockReturnValue(mockClient);
  ...
  describe('Agent Execution Events', () => {
    it('should handle AgentExecutionStopped event with systemMessage', async () => {
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          yield {
            type: ServerZEROEventType.AgentExecutionStopped,
            value: {
              reason: 'hook-reason',
    ...
    it('should handle AgentExecutionBlocked event with systemMessage', async () => {
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          yield {
            type: ServerZEROEventType.AgentExecutionBlocked,
            value: {
              reason: 'hook-reason',
  ...
  describe('Stream Splitting', () => {
    it('should not add empty history item when splitting message results in empty or whitespace-only beforeText', async () => {
      ...
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          yield { type: ServerZEROEventType.Content, value: 'test content' };
        })(),
      );
    ...
    it('should add whitespace-only history item when splitting message', async () => {
      ...
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          yield { type: ServerZEROEventType.Content, value: '   content' };
        })(),
      );
  ...
  it('should trace UserPrompt telemetry on submitQuery', async () => {
    ...
    mockSendMessageStream.mockReturnValue(
      (async function* () {
        yield { type: ServerZEROEventType.Content, value: 'Response' };
      })(),
    );
    ...
    const userPromptCall = mockRunInDevTraceSpan.mock.calls.find(
      (call) =>
        call[0].operation === ZEROCliOperation.UserPrompt ||
