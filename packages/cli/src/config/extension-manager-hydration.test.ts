...
describe('ExtensionManager hydration', () => {
  ...
  beforeEach(() => {
    ...
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'zero-test-'));
    mockHomedir.mockReturnValue(tempDir);
    vi.stubEnv('ZERO_CLI_HOME', tempDir);

    // Create the extensions directory that ExtensionManager expects
    extensionsDir = path.join(tempDir, '.zero', EXTENSIONS_DIRECTORY_NAME);
...
`,
    ...
    extensionManager.setRequestSetting(async (setting) => {
      if (setting.envVar === 'MODEL_NAME') return 'zero-pro';
      return '';
    });
