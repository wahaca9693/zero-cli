...
// Use the (mocked) ZERO_DIR for consistency
...
vi.mock('@allhands/zero-cli-core', async (importOriginal) => {
  ...
  vi.spyOn(actual.Storage.prototype, 'isWorkspaceHomeDir').mockImplementation(
    function (this: Storage) {
      const target = testResolve(pathMod.dirname(this.getZERODir()));
...
describe('Settings Loading and Merging', () => {
  ...
  describe('loadSettings', () => {
    ...
    describe('when ZERO_CLI_SYSTEM_SETTINGS_PATH is set', () => {
  ...
  describe('loadEnvironment', () => {
    function setup({
      ...
      delete process.env['ZERO_API_KEY']; // reset
      delete process.env['TESTTEST']; // reset
      const zeroEnvPath = path.resolve(
        path.join(MOCK_WORKSPACE_DIR, ZERO_DIR, '.env'),
      );
      ...
      (fs.readFileSync as Mock).mockImplementation(
        (p: fs.PathOrFileDescriptor) => {
          ...
          if (normalizedP === path.resolve(USER_SETTINGS_PATH))
            return JSON.stringify(userSettingsContent);
          if (normalizedP === zeroEnvPath || normalizedP === workspaceEnvPath)
    ...
    it('sets environment variables from .env files', () => {
      ...
      expect(process.env['ZERO_API_KEY']).toEqual('test-key');
    });
    ...
    it('loads whitelisted env files from untrusted spaces if sandboxing is enabled', () => {
      ...
      // ZERO_API_KEY is in the whitelist, so it should be loaded.
      expect(process.env['ZERO_API_KEY']).toEqual('test-key');
  ...
  describe('Security and Sandbox', () => {
    ...
    beforeEach(() => {
      ...
      delete process.env['_ZERO_USER_GCP_PROJECT'];
      vi.resetAllMocks();
    ...
    describe('sandbox detection', () => {
      it('should detect sandbox when -s is a real flag', () => {
        process.argv = ['node', 'zero', '-s', 'some prompt'];
        ...
        // If sandboxed and untrusted, FOO should NOT be loaded, but ZERO_API_KEY should be.
        expect(process.env['FOO']).toBeUndefined();
        expect(process.env['ZERO_API_KEY']).toBe('secret');
      });

      it('should detect sandbox when --sandbox is a real flag', () => {
        process.argv = ['node', 'zero', '--sandbox', 'prompt'];
        vi.mocked(isWorkspaceTrusted).mockReturnValue({
          isTrusted: false,
          source: 'file',
        });
        vi.mocked(fs.existsSync).mockReturnValue(true);
        vi.mocked(fs.readFileSync).mockReturnValue('ZERO_API_KEY=secret');
      ...
      it('should ignore sandbox flags if they appear after --', () => {
        ...
        expect(process.env['ZERO_API_KEY']).toEqual('secret');
      });

      it('should NOT be tricked by positional arguments that look like flags', () => {
        process.argv = ['node', 'zero', 'my -s prompt'];
        vi.mocked(isWorkspaceTrusted).mockReturnValue({
          isTrusted: false,
          source: 'file',
        });
    ...
    describe('env var sanitization', () => {
      it('should strictly enforce whitelist in untrusted/sandboxed mode', () => {
        process.argv = ['node', 'zero', '-s', 'prompt'];
        vi.mocked(isWorkspaceTrusted).mockReturnValue({
          isTrusted: false,
          source: 'file',
        });
...
GOOGLE_API_KEY=another-secret
    `);
        ...
        expect(process.env['ZERO_API_KEY']).toBe('secret-key');
      ...
      it('should sanitize shell injection characters in whitelisted env vars in untrusted mode', () => {
        ...
        expect(process.env['ZERO_API_KEY']).toBe('key-whoami-id-');
      });
...
MALICIOUS_VAR=allowed-because-trusted
    `);
        ...
        expect(process.env['ZERO_API_KEY']).toBe('un-sanitized;key!');
    ...
    describe('Cloud Shell security', () => {
      ...
      it('should restore original project when switching to Vertex AI after Cloud Shell override', () => {
        ...
        // First call: saves original to _ZERO_USER_GCP_PROJECT, sets cloudshell-gca
        loadEnvironment(
          createMockSettings({ tools: { sandbox: false } }).merged,
          MOCK_WORKSPACE_DIR,
        );
        expect(process.env['GOOGLE_CLOUD_PROJECT']).toBe('cloudshell-gca');
        expect(process.env['_ZERO_USER_GCP_PROJECT']).toBe('my-real-project');
      ...
      it('should restore project after restart when child inherits cloudshell-gca', () => {
        ...
        process.env['_ZERO_USER_GCP_PROJECT'] = 'my-real-project';
        process.argv = ['node', 'zero', '-s', 'prompt'];
        vi.mocked(isWorkspaceTrusted).mockReturnValue({
          isTrusted: false,
          source: 'file',
