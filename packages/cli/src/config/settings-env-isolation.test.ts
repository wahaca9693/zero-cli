...
import { ZERO_DIR, homedir as coreHomedir } from '@allhands/zero-cli-core';
...
describe('Environment Isolation', () => {
  ...
  it('should load local .env by default', () => {
    ...
    vi.mocked(fs.readFileSync).mockReturnValue('ZERO_API_KEY=local');
    ...
    expect(process.env['ZERO_API_KEY']).toBe('local');
    delete process.env['ZERO_API_KEY'];
  });
  ...
  it('should ignore local .env when ignoreLocalEnv is true', () => {
    ...
    vi.mocked(fs.readFileSync).mockImplementation((p) => {
      const ps = p.toString();
      if (ps === workspaceEnv) return 'ZERO_API_KEY=local';
      if (ps === homeEnv) return 'ZERO_API_KEY=home';
      return '';
    });
    ...
    expect(process.env['ZERO_API_KEY']).toBe('home');
    delete process.env['ZERO_API_KEY'];
  });

  it('should still load .zero/.env even if ignoreLocalEnv is true', () => {
    const workspaceZEROEnv = path.join(mockWorkspace, ZERO_DIR, '.env');
    vi.mocked(fs.existsSync).mockImplementation(
      (p) => p.toString() === workspaceZEROEnv,
    );
    vi.mocked(fs.readFileSync).mockReturnValue('ZERO_API_KEY=zero-local');
    ...
    expect(process.env['ZERO_API_KEY']).toBe('zero-local');
    delete process.env['ZERO_API_KEY'];
  });
  ...
  it('should respect --ignore-env flag', () => {
    ...
    process.argv = ['node', 'zero', '--ignore-env'];
    const settings = { advanced: { ignoreLocalEnv: false } } as Settings;
    loadEnvironment(settings, mockWorkspace, () => ({
      isTrusted: true,
      source: 'file',
    ...
    expect(process.env['ZERO_API_KEY']).toBeUndefined();
  });
  ...
  it('should allow home .env even with ignoreLocalEnv true', () => {
    ...
    vi.mocked(fs.readFileSync).mockReturnValue('ZERO_API_KEY=home');
  ...
  it('should skip local .env and its parents until home when ignoreLocalEnv is true', () => {
    ...
    vi.mocked(fs.readFileSync).mockImplementation((p) => {
      const ps = p.toString();
      if (ps === deepEnv) return 'ZERO_API_KEY=deep';
      if (ps === parentEnv) return 'ZERO_API_KEY=parent';
      if (ps === homeEnv) return 'ZERO_API_KEY=home';
      return '';
    });
  ...
  it('should respect both -s and --ignore-env flags simultaneously', () => {
    ...
    process.argv = ['node', 'zero', '-s', '--ignore-env'];
    const settings = { advanced: { ignoreLocalEnv: false } } as Settings;
    loadEnvironment(settings, mockWorkspace, () => ({
      isTrusted: true,
      source: 'file',
