...
export const DEFAULT_EXCLUDED_ENV_VARS = [
  'DEBUG',
  'DEBUG_MODE',
  'ZERO_CLI_IDE_SERVER_STDIO_COMMAND',
  'ZERO_CLI_IDE_SERVER_STDIO_ARGS',
];
...
export function getSystemSettingsPath(): string {
  ...
  if (platform() === 'darwin') {
    return '/Library/Application Support/ZEROCli/settings.json';
  } else if (platform() === 'win32') {
    return 'C:\\ProgramData\\zero-cli\\settings.json';
  } else {
    return '/etc/zero-cli/settings.json';
  }
}

export function getSystemDefaultsPath(): string {
  if (process.env['ZERO_CLI_SYSTEM_DEFAULTS_PATH']) {
    return process.env['ZERO_CLI_SYSTEM_DEFAULTS_PATH'];
  }
  return path.join(
...
function findEnvFile(
  ...
  while (true) {
    ...
    if (parentDir === currentDir || !parentDir) {
      ...
      if (isTrusted) {
        const homeZEROEnvPath = path.join(homedir(), ZERO_DIR, '.env');
...
const USER_GCP_PROJECT = '_ZERO_USER_GCP_PROJECT';
...
export function loadEnvironment(
  ...
  if (envFilePath) {
    ...
    try {
      ...
      const isProjectEnvFile = !envFilePath.includes(ZERO_DIR);
