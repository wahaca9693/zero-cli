...
vi.mock('@google/zero-cli-core', async () => {
  ...
  return {
    ...
    getAdminErrorMessage: vi.fn(
      (_feature) =>
        `YOLO mode is disabled by your administrator. To enable it, please request an update to the settings at: https://goo.gle/manage-zero-cli`,
    ),
...
const originalZEROModel = process.env['ZERO_MODEL'];
...
afterEach(() => {
  ...
  if (originalZEROModel !== undefined) {
    process.env['ZERO_MODEL'] = originalZEROModel;
  } else {
    delete process.env['ZERO_MODEL'];
  }
  Object.defineProperty(process.stdout, 'isTTY', {
    ...
    it.each([
      {
        ...
        argv: ['node', 'script.js', 'Hi ZERO'],
        expectedQuery: 'Hi ZERO',
...
describe('loadCliConfig', () => {
  beforeEach(() => {
    ...
    vi.stubEnv('ZERO_API_KEY', 'test-api-key');
  ...
  describe('Model resolution', () => {
    it('should handle multiple --model flags by taking the last one', async () => {
      const argv = {
        ...
        model: ['zero-1.5-pro', 'zero-2.0-flash'] as any,
        sandbox: undefined,
        debug: false,
        prompt: undefined,
      ...
      expect(config.getModel()).toBe('zero-2.0-flash');
    });
  ...
  it('should add IDE workspace folders from ZERO_CLI_IDE_WORKSPACE_PATH to include directories', async () => {
    vi.stubEnv(
  ...
  it('should skip inaccessible workspace folders from ZERO_CLI_IDE_WORKSPACE_PATH', async () => {
  ...
  it('should use default fileFilter options when unconfigured', async () => {
    ...
    expect(config.getFileFilteringRespectZEROIgnore()).toBe(
...
describe('loadCliConfig model selection', () => {
  ...
  it('selects a model from settings.json if provided', async () => {
    ...
    expect(config.getModel()).toBe('zero-2.5-pro');
  });

  it('uses the default zero model if nothing is set', async () => {
  ...
  it('always prefers model from argv', async () => {
    process.argv = ['node', 'script.js', '--model', 'zero-2.5-flash-preview'];
    const argv = await parseArguments(createTestMergedSettings());
    const config = await loadCliConfig(
      createTestMergedSettings({
        model: {
    ...
    expect(config.getModel()).toBe('zero-2.5-flash-preview');
  });
...
describe('loadCliConfig folderTrust', () => {
  ...
  beforeEach(() => {
    ...
    originalIntegrationTest = process.env['ZERO_CLI_INTEGRATION_TEST'];
    delete process.env['VITEST'];
    delete process.env['ZERO_CLI_INTEGRATION_TEST'];
  });
  ...
  afterEach(() => {
    ...
    if (originalIntegrationTest !== undefined) {
      process.env['ZERO_CLI_INTEGRATION_TEST'] = originalIntegrationTest;
    }
...
describe('loadCliConfig interactive', () => {
  ...
  it('should be interactive if positional prompt words are provided with other flags', async () => {
    process.stdin.isTTY = true;
    process.argv = ['node', 'script.js', '--model', 'zero-2.5-pro', 'Hello'];
    const argv = await parseArguments(createTestMergedSettings());
    const config = await loadCliConfig(
      createTestMergedSettings(),
      'test-session',
  ...
  it('should handle multiple positional words with flags', async () => {
    ...
    expect(argv.model).toBe('zero-2.5-pro');
  });
  ...
  it('should be interactive if no positional prompt words are provided with flags', async () => {
    process.stdin.isTTY = true;
    process.argv = ['node', 'script.js', '--model', 'zero-2.5-pro'];
    const argv = await parseArguments(createTestMergedSettings());
    const config = await loadCliConfig(
      createTestMergedSettings(),
      'test-session',
...
describe('loadCliConfig fileFiltering', () => {
  ...
  const testCases: Array<{
    ...
    {
      property: 'respectZEROIgnore',
      getter: (c) => c.getFileFilteringRespectZEROIgnore(),
      value: true,
    },
    {
      property: 'respectZEROIgnore',
...
describe('Telemetry configuration via environment variables', () => {
  ...
  it('should prioritize ZERO_TELEMETRY_ENABLED over settings', async () => {
    vi.stubEnv('ZERO_TELEMETRY_ENABLED', 'true');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments(createTestMergedSettings());
    const settings = createTestMergedSettings({
  ...
  it('should prioritize ZERO_TELEMETRY_TARGET over settings', async () => {
    vi.stubEnv('ZERO_TELEMETRY_TARGET', 'gcp');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments(createTestMergedSettings());
    const settings = createTestMergedSettings({
  ...
  it('should throw when ZERO_TELEMETRY_TARGET is invalid', async () => {
    vi.stubEnv('ZERO_TELEMETRY_TARGET', 'bogus');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments(createTestMergedSettings());
    const settings = createTestMergedSettings({
  ...
  it('should prioritize ZERO_TELEMETRY_OTLP_ENDPOINT over settings and default env var', async () => {
    vi.stubEnv('OTEL_EXPORTER_OTLP_ENDPOINT', 'http://default.env.com');
    vi.stubEnv('ZERO_TELEMETRY_OTLP_ENDPOINT', 'http://zero.env.com');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments(createTestMergedSettings());
    const settings = createTestMergedSettings({
    ...
    const config = await loadCliConfig(settings, 'test-session', argv);
    expect(config.getTelemetryOtlpEndpoint()).toBe('http://zero.env.com');
  });

  it('should prioritize ZERO_TELEMETRY_OTLP_PROTOCOL over settings', async () => {
    vi.stubEnv('ZERO_TELEMETRY_OTLP_PROTOCOL', 'http');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments(createTestMergedSettings());
    const settings = createTestMergedSettings({
  ...
  it('should prioritize ZERO_TELEMETRY_LOG_PROMPTS over settings', async () => {
    vi.stubEnv('ZERO_TELEMETRY_LOG_PROMPTS', 'false');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments(createTestMergedSettings());
    const settings = createTestMergedSettings({
  ...
  it('should prioritize ZERO_TELEMETRY_OUTFILE over settings', async () => {
    vi.stubEnv('ZERO_TELEMETRY_OUTFILE', '/zero/env/telemetry.log');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments(createTestMergedSettings());
    const settings = createTestMergedSettings({
    ...
    const config = await loadCliConfig(settings, 'test-session', argv);
    expect(config.getTelemetryOutfile()).toBe('/zero/env/telemetry.log');
  });

  it('should prioritize ZERO_TELEMETRY_USE_COLLECTOR over settings', async () => {
    vi.stubEnv('ZERO_TELEMETRY_USE_COLLECTOR', 'true');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments(createTestMergedSettings());
    const settings = createTestMergedSettings({
  ...
  it('should use settings value when ZERO_TELEMETRY_ENABLED is not set', async () => {
    vi.stubEnv('ZERO_TELEMETRY_ENABLED', undefined);
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments(createTestMergedSettings());
  ...
  it('should use settings value when ZERO_TELEMETRY_TARGET is not set', async () => {
    vi.stubEnv('ZERO_TELEMETRY_TARGET', undefined);
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments(createTestMergedSettings());
    const settings = createTestMergedSettings({
      telemetry: { target: ServerConfig.TelemetryTarget.LOCAL },
  ...
  it("should treat ZERO_TELEMETRY_ENABLED='1' as true", async () => {
    vi.stubEnv('ZERO_TELEMETRY_ENABLED', '1');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments(createTestMergedSettings());
    const config = await loadCliConfig(
      createTestMergedSettings(),
  ...
  it("should treat ZERO_TELEMETRY_ENABLED='0' as false", async () => {
    vi.stubEnv('ZERO_TELEMETRY_ENABLED', '0');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments(createTestMergedSettings());
    const config = await loadCliConfig(
  ...
  it("should treat ZERO_TELEMETRY_LOG_PROMPTS='1' as true", async () => {
    vi.stubEnv('ZERO_TELEMETRY_LOG_PROMPTS', '1');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments(createTestMergedSettings());
    const config = await loadCliConfig(
      createTestMergedSettings(),
  ...
  it("should treat ZERO_TELEMETRY_LOG_PROMPTS='false' as false", async () => {
    vi.stubEnv('ZERO_TELEMETRY_LOG_PROMPTS', 'false');
    process.argv = ['node', 'script.js'];
    const argv = await parseArguments(createTestMergedSettings());
    const config = await loadCliConfig(
  ...
  describe('extension plan settings', () => {
    beforeEach(() => {
      vi.spyOn(Storage.prototype, 'getProjectTempDir').mockReturnValue(
        '/mock/home/user/.zero/tmp/test-project',
      );
    });
    ...
    it('should use plan directory from active extension when user has not specified one', async () => {
      ...
      vi.spyOn(ExtensionManager.prototype, 'getExtensions').mockReturnValue([
        ...
        } as unknown as ZEROCLIExtension,
      ]);
