...
describe('Trusted Folders', () => {
  ...
  beforeEach(() => {
    ...
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'zero-cli-test-'));
    ...
    resetTrustedFoldersForTesting();
    vi.clearAllMocks();
    delete process.env['ZERO_CLI_TRUST_WORKSPACE'];
  });
  ...
  describe('isWorkspaceTrusted headless mode', () => {
    ...
    it('should NOT return true when isHeadlessMode is true, ignoring config', async () => {
      const zeroCore = await import('@allhands/zero-cli-core');
      vi.spyOn(zeroCore, 'isHeadlessMode').mockReturnValue(true);
    ...
    it('should return true when ZERO_CLI_TRUST_WORKSPACE is true', async () => {
    ...
    it('should fall back to config when isHeadlessMode is false', async () => {
      const zeroCore = await import('@allhands/zero-cli-core');
      vi.spyOn(zeroCore, 'isHeadlessMode').mockReturnValue(false);
  ...
  describe('Verification: Auth and Trust Interaction', () => {
    it('should verify loadEnvironment returns early when untrusted', () => {
      ...
      fs.writeFileSync(envPath, 'ZERO_API_KEY=secret', 'utf-8');
