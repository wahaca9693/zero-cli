...
export class ExtensionManager extends ExtensionLoader {
  ...
  private loadedExtensions: ZEROCLIExtension[] | undefined;
  private loadingPromise: Promise<ZEROCLIExtension[]> | null = null;
  ...
  async installOrUpdateExtension(
    ...
    let extension: ZEROCLIExtension | null;
    try {
...
Would you like to attempt to install via "git clone" instead?`,
      ...
      try {
        ...
        if (missingSettings.length > 0) {
          const message = `Extension "${newExtensionConfig.name}" has missing settings: ${missingSettings
            ...
            .join(
              ', ',
            )}. Please run "zero extensions config ${newExtensionConfig.name} [setting-name]" to configure them.`;
  ...
  protected override async startExtension(extension: ZEROCLIExtension) {
  ...
  protected override async stopExtension(extension: ZEROCLIExtension) {
   ...
   */
  async loadExtensions(): Promise<ZEROCLIExtension[]> {
    if (this.loadedExtensions) {
    ...
    this.loadingPromise = (async () => {
      try {
        ...
        const builtExtensions = builtExtensionsOrNull.filter(
          (ext): ext is ZEROCLIExtension => ext !== null,
        );
  ...
  toOutputString(extension: ZEROCLIExtension): string {
