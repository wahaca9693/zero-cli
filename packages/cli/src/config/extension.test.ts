...
describe('extension tests', () => {
  ...
  beforeEach(() => {
    ...
    tempHomeDir = getRealPath(
      fs.mkdtempSync(path.join(os.tmpdir(), 'zero-cli-test-home-')),
    );
    tempWorkspaceDir = getRealPath(
      fs.mkdtempSync(path.join(tempHomeDir, 'zero-cli-test-workspace-')),
    );
  ...
  describe('loadExtensions', () => {
    ...
    it('should load context file path when ZERO.md is present', async () => {
      createExtension({
        extensionsDir: userExtensionsDir,
        name: 'ext1',
        version: '1.0.0',
      ...
      expect(ext1?.contextFiles).toEqual([
        path.join(userExtensionsDir, 'ext1', 'ZERO.md'),
      ]);
...
`;
    ...
    describe('id generation', () => {
      ...
      it('should warn about hooks during installation', async () => {
        ...
        fs.writeFileSync(
          path.join(sourceExtDir, 'zero-extension.json'),
          JSON.stringify({
  ...
  describe('installExtension', () => {
    ...
    it('should throw an error and cleanup if zero-extension.json is missing', async () => {
    ...
    it('should throw an error for invalid JSON in zero-extension.json', async () => {
    ...
    it('should throw an error for missing name in zero-extension.json', async () => {
      const sourceExtDir = getRealPath(
        createExtension({
          extensionsDir: tempHomeDir,
    ...
    it('should add the workspace to trusted folders if user consents', async () => {
      ...
      vi.stubEnv('ZERO_CLI_TRUSTED_FOLDERS_PATH', trustedFoldersPath);
      vi.mocked(isWorkspaceTrusted).mockReturnValue({
        isTrusted: false,
        source: undefined,
      });
...
${INSTALL_WARNING_MESSAGE}`,
    ...
    describe('installing from github', () => {
      const gitUrl = 'https://github.com/google/zero-test-extension.git';
      const extensionName = 'zero-test-extension';
  ...
  describe('uninstallExtension', () => {
    ...
    it('should uninstall an extension by its source URL', async () => {
      const gitUrl = 'https://github.com/google/zero-sql-extension.git';
      const sourceExtDir = createExtension({
        extensionsDir: userExtensionsDir,
        name: 'zero-sql-extension',
        version: '1.0.0',
        installMetadata: {
          source: gitUrl,
          type: 'git',
      ...
      expect(ExtensionUninstallEvent).toHaveBeenCalledWith(
        'zero-sql-extension',
        hashValue('zero-sql-extension'),
        hashValue('https://github.com/google/zero-sql-extension'),
        'success',
      );
    });
  ...
  describe('enableExtension', () => {
    ...
    const getActiveExtensions = (): ZEROCLIExtension[] => {
      const extensions = extensionManager.getExtensions();
