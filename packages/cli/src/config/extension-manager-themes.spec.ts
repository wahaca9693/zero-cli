...
describe('ExtensionManager theme loading', () => {
  ...
  beforeAll(async () => {
    tempHomeDir = await fs.promises.mkdtemp(
      path.join(tmpdir(), 'zero-cli-test-'),
    );
  });
  ...
  beforeEach(() => {
    process.env['ZERO_CLI_HOME'] = tempHomeDir;
    userExtensionsDir = path.join(tempHomeDir, ZERO_DIR, 'extensions');
