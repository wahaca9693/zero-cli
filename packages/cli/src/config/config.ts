...
import {
  setZEROMdFilename as setServerZEROMdFilename,
  resetZEROMdFilename,
...
export async function parseArguments(
  ...
  const yargsInstance = yargs(rawArgv)
    ...
    .scriptName('zero')
    .usage(
      'Usage: zero [options] [command]\n\nZERO CLI - Defaults to interactive mode. Use -p/--prompt for non-interactive (headless) mode.',
    )
    .option('isCommand', {
      type: 'boolean',
      hidden: true,
  ...
  yargsInstance
    .command('$0 [query..]', 'Launch ZERO CLI', (yargsInstance) =>
      yargsInstance
        .positional('query', {
          description:
        ...
        .option('worktree', {
          ...
          skipValidation: true,
          description:
            'Start ZERO in a new git worktree. If no name is provided, one is generated automatically.',
        ...
        .option('allowed-tools', {
          ...
          nargs: 1,
          description:
            '[DEPRECATED: Use Policy Engine instead See https://zerocli.com/docs/core/policy-engine] Tools that are allowed to run without confirmation',
          coerce: coerceCommaSeparated,
        })
        .option('extensions', {
          alias: 'e',
        ...
        .option('resume', {
          ...
          coerce: (value: string): string => {
            // When --resume passed with a value (`zero --resume 123`): value = "123" (string)
            // When --resume passed without a value (`zero --resume`): value = "" (string)
  ...
  try {
    ...
    if (result['skip-trust']) {
      process.env['ZERO_CLI_TRUST_WORKSPACE'] = 'true';
    }
  } catch (e) {
...
export async function loadCliConfig(
  ...
  // directly to the Config constructor in core, and have core handle setZEROMdFilename.
  // However, loadHierarchicalZEROMemory is called *before* createServerConfig.
  if (settings.context?.fileName) {
    setServerZEROMdFilename(settings.context.fileName);
  } else {
  ...
  // so ZERO has context of all open folders, not just the cwd.
  const ideWorkspacePath = process.env['ZERO_CLI_IDE_WORKSPACE_PATH'];
  if (ideWorkspacePath) {
  ...
  let extensionRegistryURI =
    process.env['ZERO_CLI_EXTENSION_REGISTRY_URI'] ??
  ...
  const defaultModel = ZERO_MODEL_ALIAS_AUTO;
  const rawModel =
    argv.model || process.env['ZERO_MODEL'] || settings.model?.name;
  ...
  const resolvedModel =
    specifiedModel === ZERO_MODEL_ALIAS_AUTO
      ? defaultModel
  ...
  return new Config({
    ...
    sessionId,
    clientVersion: await getVersion(),
    embeddingModel: DEFAULT_ZERO_EMBEDDING_MODEL,
    ...
    // recursive continuation turns inside ZEROClient.processTurn() that
...
async function resolveWorktreeSettings(
  ...
  try {
    ...
    if (isZEROWorktree(toplevel, projectRoot)) {
