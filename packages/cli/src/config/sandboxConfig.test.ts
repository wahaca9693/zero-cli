...
describe('loadSandboxConfig', () => {
  ...
  beforeEach(() => {
    ...
    delete process.env['ZERO_SANDBOX'];
  ...
  describe('with ZERO_SANDBOX environment variable', () => {
    it('should use docker if ZERO_SANDBOX=docker and it exists', async () => {
      process.env['ZERO_SANDBOX'] = 'docker';
    ...
    it('should throw if ZERO_SANDBOX is an invalid command', async () => {
      process.env['ZERO_SANDBOX'] = 'invalid-command';
    ...
    it('should throw if ZERO_SANDBOX command does not exist', async () => {
      process.env['ZERO_SANDBOX'] = 'docker';
      mockedCommandExistsSync.mockReturnValue(false);
      await expect(loadSandboxConfig({}, {})).rejects.toThrow(
        "Missing sandbox command 'docker' (from ZERO_SANDBOX)",
      );
    });

    it('should use lxc if ZERO_SANDBOX=lxc and it exists', async () => {
      process.env['ZERO_SANDBOX'] = 'lxc';
      mockedCommandExistsSync.mockReturnValue(true);
      const config = await loadSandboxConfig({}, {});
      expect(config).toEqual({
        enabled: true,
    ...
    it('should throw if ZERO_SANDBOX=lxc but lxc command does not exist', async () => {
      process.env['ZERO_SANDBOX'] = 'lxc';
      mockedCommandExistsSync.mockReturnValue(false);
      await expect(loadSandboxConfig({}, {})).rejects.toThrow(
        "Missing sandbox command 'lxc' (from ZERO_SANDBOX)",
      );
    });
  });
  ...
  describe('with sandbox: true', () => {
    ...
    it('should throw if sandbox: true but no command is found', async () => {
      ...
      await expect(loadSandboxConfig({}, { sandbox: true })).rejects.toThrow(
        'ZERO_SANDBOX is true but failed to determine command for sandbox; ' +
          'install docker or podman or specify command in ZERO_SANDBOX',
      );
    });
  });
  ...
  describe('image configuration', () => {
    it('should use image from ZERO_SANDBOX_IMAGE env var if set', async () => {
      process.env['ZERO_SANDBOX_IMAGE'] = 'env/image';
      process.env['ZERO_SANDBOX'] = 'docker';
      mockedCommandExistsSync.mockReturnValue(true);
      const config = await loadSandboxConfig({}, {});
      expect(config).toEqual({
  ...
  describe('with sandbox: runsc (gVisor)', () => {
    ...
    it('should use runsc via ZERO_SANDBOX environment variable', async () => {
      process.env['ZERO_SANDBOX'] = 'runsc';
      const config = await loadSandboxConfig({}, {});
    ...
    it('should prioritize ZERO_SANDBOX over CLI and settings', async () => {
      process.env['ZERO_SANDBOX'] = 'runsc';
      const config = await loadSandboxConfig(
    ...
    it('should throw if runsc binary not found', async () => {
      ...
      await expect(loadSandboxConfig({}, { sandbox: 'runsc' })).rejects.toThrow(
        "Missing sandbox command 'runsc' (from ZERO_SANDBOX)",
      );
    });
