...
export function getNodeMemoryArgs(isDebugMode: boolean): string[] {
  ...
  if (process.env['ZERO_CLI_NO_RELAUNCH']) {
    return [];
  }
...
export async function main() {
  ...
  if (
    ...
    coreEvents.emitFeedback(
      'warning',
      'Warning: --allowed-tools cli argument and tools.allowed in settings.json are deprecated and will be removed in 1.0: Migrate to Policy Engine: https://zerocli.com/docs/core/policy-engine/',
    );
  }
  ...
  if (
    ...
    coreEvents.emitFeedback(
      'warning',
      'Warning: tools.exclude in settings.json is deprecated and will be removed in 1.0. Migrate to Policy Engine: https://zerocli.com/docs/core/policy-engine/',
    );
  }
  ...
  // to run ZERO CLI. It is now safe to perform expensive initialization that
  // may have side effects.
  {
    ...
    if (!input) {
      debugLogger.error(
        `No input provided via stdin. Input can be provided by piping data into zero or using the --prompt option.`,
      );
      await runExitCleanup();
      process.exit(ExitCodes.FATAL_INPUT_ERROR);
    }
