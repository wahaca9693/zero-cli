...
import { ZEROCliAgent } from './agent.js';
...
describe('ZEROCliAgent Integration', () => {
  beforeEach(() => {
  ...
  it('throws on invalid instructions', () => {
    ...
    expect(() => new ZEROCliAgent({} as any).session()).not.toThrow();

    expect(() =>
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      new ZEROCliAgent({ instructions: 123 as any }).session(),
