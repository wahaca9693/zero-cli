...
/**
 * The main entry point for the ZERO CLI SDK.
 *
 ...
 * ```
 */
export class ZEROCliAgent {
  private options: ZEROCliAgentOptions;

  constructor(options: ZEROCliAgentOptions) {
   ...
   * @returns A new {@link ZEROCliSession} instance.
   */
  session(options?: { sessionId?: string }): ZEROCliSession {
    const sessionId = options?.sessionId || createSessionId();
    return new ZEROCliSession(this.options, sessionId, this);
  }
   ...
   * @returns A {@link ZEROCliSession} with the prior conversation loaded.
   * @throws {Error} If no sessions exist or the specified ID is not found.
   */
  async resumeSession(sessionId: string): Promise<ZEROCliSession> {
