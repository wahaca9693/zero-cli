...
> **Status:** Implemented. `ZEROCliAgent` supports `session()` and
> `resumeSession()`.

Equivalent to `zero -p "what does this project do?"`. Loads all workspace and
user settings.

```ts
import { ZEROCliAgent } from '@google/zero-cli-sdk';

const simpleAgent = new ZEROCliAgent({
...
- Static string instructions show up where ZERO.md content normally would in
  model call
- Dynamic instructions show up and contain dynamic content.

## `Custom Tools`

> **Status:** Implemented. `tool()` helper and `ZEROCliAgent` support custom
> tool definitions and execution.

```ts
import { ZEROCliAgent, tool, z } from "@google/zero-cli-sdk";
...
const toolAgent = new ZEROCliAgent({
  tools: [addTool],
});
...
```ts
import { ZEROCliAgent, hook, z } from '@google/zero-cli-sdk';
...
const myHook = hook(
  ...
  (hook, ctx) => {
    ...
    // ctx.fs gives us a filesystem interface that obeys ZERO CLI permissions/sandbox
...
```ts
import { hook } from "@google/zero-cli-sdk";
...
> **Status:** Implemented. `skillDir` helper and `ZEROCliAgent` support
...
```typescript
import { ZEROCliAgent, skillDir } from '@google/zero-sdk';
...
```ts
import { ZEROCliAgent, subagent } from "@google/zero-cli";
...
Potentially the most important feature of the ZERO CLI SDK is support for
extensions, which modularly encapsulate all of the primitives listed above:

```ts
import { ZEROCliAgent, extension } from "@google/zero-cli-sdk";
...
```ts
import { ZEROCliAgent } from "@google/zero-cli-sdk";
import { ZEROCliAcpServer } from "@google/zero-cli-sdk/acp";

const server = new ZEROCliAcpServer(new ZEROCliAgent({...}));
...
export interface AgentShellProcess {
  ...
  // investigate how ZERO CLI already does this
}
```
...
- To validate the SDK, it would be useful to have a robust way to mock the
  ...
  deterministic.
- Need to work in both ZERO-CLI-triggered approvals and optional
...
a `hooks` option to `ZEROCliAgentOptions`.

**Tasks:**

1.  Define `Hook` interface and helper functions.
2.  Add `hooks` option to `ZEROCliAgentOptions`.
3.  Implement hook registration logic in `ZEROCliAgent`.
