...
import { TestRig, MemoryTestHarness } from '@google/zero-cli-test-utils';
...
const TEST_ENV = {
  ZERO_API_KEY: 'fake-memory-test-key',
  ZERO_MEMORY_MONITOR_INTERVAL: '100',
};
...
async function generateSharedLargeChatData(tempDir: string) {
  ...
  for (let i = 0; i < messages.length; i++) {
    ...
    if (msg.type === 'user') {
      ...
      // Find all subsequent zero messages until the next user message
      let j = i + 1;
      while (j < messages.length && messages[j].type === 'zero') {
        const zeroMsg = messages[j];
        const parts = [];
        if (zeroMsg.content) {
          parts.push({ text: zeroMsg.content });
        }
        if (zeroMsg.toolCalls) {
          for (const tc of zeroMsg.toolCalls) {
            parts.push({
              functionCall: {
                name: tc.name,
                args: tc.args,
      ...
      // Skip the zero messages we just processed
      i = j - 1;
    }
  }
