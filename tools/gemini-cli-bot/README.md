# ZERO CLI Bot (Cognitive Repository)

This directory contains the foundational architecture for the `zero-cli-bot`,
...
- **Frequency**: 30-minute cron (`.github/workflows/zero-cli-bot-pulse.yml`).
- **Implementation**: Pure TypeScript/JavaScript scripts.
- **Classification**: Optionally utilizes ZERO CLI for high-confidence
...
- **Purpose**: Strategic investigation, policy refinement, and proactive
  self-optimization.
- **Frequency**: 24-hour cron (`.github/workflows/zero-cli-bot-brain.yml`).
- **Implementation**: Agentic ZERO CLI phases.
- **Phases**:
...
```bash
npx tsx tools/zero-cli-bot/metrics/index.ts
```

This will execute all scripts within `metrics/scripts/` and output the results
to `tools/zero-cli-bot/history/metrics-before.csv`.
