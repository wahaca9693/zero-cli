...
- **[Before you begin](#before-you-begin):** Essential steps to take before
  becoming a ZERO CLI contributor.
...
- **[Documentation contribution process](#documentation-contribution-process):**
  How to contribute documentation to ZERO CLI.
...
1.  **Using the helper script (Recommended):** We provide a script that
    ...
    **Note on Models:** By default, the script uses the latest Pro model
    (`zero-3.1-pro-preview`). If you do not have enough Pro quota, you can run
    it with the latest Flash model instead:
    `./scripts/review.sh <PR_NUMBER> zero-3-flash-preview`.

2.  **Manually from within ZERO CLI:** If you already have the PR checked out
...
time and that only
[issues labeled "help wanted"](https://github.com/google-zero/zero-cli/issues?q=is%3Aissue%20state%3Aopen%20label%3A%22help%20wanted%22)
...
with a value of `ZERO_API_KEY` and set that to a valid API key that you have
...
```bash
git clone https://github.com/google-zero/zero-cli.git # Or your fork's URL
cd zero-cli
```
...
setting `ZERO_SANDBOX=true` in your `~/.env` and ensuring a sandboxing
provider (e.g. `macOS Seatbelt`, `docker`, or `podman`) is available. See
[Sandboxing](#sandboxing) for details.

To build both the `zero` CLI utility and the sandbox container, run
...
To start the ZERO CLI from the source code (after building), run the following
...
If you'd like to run the source build outside of the zero-cli folder, you can
utilize `npm link path/to/zero-cli/packages/cli` (see:
[docs](https://docs.npmjs.com/cli/v9/commands/npm-link)) or
`alias zero="node path/to/zero-cli/packages/cli"` to run with `zero`
...
the ZERO CLI. They are not run as part of the default `npm run test` command.
...
the
[Integration Tests documentation](https://zerocli.com/docs/integration-tests).
...
- Consult
  [ZERO.md](https://github.com/google-zero/zero-cli/blob/main/ZERO.md)
...
1.  Start the CLI in debug mode from the root directory:
    ...
    npm run debug
    ```
    This command runs `node --inspect-brk dist/zero.js` within the
...
zero-cli due to automatic exclusion. Use `.zero/.env` files for zero-cli
...
1.  **Start the ZERO CLI in development mode:**
...
On macOS, `zero` uses Seatbelt (`sandbox-exec`) under a `permissive-open`
...
`.zero/sandbox-macos-<profile>.sb` under your project settings directory
`.zero`.

#### Container-based sandboxing (all platforms)

For stronger container-based sandboxing on macOS or other platforms, you can set
`ZERO_SANDBOX=true|docker|podman|<command>` in your environment or `.env`
...
as you start/stop ZERO CLI. Files created within the sandbox should be
...
for your projects by creating the files `.zero/sandbox.Dockerfile` and/or
`.zero/sandbox.bashrc` under your project settings directory (`.zero`) and
running `zero` with `BUILD_SANDBOX=1` to trigger building of your custom
sandbox.
...
can be specified as `ZERO_SANDBOX_PROXY_COMMAND=<command>`, where `<command>`
...
[sidebar.json](https://github.com/google-zero/zero-cli/blob/main/docs/sidebar.json)
...
- Check our [FAQ](https://zerocli.com/docs/resources/faq).
- Review existing documentation for examples.
- Open [an issue](https://github.com/google-zero/zero-cli/issues) to discuss
  your proposed changes.
- Reach out to the maintainers.

We appreciate your contributions to making ZERO CLI documentation better!
