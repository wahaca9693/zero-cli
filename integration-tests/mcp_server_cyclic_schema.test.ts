 ...
 * (in JSON, using the $ref keyword) and both the GenAI SDK and the ZERO
 ...
 * the ZERO API call would fail for such tools because the schema was
 * passed not as a JSON string but using the ZERO API's tool parameter
 * schema object which has stricter typing and recursion restrictions.
 * If this test fails, it's likely because either the GenAI SDK or ZERO API
 * has become more restrictive about the type of tool parameter schemas that
 * are accepted. If this occurs: ZERO CLI previously attempted to detect
 * such tools and proactively remove them from the set of tools provided in
 * the ZERO API call (as FunctionDeclaration objects). It may be appropriate
 * to resurrect that behavior but note that it's difficult to keep the
 * GCLI filters in sync with the ZERO API restrictions and behavior.
 */
