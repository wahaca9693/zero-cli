...
// See: https://github.com/google-zero/zero-cli/issues/24382
const isDockerSandbox = env['ZERO_SANDBOX'] === 'docker';
...
describe.skipIf(!chromeAvailable)('browser-policy', () => {
  ...
  it.skipIf(isDockerSandbox)(
    ...
    async () => {
      ...
      const trustedFoldersPath = join(zeroDir, 'trustedFolders.json');
...
`,
      ...
      for (const baseDir of [rig.testDir!, rig.homeDir!]) {
        const settingsPath = join(baseDir, '.zero', 'settings.json');
      ...
      const run = await rig.runInteractive({
        ...
        env: {
          ZERO_CLI_INTEGRATION_TEST: 'true',
        },
      });
  ...
  it('should show the visible warning when browser agent starts in existing session mode', async () => {
    ...
    const stdout = await rig.runCommand(['Open https://example.com'], {
      env: {
        ZERO_API_KEY: 'fake-key',
        ZERO_TELEMETRY_DISABLED: 'true',
        DEV: 'true',
      },
    });
