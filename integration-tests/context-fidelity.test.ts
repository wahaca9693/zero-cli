...
import type { FakeResponse, HistoryTurn } from '@google/zero-cli-core';
...
describe('Context Management Fidelity E2E', () => {
  ...
  it(
    ...
    async () => {
      ...
      fs.writeFileSync(
        path.join(rig.testDir!, '.zeroignore'),
        'traces/\nresp*.json\ndebug.log\n',
      );

      const commonEnv = {
        ZERO_API_KEY: 'mock-key',
        ZERO_CONTEXT_TRACE_DIR: traceDir,
        ZERO_CONTEXT_TRACE_ENABLED: 'true',
        ZERO_DEBUG_LOG_FILE: path.join(rig.testDir!, 'debug.log'),
      };
