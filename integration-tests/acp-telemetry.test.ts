...
const sandboxEnv = env['ZERO_SANDBOX'];
...
describe('ACP telemetry', () => {
  ...
  itMaybe('should flush telemetry when connection closes', async () => {
    ...
    const bundlePath = join(import.meta.dirname, '..', 'bundle/zero.js');
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          ZERO_CLI_HOME: rig.homeDir!,
          ZERO_TELEMETRY_ENABLED: 'true',
          ZERO_TELEMETRY_TRACES_ENABLED: 'true',
          ZERO_TELEMETRY_TARGET: 'local',
          ZERO_TELEMETRY_OUTFILE: telemetryPath,
        },
      },
    );
